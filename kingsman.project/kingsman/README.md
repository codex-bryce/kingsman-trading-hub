...
This project is configured for Vercel with `vercel.json`. Set the required
build-time variables in the Vercel project settings, using `.env.example` as
the reference. Then deploy with:
...
Vercel uses `dist` as the output directory and routes client-side application
