...
ws.onmessage = (event) => {
  ...
  if (data.active_symbols) {
    ...
    console.log(
      "1-Second Volatility Indices found:",
      oneSecondVolatilityIndices,
    );
    ...
    oneSecondVolatilityIndices.forEach((symbolData) => {
      ...
      subscribeTicks(symbol);
      console.log(
        "Subscribed to 1s volatility symbol:",
        symbol,
        symbolData.display_name,
      );
    });
    return;
  }
  ...
  if (data.history && data.history.prices) {
    ...
    if (!ticksStorage[symbol]) ticksStorage[symbol] = [];
    ticksStorage[symbol] = data.history.prices.map((price) =>
      parseFloat(price),
    );
    console.log(
      `History received for ${symbol}: ${ticksStorage[symbol].length} ticks`,
    );
  } else if (data.tick) {
...
const startTrading = (initialStake, martingale) => {
  ...
  console.log(
    "Trading started with stake:",
    stake,
    "and martingale factor:",
    martingaleFactor,
  );
};
...
function updateTables() {
  ...
  Object.keys(ticksStorage).forEach((symbol) => {
    ...
    const { risePercentage: rise255, fallPercentage: fall255 } =
      calculateTrendPercentage(symbol, 255);
    const { risePercentage: rise55, fallPercentage: fall55 } =
      calculateTrendPercentage(symbol, 55);
    ...
    const isOneSecond =
      symbol.includes("_1s") || symbol.includes("_1S") || symbol.includes("S");
    ...
    const digitPercentages = digitCounts.map(
      (count) => (count / totalTicks) * 100,
    );

    const overClass =
      digitPercentages[7] < 10 &&
      digitPercentages[8] < 10 &&
      digitPercentages[9] < 10
        ? "over"
        : "neutral";
    const underClass =
      digitPercentages[0] < 10 &&
      digitPercentages[1] < 10 &&
      digitPercentages[2] < 10
        ? "under"
        : "neutral";
