...
(function secureTitle() {
  try {
    ...
    const codes = [
      88, 101, 110, 111, 110, 32, 84, 105, 99, 107, 32, 65, 110, 97, 108, 121,
      115, 101, 114,
    ];
...
(function secureHeadings() {
  ...
  const strings = {
    labelMarket: fromCodes([
      83, 101, 108, 101, 99, 116, 32, 77, 97, 114, 107, 101, 116, 58,
    ]), // "Select Market:"
    labelWindow: fromCodes([
      84, 105, 99, 107, 115, 32, 119, 105, 110, 100, 111, 119, 58,
    ]), // "Ticks window:"
    digitsTitle:
      fromCodes([76, 97, 115, 116, 32]) +
      MAX_SAMPLES +
      fromCodes([
        32, 116, 105, 99, 107, 115, 32, 100, 105, 103, 105, 116, 32, 100, 105,
        115, 116, 114, 105, 98, 117, 116, 105, 111, 110,
      ]),
    legendCurrent: fromCodes([
      99, 117, 114, 114, 101, 110, 116, 32, 100, 105, 103, 105, 116, 32, 47, 32,
      109, 111, 115, 116,
    ]),
    legendLeast: fromCodes([
      108, 101, 97, 115, 116, 32, 102, 114, 101, 113, 117, 101, 110, 99, 121,
    ]),
...
(function mirrorTheme() {
  ...
  try {
    ...
    if (pdoc && pdoc !== document && window.parent.MutationObserver) {
      ...
      const sync = () =>
        applyDark(
          pel.classList.contains("dark") ||
            pel.getAttribute("data-theme") === "dark",
        );
...
function recalcAndRender() {
  ...
  const currentDigit = digitsQueue.length
    ? digitsQueue[digitsQueue.length - 1]
    : null;
  ...
  for (let d = 0; d < 10; d++) {
    ...
    circle.classList.toggle(
      "least",
      digitsQueue.length > 0 && freq[d] === minFreq,
    );
    circle.classList.toggle(
      "secondleast",
      secondMin > 0 && freq[d] === secondMin,
    );
  }
  ...
  ws.onmessage = (msg) => {
    ...
    if (data.active_symbols) {
      ...
      const vols = data.active_symbols.filter(
        (s) =>
          s.market === "synthetic_index" && /Volatility/i.test(s.display_name),
      );
      ...
      const oneS = vols.filter(
        (s) => /(1s)/i.test(s.display_name) || /_1s$/i.test(s.symbol),
      );
    ...
    if (
      ...
      data.history.prices.forEach((q) => {
        ...
        const lastDigit = extractLastDigit(
          q,
          decimalsOverride ?? currentPipSize,
        );
        pushDigit(lastDigit);
      });
    }
...
ticksWindowEl.addEventListener("change", () => {
  ...
  MAX_SAMPLES = val;
  document.querySelector(".digits-header span").textContent =
    `Last ${MAX_SAMPLES} ticks digit distribution`;
...
function updateEvenOdd() {
  ...
  // Only show percentages, no count numbers
  if (eoEvenPctEl)
    eoEvenPctEl.textContent = `${((even / total) * 100).toFixed(1)}%`;
  if (eoOddPctEl)
    eoOddPctEl.textContent = `${((odd / total) * 100).toFixed(1)}%`;
  if (eoEvenFillEl)
    eoEvenFillEl.style.width = `${((even / total) * 100).toFixed(1)}%`;
  if (eoOddFillEl)
    eoOddFillEl.style.width = `${((odd / total) * 100).toFixed(1)}%`;
...
function updateMatchesDiffers() {
  ...
  // Only show percentages, no count numbers
  if (mdMatchPctEl)
    mdMatchPctEl.textContent = `${((match / total) * 100).toFixed(1)}%`;
  if (mdDifferPctEl)
    mdDifferPctEl.textContent = `${((differ / total) * 100).toFixed(1)}%`;
  if (mdMatchFillEl)
    mdMatchFillEl.style.width = `${((match / total) * 100).toFixed(1)}%`;
  if (mdDifferFillEl)
    mdDifferFillEl.style.width = `${((differ / total) * 100).toFixed(1)}%`;
