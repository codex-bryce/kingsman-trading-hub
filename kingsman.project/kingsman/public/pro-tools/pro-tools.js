...
function updateOverUnder() {
  ...
  if (ouOverPercentageEl)
    ouOverPercentageEl.textContent = `${oPct.toFixed(2)}%`;
  if (ouUnderPercentageEl)
    ouUnderPercentageEl.textContent = `${uPct.toFixed(2)}%`;
  ...
  for (let i = 0; i < 10; i++)
    prefix[i] = counts[i] + (i > 0 ? prefix[i - 1] : 0);
  for (let i = 9; i >= 0; i--)
    suffix[i] = counts[i] + (i < 9 ? suffix[i + 1] : 0);
  const ns = "http://www.w3.org/2000/svg";
  const TOP_Y = 10,
    BOTTOM_Y = 250,
    H = BOTTOM_Y - TOP_Y;
  function renderSeries(chartEl, values, title) {
    ...
    for (let t = 0; t <= 9; t++) {
      ...
      rect.setAttribute(
        "class",
        title === "Over" ? "bar-rect-odd" : "bar-rect-even",
      );
...
function updateMatchDiffer() {
  ...
  if (mdSummary)
    mdSummary.textContent = `Match: ${mPct.toFixed(1)}% | Differ: ${dPct.toFixed(1)}%`;
  drawMdHistGridOnce();
  if (mdHistChart) {
    const ns = "http://www.w3.org/2000/svg";
    const counts = new Array(10).fill(0);
...
function updateRiseFall() {
  ...
  if (rfChipsEl && digitsQueue.length >= 2) {
    ...
    while (rfChipsEl.children.length > 30)
      rfChipsEl.removeChild(rfChipsEl.firstChild);
  }
  ...
  if (rfRisePercentageEl)
    rfRisePercentageEl.textContent = `${rPct.toFixed(2)}%`;
  if (rfEqualPercentageEl)
    rfEqualPercentageEl.textContent = `${ePct.toFixed(2)}%`;
  if (rfFallPercentageEl)
    rfFallPercentageEl.textContent = `${fPct.toFixed(2)}%`;
}
function connect() {
  ...
  ws.onmessage = (msg) => {
    ...
    if (data.tick) {
      ...
      if (symbol === currentSymbol) {
        ...
        if (livePriceEl)
          livePriceEl.textContent = toFixedWithPip(quote, usedDecimals);
        if (eoChipsEl) {
          const chip = document.createElement("span");
          chip.className = `chip ${lastDigit % 2 === 0 ? "e" : "o"}`;
          eoChipsEl.appendChild(chip);
          while (eoChipsEl.children.length > 30)
            eoChipsEl.removeChild(eoChipsEl.firstChild);
        }
        if (mdChipsEl && mdInput) {
          ...
          while (mdChipsEl.children.length > 30)
            mdChipsEl.removeChild(mdChipsEl.firstChild);
        }
        if (ouChipsEl && ouInput) {
          ...
          while (ouChipsEl.children.length > 30)
            ouChipsEl.removeChild(ouChipsEl.firstChild);
        }
        pushDigit(lastDigit);
        updateOverUnder();
        updateMatchDiffer();
