...
const useFullScreen = () => {
  ...
  const onFullScreen = useCallback(
    () =>
      setFullScreenMode(
        screenElement.some((element) => document[element as keyof Document]),
      ),
    [screenElement],
  );
  ...
  const toggleFullScreenMode = (event?: MouseEvent<HTMLButtonElement>) => {
    ...
    const exitFullScreen = exit.find(
      (method) => document[method as keyof Document],
    );
    const requestFullScreen = request.find(
      (method) => document.documentElement[method as keyof HTMLElement],
    );
    ...
    if (isInFullScreenMode && exitFullScreen) {
      ...
      (
        document.documentElement[
          requestFullScreen as keyof HTMLElement
