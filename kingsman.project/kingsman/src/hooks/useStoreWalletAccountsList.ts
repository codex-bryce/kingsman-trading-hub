...
const currency_to_icon_mapper: Record<
  string,
    ...
    return (
      Object.keys(accounts)
        ...
        ?.map((id) => {
          ...
          const landing_company_name = wallet.landing_company_name?.replace(
            "maltainvest",
            "malta",
          );
          const is_malta_wallet = landing_company_name === "malta";
          const dtrade_loginid = wallet?.linked_to?.find(
            (account) => account?.platform === "dtrade",
          )?.loginid;
          const dtrade_balance =
            all_accounts_balance?.accounts?.[dtrade_loginid ?? ""]?.balance;
          const is_dtrader_account_disabled = Boolean(
            accounts?.[dtrade_loginid ?? ""]?.is_disabled,
          );
