...
export const useBlockScroll = (target_ref?: RefObject<HTMLElement>) => {
  React.useEffect(() => {
    ...
    const getScrollableParentElement: (
      prop: HTMLElement | null,
    ) => HTMLElement | null = (elem: HTMLElement | null) => {
      if (!elem) return null;
      if (
        elem.classList.contains("dc-themed-scrollbars") &&
    ...
    const content_width_style = is_firefox_browser
      ? "100%"
      : "calc(100% - 5px)";
