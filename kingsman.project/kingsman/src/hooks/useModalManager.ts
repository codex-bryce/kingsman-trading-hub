...
export default function useModalManager(config?: TUseModalManagerConfig) {
  ...
  const syncModalParams = () => {
    ...
    if (
      config?.shouldReinitializeModals !== undefined &&
      config.shouldReinitializeModals === false
    ) {
  ...
  const showModal = (modalId: string, options?: TShowModalOptions) => {
    ...
    if (modalHash) {
      ...
      setQueryString({
        modal: options?.shouldClearPreviousModals
          ? modalId
          : modalIds.join(MODAL_QUERY_SEPARATOR),
      });
    } else {
      actions.set(modalId, true);
      setQueryString({
