...
export default class ToolboxStore {
      ...
      setTimeout(() => {
        ...
        const toolbox_width =
          document.getElementById("gtm-toolbox")?.getBoundingClientRect()
            .width || 0;
        const block_canvas_rect =
          workspace.svgBlockCanvas_?.getBoundingClientRect(); // eslint-disable-line

        if (workspace.RTL && block_canvas_rect) {
          const is_mobile = this.core.ui.is_mobile;
          const block_canvas_space = is_mobile
            ? block_canvas_rect.right
            : block_canvas_rect.left;

          const scroll_distance_mobile =
            toolbox_width - block_canvas_space + 20;
          const scroll_distance_desktop =
            toolbox_width - block_canvas_space + 36;
  ...
  onToolboxItemExpand(index: number) {
    if ((this.sub_category_index as number[]).includes(index)) {
      const open_ids = this.sub_category_index.filter(
        (open_id) => open_id !== index,
      );
  ...
  getAllCategories = (): ChildNode[] => {
    ...
    Array.from((this.toolbox_dom as HTMLElement).childNodes).forEach(
      (category: ChildNode) => {
        categories.push(category);
        if (this.hasSubCategory((category as HTMLElement).children)) {
          Array.from((category as HTMLElement).children).forEach(
            (subCategory) => {
  ...
  showSearch = (search: string) => {
    ...
    if (search_term.length === 0) {
      ...
      search_words.every((term) => general_term.includes(term))
    ) {
    ...
    const pushBlockWithPriority = (priority: string) => {
      block_contents.forEach((block_content) => {
        ...
        const block_definitions =
          block.definition instanceof Function && block.definition();
        ...
        switch (priority) {
          ...
          case "exact_block_name": {
            if (
              search_term === block_name.toUpperCase() ||
              search_term === block_type.toUpperCase()
            ) {
          ...
          case "match_words": {
            if (
              search_words.every((word) =>
                block_name_terms.some((term: string) => term.includes(word)),
              ) ||
              search_words.every((word) =>
                block_type_terms.some((term: string) => term.includes(word)),
              )
            ) {
              pushIfNotExists(flyout_content, block_content);
            }
          ...
          case "block_term": {
            if (
              block_type_terms.some((term: string) =>
                search_words.some((word) => term.includes(word)),
              ) ||
              block_name_terms.some((term: string) =>
                search_words.some((word) => term.includes(word)),
              )
            ) {
              pushIfNotExists(flyout_content, block_content);
              ...
              if (definition instanceof Array) {
                definition.forEach((def) => {
                  ...
                  if (
                    ...
                    search_words.some((word) =>
                      definition_strings.includes(word),
                    )
                  ) {
                    pushIfNotExists(flyout_content, block_content);
                  }
            ...
            const matched_meta = Object.keys(block_meta)
              ...
              .find((key) => {
                ...
                return search_words.some((word) =>
                  block_meta_strings.some((meta) => meta.includes(word)),
                );
              });
    ...
    const matched_variables = all_variables.filter((variable) =>
      variable.name.toUpperCase().includes(search_term),
    );
    const variables_blocks =
      window.Blockly.DataCategory.search(matched_variables);
    // eslint-disable-next-line consistent-return
    ...
    const procedures_blocks =
      window.Blockly.Procedures.populateDynamicProcedures(searched_procedures);
    // eslint-disable-next-line consistent-return
