...
export default class LoadModalStore {
  ...
  get selected_strategy(): TStrategy {
    return (
      this.dashboard_strategies.find(
        (ws: { id: string }) => ws.id === this.selected_strategy_id,
  ...
  onEntered = (): void => {
    if (
      this.recent_strategies.length === 0 ||
      this.tab_name !== tabs_title.TAB_RECENT
    )
      return;
  ...
  onActiveIndexChange = (): void => {
    ...
    if (this.tab_name === tabs_title.TAB_LOCAL) {
      if (!this.drop_zone) {
        ...
        if (this.drop_zone) {
          this.drop_zone.addEventListener("drop", (event) =>
    ...
    if (this.tab_name !== tabs_title.TAB_LOCAL && this.drop_zone) {
      this.drop_zone.removeEventListener("drop", (event) =>
        this.handleFileChange(event, false),
      );
    }
    this.setOpenButtonDisabled(false);
  ...
  saveStrategyToLocalStorage = async () => {
    ...
    if (recent_files?.length > 0)
      this.setSelectedStrategyId(recent_files[0]?.id);
  };
  ...
  updateXmlValuesOnStrategySelection = () => {
    ...
    updateXmlValues({
      ...
      convertedDom: window?.Blockly?.utils?.xml?.textToDom(
  ...
  loadStrategyOnModalRecentPreview = async (workspace_id) => {
    ...
    if (ref_preview) {
      if (!this.recent_workspace)
        this.recent_workspace = window.Blockly.inject(
          ref_preview,
          inject_options,
        );
      (this.recent_workspace as any).RTL = isDbotRTL();
  ...
  loadStrategyOnModalLocalPreview = async (load_options) => {
    ...
    if (!this.local_workspace)
      this.local_workspace = await window.Blockly.inject(
        ref_preview,
        injectWorkspace,
      );
