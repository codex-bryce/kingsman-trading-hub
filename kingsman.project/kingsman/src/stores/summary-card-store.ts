...
export default class SummaryCardStore {
  ...
  getLimitOrder() {
    ...
    limit_order.take_profit = this.has_contract_update_take_profit
      ? +(this.contract_update_take_profit ?? 0)
      : 0;
    // send positive stop_loss to update or null to cancel
    limit_order.stop_loss = this.has_contract_update_stop_loss
    ...
    Object.keys(movements).forEach((name) => {
      ...
      if (name in this && movement && movement !== current_movement) {
        this[`${name as keyof TMovements}_movement`] =
          movement && movement > (this[name as keyof TMovements] || 0)
            ? "profit"
            : "loss";
  ...
  populateContractUpdateConfig(response: UpdateContractResponse) {
    const contract_update_config = getContractUpdateConfig(
      response?.contract_update,
    );
    ...
    if (!isEqualObject(this.contract_update_config, contract_update_config)) {
      ...
      if (this.contract_info && contract_update && !error) {
        this.contract_info.limit_order = Object.assign(
          this.contract_info?.limit_order || {},
          contract_update,
        );
      }
    }
  ...
  setContractUpdateConfig(
    contract_update_take_profit?: number | null,
    contract_update_stop_loss?: number | null,
  ) {
    if (contract_update_take_profit && contract_update_stop_loss) {
      ...
      this.contract_update_take_profit = this.has_contract_update_take_profit
        ? +contract_update_take_profit
        : null;
      this.contract_update_stop_loss = this.has_contract_update_stop_loss
        ? +contract_update_stop_loss
        : null;
    }
  }
  ...
  setValidationErrorMessages(
    propertyName: TValidationRuleIndex,
    messages: string,
  ) {
    const is_different = () =>
  ...
  registerReactions() {
    ...
    this.disposeSwitchAcountListener = reaction(
      ...
      (loginid, previousLoginId) => {
        ...
        const wasSpecialCR =
          previousLoginId && isSpecialCRAccount(previousLoginId);
