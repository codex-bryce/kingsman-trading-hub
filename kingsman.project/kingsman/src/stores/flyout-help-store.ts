...
export default class FlyoutHelpStore {
  ...
  setHelpContent = async (block_node) => {
    const block_type: keyof typeof help_strings | "" =
    ...
    const example_blocks = this.examples.map(
      (example) => example.childNodes[0],
    );
    setTimeout(
  ...
  getFilledBlocksIndex = async (blocks_type) => {
    const blocks_content = await Promise.all(
      blocks_type.map((block) => this.getHelpContent(block)),
    );
  ...
  getNextHelpContentIndex = async (is_last) => {
    const filled_blocks_index = await this.getFilledBlocksIndex(
      Object.keys(this.xml_list_group),
    );
    return is_last
      ? filled_blocks_index[filled_blocks_index.length - 1]
      : filled_blocks_index[0];
  };
  ...
  async onSequenceClick(should_go_next) {
    const current_block = Array.from(this.xml_list).find(
      (xml) => xml.getAttribute("type") === this.block_type,
    );
    ...
    const getNextBlock = async (xml, current_index, is_next) => {
      ...
      const next_filled_block =
        await this.getFilledBlocksIndex(next_blocks_type);
      ...
      try {
        await import(
          /* webpackChunkName: `[request]` */ "@/external/bot-skeleton"
        );
    ...
    const block_type = await getNextBlock(
      this.xml_list_group,
      current_block_index,
      should_go_next,
    );
  ...
  async updateSequenceButtons() {
    ...
    const first_filled_content_index =
      await this.getNextHelpContentIndex(false);

    runInAction(() => {
      this.should_previous_disable =
        current_index === 0 || current_index === first_filled_content_index;
  ...
  groupBy(nodes, should_include_block_only = false) {
    return Array.from(nodes).reduce((block_group, node) => {
      ...
      if (
        !should_include_block_only ||
        (should_include_block_only && type !== null)
      ) {
  ...
  setExamples(block_type) {
    ...
    const help_content = help_content_config(window.__webpack_public_path__)[
      block_type
    ];
    const examples_ids = help_content
      .filter((el) => el.type === "example")
      .map((example) => example.example_id);
    const examples = examples_ids.map((id) =>
      all_examples.find((example) => example.id === id),
    );
