...
import {
  ...
  NOTIFICATION_TYPE,
} from "@/components/bot-notification/bot-notification-utils";
...
import {
  ...
  tour_type,
  TTourType,
} from "../pages/tutorials/dbot-tours/utils";
import {
  TFaqContent,
  TGuideContent,
...
export default class DashboardStore implements IDashboardStore {
  ...
  constructor(root_store: RootStore, core: TStores) {
    ...
    const getVideoContent = [...guide_content()].map(
      (item) => `${item.search_id}# ${item.content?.toLowerCase()}`,
    );
    ...
    const getQuickStrategyContent = quick_strategy_content().map((item) => {
      ...
      let qs_description_content = getQSDescriptionContent(
        strategy_description?.[item.qs_name],
      );
  ...
  filterTuotrialTab = (search_param: string) => {
    ...
    const filtered_tutorial_content = foundItems.map((item) => {
      ...
      filtered_quick_strategy_content.push(
        quick_strategy_content()[Number(index)],
      );
  ...
  initInfoPanel() {
    if (!localStorage.getItem("dbot_should_show_info"))
      this.is_info_panel_visible = true;
  }
