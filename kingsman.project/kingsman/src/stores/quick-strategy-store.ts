...
export default class QuickStrategyStore implements IQuickStrategyStore {
  ...
  onSubmit = async (data: TFormData) => {
    ...
    const trade_type_cat = await contracts_for.getTradeTypeCategoryByTradeType(
      data.tradetype,
    );
    const selected_strategy = STRATEGIES()[this.selected_strategy];
    const strategy_xml = await import(
      /* webpackChunkName: `[request]` */ `../xml/${selected_strategy.name}.xml`
    );
    const strategy_dom = window.Blockly.utils.xml.textToDom(
      strategy_xml.default,
    );
    addDynamicBlockToDOM(
      "PREDICTION",
    ...
    const modifyValueInputs = (key: string, value: number) => {
      const el_value_inputs = strategy_dom?.querySelectorAll(
    ...
    const modifyFieldDropdownValues = (name: string, value: string) => {
      ...
      const el_blocks = strategy_dom?.querySelectorAll(
