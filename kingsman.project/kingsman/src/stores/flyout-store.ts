...
export interface IFlyoutStore {
  ...
  initFlyout: () => void;
  initBlockWorkspace: (
    el_block_workspace: HTMLElement,
    block_node: Node,
  ) => void;
  getFlyout: () => Element;
...
export default class FlyoutStore implements IFlyoutStore {
  ...
  initFlyout() {
    ...
    this.flyout.workspace_.getGesture =
      this.flyout.targetWorkspace.getGesture.bind(this.flyout.targetWorkspace_);
  ...
  initBlockWorkspace(el_block_workspace: HTMLElement, block_node: Node) {
    ...
    this.block_listeners.push(
      window?.Blockly?.browserEvents?.conditionalBind(
        block_svg_root,
        "mousedown",
        null,
        (event) => {
          ...
          this.flyout.blockMouseDown(block)(
            event as window.Blockly.Events.UiBase,
          );
        },
      ),
      window?.Blockly?.browserEvents?.bind(
        block_svg_root,
        "mouseout",
        block,
        block.removeSelect,
  ...
  setContents(xml_list: Element[], search_term = "") {
    ...
    this.search_term =
      search_term.length > text_limit
        ? `${search_term.substring(0, text_limit)}...`
        : search_term;
  ...
  setFlyoutWidth(xmlList: Element[]) {
    ...
    xmlList.forEach((node: Element) => {
      ...
      if (tag_name === window.Blockly.Xml.NODE_BLOCK) {
        ...
        node.setAttribute(
          "width",
          String(Math.ceil(block_hw.width * this.options.zoom.startScale)),
        );
        node.setAttribute(
          "height",
          String(Math.ceil(block_hw.height * this.options.zoom.startScale)),
        );
    ...
    this.flyout_width = Math.max(
      this.flyout_min_width,
      longest_block_width + 65,
    );
  }
  ...
  onClickOutsideFlyout(event: MouseEvent) {
    ...
    const is_flyout_click = path.some(
      (el) =>
        ...
        (el as Element).classList.contains("flyout"),
    );
  ...
  get variables_blocks_count() {
    return this.flyout_content.filter(
      (block: Element) => block.getAttribute("type") === "variables_get",
    ).length;
  }
