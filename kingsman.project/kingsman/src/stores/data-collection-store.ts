...
export default class DataCollectionStore {
  ...
  async trackRun() {
    ...
    this.setRunId(
      this.getHash(xml_hash + this.core.client.loginid + Math.random()),
    );
  ...
  async trackTransaction(
    ...
    const is_known_transaction = Object.keys(this.transaction_ids).includes(
  ...
  cleanXmlDom = (xml_dom: XMLDocument) => {
    ...
    const removeAttributesRecursively = (element: Element) => {
      useless_attributes.forEach((useless_attribute) =>
        element.removeAttribute(useless_attribute),
      );
