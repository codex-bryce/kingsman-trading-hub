...
interface ISaveModalStore {
  ...
  onConfirmSave: ({
    ...
    save_as_collection,
    bot_name,
  }: IOnConfirmProps) => void;
  updateBotName: (bot_name: string) => void;
  setButtonStatus: (
    status: { [key: string]: string } | string | number,
  ) => void;
}
...
export default class SaveModalStore implements ISaveModalStore {
  ...
  addStrategyToWorkspace = async (
    ...
    try {
      ...
      const current_workspace_index = workspace.findIndex(
        (strategy: TStrategy) => strategy.id === workspace_id,
      );
      const {
        load_modal: { getSaveType },
      } = this.root_store;
      const local_type = is_local ? save_types.LOCAL : save_types.GOOGLE_DRIVE;
      const save_collection = save_as_collection
        ? save_types.UNSAVED
        : local_type;
      ...
      localForage.setItem(
        "saved_workspaces",
        LZString.compress(JSON.stringify(workspace)),
      );
  ...
  onConfirmSave = async ({
    ...
    if (active_tab === 1) {
      ...
      main_strategy = recent_files.filter(
        (strategy: TStrategy) => strategy.id === selected_strategy.id,
      )?.[0];
      main_strategy.name = bot_name;
      main_strategy.save_type = is_local
        ? save_types.LOCAL
    ...
    if (active_tab === 0) {
      ...
      await saveWorkspaceToRecent(
        xml,
        is_local ? save_types.LOCAL : save_types.GOOGLE_DRIVE,
      );
    }
