...
export const getSuccessJournalMessage = (message: string, extra: TExtra) => {
  const { profit, sold_for, longcode, transaction_id, current_currency } =
    extra;
  switch (message) {
    ...
    case LogTypes.PURCHASE: {
      return localize("Bought: {{longcode}} (ID: {{transaction_id}})", {
        longcode,
        transaction_id,
      });
    }
    ...
    case LogTypes.WELCOME: {
      if (current_currency)
        return localize("You are using your {{current_currency}} account.", {
          current_currency,
        });
      break;
    }
...
export const downloadFile = (file_name: string, content: string) => {
  ...
  link.setAttribute(
    "download",
    `${file_name} ${getCurrentDateTimeLocale()}.csv`,
  );
