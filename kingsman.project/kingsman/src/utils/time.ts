...
export const formatTime = (minutes: number, localize: TLocalize) => {
  ...
  const minutesText =
    remainingMinutes === 1 ? localize("minute") : localize("minutes");
...
export const getFormattedDateString = (
  ...
  const dateString = isLocal
    ? dateObj.toString().split(" ")
    : dateObj.toUTCString().split(" ");
 ...
 */
export const getDistanceToServerTime = (
  compareTime: number,
  serverTime?: Moment,
): number => {
 ...
 */
export const getDateAfterHours = (
  initialEpoch: number,
  hours: number,
): string => {
 ...
 */
export const epochToUTC = (time: number, format: string) =>
  moment.unix(time).utc().format(format);
 ...
 *  // Output will vary depending on the local timezone, e.g., '2020-12-31 19:00:00' for EST
 */
export const epochToLocal = (time: number, format: string) =>
  moment.unix(time).utc().local().format(format);
