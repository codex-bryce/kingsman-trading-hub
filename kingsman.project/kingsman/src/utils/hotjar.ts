import {
  epochToMoment,
  toMoment,
} from "@/components/shared/utils/date/date-time";
...
const initHotjar = (client: TStores["client"]) => {
  ...
  (function (h: any, o, t, j) {
    ...
    const account_open_date = client.account_open_date
      ? epochToMoment(client.account_open_date)
      : undefined;

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (window as any).hj("identify", user_id, {
      "Account created": account_open_date
        ? toMoment(account_open_date).format("YYYY-MM-DD")
        : "",
