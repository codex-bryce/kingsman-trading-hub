...
export const rudderstack_text_error =
  "Rudderstack: unable to get dropdown text";
...
const hasStoredText = (parameter: string) =>
  parameter && parameter !== STORED_ITEM_NOT_FOUND;

export const getRsStrategyType = (selected_strategy: string) =>
  STRATEGIES()[selected_strategy]?.rs_strategy_name;

export const getQsActiveTabString = (tab: string) =>
  tab === "TRADE_PARAMETERS" ? "trade parameters" : "learn more";
...
export const getTradeParameterData = ({ form_values }: TFormStrategy) => {
  ...
  return {
    asset_type: hasStoredText(stored_texts?.symbol)
      ? stored_texts.symbol
      : symbol,
    trade_type: hasStoredText(stored_texts?.tradetype)
      ? stored_texts?.tradetype
      : tradetype,
    purchase_condition: hasStoredText(stored_texts?.type)
      ? stored_texts?.type
      : type,
    initial_stake: hasStoredText(stored_texts?.stake)
      ? stored_texts?.stake
      : stake,
  };
};
