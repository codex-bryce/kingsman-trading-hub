...
window.Blockly.VariableMap.prototype.createVariable = function (
  name,
  opt_type,
  opt_id,
) {
  ...
  const type = opt_type || "";
  variable = new window.Blockly.VariableModel(
    this.workspace,
  ...
  window.Blockly.Events.fire(
    new (window.Blockly.Events.get(window.Blockly.Events.VAR_CREATE))(variable),
  );
