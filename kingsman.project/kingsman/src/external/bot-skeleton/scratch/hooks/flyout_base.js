...
window.Blockly.Flyout.prototype.createBlock = function (event, original_block) {
  ...
  const new_variables = window.Blockly.Variables.getAddedVariables(
    main_workspace,
    variables_before_creation,
  );
  ...
  if (window.Blockly.Events.isEnabled()) {
    ...
    new_variables.forEach((new_variable) => {
      window.Blockly.Events.fire(
        new window.Blockly.Events.VarCreate(new_variable),
      );
    });

    window.Blockly.Events.fire(
      new window.Blockly.Events.BlockCreate(new_block),
    );
  }
 ...
 * @private
 */
window.Blockly.Flyout.prototype.placeNewBlock_ = function (
  event,
  old_block = window.Blockly.getSelected(),
) {
  const main_workspace = this.targetWorkspace;
  ...
  if (event) {
    ...
    const final_offset_pixels = goog.math.Coordinate.difference(
      old_block_pos_pixels,
      main_offset_pixels,
    );

    // The position of the old block in main workspace coordinates.
    const final_offset_main_ws = final_offset_pixels.scale(
      1 / main_workspace.scale,
    );
