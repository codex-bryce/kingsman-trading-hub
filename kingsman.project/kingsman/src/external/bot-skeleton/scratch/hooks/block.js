...
window.Blockly.Block.prototype.getChildFieldValue = function (
  childType,
  childField,
) {
...
window.Blockly.Block.prototype.childValueToCode = function (
  childType,
  childField,
) {
  const childBlock = this.getChildByType(childType);
...
window.Blockly.Block.prototype.getBlocksInStatement = function (
  statementInputName,
) {
...
window.Blockly.Block.prototype.getLastConnectionInStatement = function (
  statement_input_name,
) {
...
window.Blockly.Block.getDimensions = function (block_node) {
  ...
  const existing_dimensions_key = Object.keys(
    window.Blockly.Block.Dimensions,
  ).find((outer_html) => block_node.outerHTML === outer_html);
...
window.Blockly.Block.prototype.hasErrorHighlightedDescendant = function () {
  const hasHighlightedDescendant = (child_blocks) =>
    child_blocks.some((child_block) => {
      ...
      const is_descendant_highlighted = hasHighlightedDescendant(
