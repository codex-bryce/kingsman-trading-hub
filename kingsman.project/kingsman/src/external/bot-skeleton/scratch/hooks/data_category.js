...
window.Blockly.DataCategory = function (workspace) {
  ...
  xmlList = xmlList.concat(
    window.Blockly.DataCategory.search(variableModelList, block_types),
  );
...
window.Blockly.DataCategory.search = function (variableModelList) {
  ...
  if (variableModelList.length > 0) {
    ...
    if (window.Blockly.Blocks.variables_set) {
      ...
      const setBlock =
        window.Blockly.utils.xml.textToDom(setBlockText).firstChild;
    ...
    if (window.Blockly.Blocks.variables_get) {
      ...
      variableModelList.sort(
        (first_variable, second_variable) =>
          variableModelList.indexOf(second_variable) -
          variableModelList.indexOf(first_variable),
      );
      ...
      variableModelList.forEach((variable) => {
        ...
        const getBlock =
          window.Blockly.utils.xml.textToDom(getBlockText).firstChild;
...
window.Blockly.DataCategory.addCreateVariable = function (xmlList, workspace) {
  ...
  const callback = function (button) {
    const el_input_container = document.querySelector(
