 ...
 * @public
 */
window.Blockly.WorkspaceSvg.prototype.centerOnBlock = function (
  id,
  hideChaff = true,
) {
  if (!this.scrollbar) {
...
window.Blockly.WorkspaceSvg.prototype.addBlockNode = function (block_node) {
  ...
  if (block_node) {
    ...
    const block = window.Blockly.Xml.domToBlock(
      block_node,
      flyout.getFlyout().targetWorkspace,
    );
    ...
    if (top_blocks.length) {
      ...
      const extra_spacing = last_block.startHat_
        ? window.Blockly.BlockSvg.START_HAT_HEIGHT
        : 0;
      const y =
        last_block_xy.y +
        last_block.getHeightWidth().height +
        extra_spacing +
        30;
    ...
    const trade_definition_block = this.getAllBlocks().find(
      (block) => block.type === "trade_definition",
    );
    if (trade_definition_block?.svgGroup_) {
      window.Blockly.utils.dom.addClass(
        trade_definition_block.svgGroup_,
        "blocklySelected",
      );
...
window.Blockly.WorkspaceSvg.prototype.cleanUp = function (
  ...
  const root_blocks = top_blocks
    ...
    .sort((a, b) => {
      const blockIndex = (block) =>
        config().mainBlocks.findIndex(
          (main_block_type) => main_block_type === block.type,
        );
  ...
  if (root_blocks.length) {
    ...
    root_blocks.forEach((block, index) => {
      ...
      const cursor_y =
        original_cursor_y -
        (is_import ? 0 : xy.y) +
        (DBotStore.instance.is_mobile ? 50 : 0);
      ...
      if (column_index === 0) {
        ...
        const fat_neighbour_block = root_blocks
          ...
          ?.reduce(
            (a, b) =>
              a.getHeightWidth().width > b.getHeightWidth().width ? a : b,
            initialValue,
          );
    ...
    const lowest_root_block = root_blocks.reduce((a, b) => {
      const a_metrics =
        a.getRelativeToSurfaceXY().y + a.getHeightWidth().height;
      const b_metrics =
        b.getRelativeToSurfaceXY().y + b.getHeightWidth().height;
  ...
  const filtered_top_blocks = top_blocks.filter(
    (block) => !block.isMainBlock(),
  );
...
window.Blockly.WorkspaceSvg.getTopLevelWorkspaceMetrics_ = function () {
  const toolbox_dimensions = window.Blockly.WorkspaceSvg.getDimensionsPx_(
    this.toolbox_,
  );
  const flyout_dimensions = window.Blockly.WorkspaceSvg.getDimensionsPx_(
    this.flyout_,
  );
  ...
  const content_dimensions = window.Blockly.WorkspaceSvg.getContentDimensions_(
    this,
    svg_size,
  );
