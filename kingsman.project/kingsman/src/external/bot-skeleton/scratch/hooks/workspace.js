...
window.Blockly.Workspace.prototype.waitForBlockEvent = function (
  block_id,
  opt_event_type = null,
) {
  ...
  if (
    !this.wait_events.some(
      (event) => event.blockId === block_id && event.type === opt_event_type,
    )
  ) {
...
window.Blockly.Workspace.prototype.dispatchBlockEventEffects = function (
  event,
) {
  this.wait_events.forEach((wait_event, idx) => {
    ...
    if (block) {
      ...
      const is_same_event_type =
        wait_event.event_type === null || wait_event.event_type === event.type;
...
window.Blockly.createVirtualWorkspace_ = function (
  fragment,
  options,
  blockDragSurface,
  ...
  const mainWorkspace = new window.Blockly.WorkspaceSvg(
    options,
    blockDragSurface,
    workspaceDragSurface,
  );
