...
window.Blockly.Blocks.controls_if = {
  ...
  rebuildShape() {
    ...
    const else_statement_connection =
  ...
  update(updateFn) {
    ...
    const old_mutation =
      old_mutation_dom && window.Blockly.Xml.domToText(old_mutation_dom);
    ...
    const new_mutation =
      new_mutation_dom && window.Blockly.Xml.domToText(new_mutation_dom);
  ...
  updateShape() {
    ...
    for (let j = 1; j <= this.else_if_count; j++) {
      ...
      this.appendDummyInput(input_names.IF_LABEL).appendField(
        localize("else if"),
      );
      this.appendValueInput(input_names.IF).setCheck("Boolean");
      this.appendDummyInput(input_names.THEN_LABEL).appendField(
        localize("then"),
      );
    ...
    for (let i = 1; i <= this.else_if_count; i++) {
      if (arg !== i) {
        ...
        this.value_connections.push(
          this.getInput(input_names.IF).connection.targetConnection,
        );
        this.statement_connections.push(
          this.getInput(input_names.DO).connection.targetConnection,
        );
      }
    }
  ...
  reconnectChildBlocks(
    opt_value_conns,
    opt_statement_conns,
    opt_else_statement_conns,
  ) {
    const value_connections = opt_value_conns ?? this.value_connections;
    const statement_connections =
      opt_statement_conns ?? this.statement_connections;
    const else_statement_connection =
      opt_else_statement_conns ?? this.else_statement_connection;
