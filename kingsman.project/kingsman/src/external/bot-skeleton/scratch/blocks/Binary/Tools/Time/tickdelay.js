    ...
    return {
      TICKDELAYVALUE: (input_value) => {
        ...
        if (evaluated_result === "invalid_input") {
          ...
          if (
            this.workspace_to_code &&
            this.workspace_to_code.includes(input_value)
          ) {
            return false;
          }
          this.error_message = localize("Invalid Input {{ input_value }}.", {
            input_value,
          });
          return true;
        }
...
Blockly.JavaScript.javascriptGenerator.forBlock.tick_delay = (block) => {
  const stack = Blockly.JavaScript.javascriptGenerator.statementToCode(
    block,
    "TICKDELAYSTACK",
  );
  const ticks_value =
