...
window.Blockly.Blocks.trade_definition_market = {
  ...
  onchange(event) {
    ...
    const is_allowed_event =
      allowed_events.findIndex(
        (event_name) => event.type === window.Blockly.Events[event_name],
      ) !== -1;
    ...
    if (
      ...
      if (event.name === "MARKET_LIST") {
        submarket_dropdown.updateOptions(
      ...
      if (
        market_dropdown.isEmpty() ||
        submarket_dropdown.isEmpty() ||
        symbol_dropdown.isEmpty()
      ) {
        populateMarketDropdown();
      }
    }
  ...
  enforceLimitations() {
    runIrreversibleEvents(() => {
      if (!this.isDescendantOf("trade_definition")) {
        ...
        const trade_definition_block = top_blocks.find(
