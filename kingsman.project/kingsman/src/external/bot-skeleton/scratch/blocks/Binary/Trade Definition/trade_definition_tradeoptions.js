...
window.Blockly.Blocks.trade_definition_tradeoptions = {
  ...
  onchange(event) {
      ...
      selected_block?.parentBlock_?.inputList
        ...
        .forEach((input) => {
          ...
          if (value?.startsWith("0")) {
            const new_value = value.includes(".")
              ? parseFloat(`${value}`)
              : parseInt(`${value}`);
    ...
    const market_block = trade_definition_block.getChildByType(
      "trade_definition_market",
    );
    const trade_type_block = trade_definition_block.getChildByType(
  ...
  updateAmountLimits() {
    ...
    account_limits
      ...
      .then((limits) => {
        ...
        const currency_child_block = currency_block
          ?.getChildren()?.[1]
          ?.getField("NUM");
        if (
          !this.getField("AMOUNT_LIMITS") &&
          !currency_block &&
          !currency_child_block
        )
          return;
  ...
  updateDurationInput(should_use_default_unit, should_update_value) {
    ...
    if (
      this.selected_trade_type === "accumulator" &&
      this.isDescendantOf("trade_definition")
    ) {
      runIrreversibleEvents(() => {
        runGroupedEvents(false, () => {
          const accumulator_block = this.workspace.newBlock(
          ...
          const trade_definition_block =
            this.workspace.getTradeDefinitionBlock();
          const parent_connection =
            trade_definition_block.getInput("SUBMARKET").connection;
          ...
          const stake_shadow_block = this.workspace.newBlock(
      ...
      this.selected_trade_type === "multiplier" &&
      this.isDescendantOf("trade_definition")
    ) {
      runIrreversibleEvents(() => {
        runGroupedEvents(false, () => {
          const multiplier_block = this.workspace.newBlock(
          ...
          const stop_loss_block = this.workspace.newBlock(
            "multiplier_stop_loss",
          );
          const stop_loss_input = stop_loss_block.getInput("AMOUNT");

          const stop_loss_shadow_block = this.workspace.newBlock(
            "math_number_positive",
          );
          ...
          stop_loss_shadow_block.outputConnection.connect(
            stop_loss_input.connection,
          );
      ...
      contracts_for
        ...
        .then((durations) => {
          ...
          duration_field_dropdown?.updateOptions(duration_options, {
            default_value: should_use_default_unit
              ? undefined
              : duration_field_dropdown.getValue(),
          });

          if (
            should_update_value &&
            duration_input &&
            duration_input.connection
          ) {
            const target_block = duration_input.connection.targetBlock();

            if (target_block && target_block.isShadow()) {
              const min_duration = durations.find(
                (duration) => duration.unit === this.selected_duration,
              );
  ...
  updateBarrierInputs(should_use_default_type, should_use_default_values) {
    ...
    contracts_for
      ...
      .then((barriers) => {
        ...
        for (let i = 0; i < barriers.values.length; i++) {
          const barrier_field_dropdown = this.getField(
            `${input_names[i]}TYPE_LIST`,
          );
          ...
          if (this.selected_duration === "d") {
            ...
            barrier_field_dropdown.updateOptions(
          ...
          if (should_use_default_values && connection) {
            ...
            if (target_block && target_block.isShadow()) {
              const barrier_value =
                barriers.values[i] !== false ? barriers.values[i] : "";
  ...
  updatePredictionInput(should_use_default_value) {
    ...
    contracts_for
      .getPredictionRange(this.selected_symbol, this.selected_trade_type)
  ...
  enforceSingleBarrierType(barrier_input_name, should_force_distinct) {
    ...
    const other_barrier_input_name =
      barrier_input_name === "BARRIEROFFSETTYPE_LIST"
    ...
    if (other_barrier_field) {
      const has_other_barrier =
        BARRIER_TYPES.findIndex((type) => type[1] === new_value) !== -1;
      const other_barrier_type = other_barrier_field.getValue();

      runIrreversibleEvents(() => {
        if (
          has_other_barrier &&
          (other_barrier_type === "absolute" || should_force_distinct)
        ) {
          const other_barrier_value = BARRIER_TYPES.find(
            (type) => type[1] !== new_value,
          );
          other_barrier_field.setValue(other_barrier_value[1]);
        } else if (
          new_value === "absolute" &&
          other_barrier_type !== "absolute"
        ) {
  ...
  domToMutation(xmlElement) {
    const has_first_barrier =
      xmlElement.getAttribute("has_first_barrier") === "true";
    const has_second_barrier =
      xmlElement.getAttribute("has_second_barrier") === "true";
    ...
    return {
      ...
      DURATION: (input) => {
        ...
        const duration = this.durations.find(
          (d) => d.unit === this.selected_duration,
        );
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.trade_definition_tradeoptions =
  (block) => {
    ...
    const getBarrierValue = (barrier_offset_type, value) => {
      ...
      if (/^(\d+(\.\d+)?)$/.test(value)) {
        return barrier_offset_type === "absolute"
          ? `'${value}'`
          : `'${barrier_offset_type}${value}'`;
      }
      return barrier_offset_type === "absolute"
        ? value
        : `'${barrier_offset_type}' + ${value}`;
    ...
    if (block.getInput("SECONDBARRIEROFFSET")) {
      const barrier_offset_type = block.getFieldValue(
        "SECONDBARRIEROFFSETTYPE_LIST",
      );
      const value =
        window.Blockly.JavaScript.javascriptGenerator.valueToCode(
