...
window.Blockly.Blocks.get_ohlc = {
  ...
  definition() {
    return {
      message0: localize("in candle list get # from end {{ input_number }}", {
        input_number: "%1",
      }),
      message1: localize("with interval: {{ candle_interval_type }}", {
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.get_ohlc = (block) => {
  ...
  const granularity =
    selectedGranularity === "default" ? "undefined" : selectedGranularity;
  const index =
