...
window.Blockly.Blocks.loader = {
  ...
  definition() {
    return {
      message0: localize("Load block from URL: {{ input_url }}", {
        input_url: "%1",
      }),
      args0: [
        {
  ...
  onchange(event) {
    ...
    if (
      ...
      const loader_blocks = this.workspace
        .getAllBlocks()
        .filter((block) => block.type === "loader");

      loader_blocks.forEach((loader_block) => {
        if (
          loader_block.id !== this.id &&
          loader_block.current_url === this.current_url
        ) {
      ...
      if (event.newValue && event.oldValue !== event.newValue) {
        if (event.newValue === this.current_url) {
          ...
          this.isValidUrl(event.newValue) &&
          !this.isKnownUrl(event.newValue)
        ) {
          this.setDisabled(false);
          this.loadBlocksFromCurrentUrl();
        } else {
  ...
  isKnownUrl(url) {
    const loader_blocks = this.workspace
      .getAllBlocks()
      .filter((block) => block.type === "loader");
    return loader_blocks.some(
      (block) => block.id !== this.id && block.current_url === url,
    );
  },
  isValidUrl(url) {
