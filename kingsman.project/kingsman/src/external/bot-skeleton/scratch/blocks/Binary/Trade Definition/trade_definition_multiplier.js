  ...
  validateBlocksInStatement() {
    ...
    if (blocks_in_multiplier.length > 0) {
      ...
      blocks_in_multiplier.forEach((block) => {
        ...
        const block_multiplier_take_profit = block.childValueToCode(
          "multiplier_take_profit",
          "AMOUNT",
        );
        const block_multiplier_stop_loss = block.childValueToCode(
          "multiplier_stop_loss",
          "AMOUNT",
        );
        if (
          block_multiplier_take_profit <= 0 ||
          block_multiplier_stop_loss <= 0
        ) {
        ...
        if (
          ...
          new Set(block_types_in_multiplier).size !==
            block_types_in_multiplier.length
        ) {
          runIrreversibleEvents(() => {
            block.unplug(true);
          });
    ...
    if (event.type === window.Blockly.Events.BLOCK_DRAG && !event.isStart) {
      ...
      if (event.blockId === this.id) {
        ...
        if (!this.selected_multiplier) {
          const fake_creation_event = new window.Blockly.Events.BlockCreate(
            this,
          );
      ...
      contracts_for
        ...
        ?.then((multiplier_range) => {
          if (multiplier_range.length > 0) {
            const multiplier_list_dropdown = this.getField(
              "MULTIPLIERTYPE_LIST",
            );
            ...
            multiplier_list_dropdown?.updateOptions(multiplier_options, {
              default_value: should_use_default_value
                ? undefined
                : multiplier_list_dropdown.getValue(),
            });
          }
        });
      return;
    ...
    if (this.isDescendantOf("trade_definition")) {
      runIrreversibleEvents(() => {
        runGroupedEvents(false, () => {
          ...
          const duration_block = this.workspace.newBlock(
          ...
          const duration_shadow_block = this.workspace.newBlock(
            "math_number_positive",
          );
          duration_shadow_block.setShadow(true);
          duration_shadow_block.outputConnection.connect(
