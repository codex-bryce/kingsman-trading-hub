...
import {
  ...
  removeErrorHandlingEventListener,
} from "../../../../utils";
...
window.Blockly.Blocks.trade_definition = {
  ...
  onchange(event) {
    ...
    if (
      ...
      const blocks_in_trade_options =
        this.getBlocksInStatement("TRADE_OPTIONS");
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.trade_definition = (
  ...
  if (!client || (!client.is_logged_in && !account)) {
  ...
  const contract_type_block = block.getChildByType(
    "trade_definition_contracttype",
  );
  const candle_interval_block = block.getChildByType(
    "trade_definition_candleinterval",
  );
  const restart_on_error_block = block.getChildByType(
    "trade_definition_restartonerror",
  );
  const restart_on_buy_sell_block = block.getChildByType(
  ...
  const candle_interval = candle_interval_block.getFieldValue(
    "CANDLEINTERVAL_LIST",
  );
  const should_restart_on_error =
  ...
  const contract_type_list =
    contract_type === "both"
      ? opposites[trade_type.toUpperCase()].map(
          (opposite) => Object.keys(opposite)[0],
        )
      : [contract_type];
