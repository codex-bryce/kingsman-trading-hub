...
window.Blockly.Blocks.apollo_purchase = {
  ...
  definition() {
    return {
      message0: localize("Apollo Purchase {{ contract_type }}", {
        contract_type: "%1",
      }),
      args0: [
        {
      ...
      tooltip: localize(
        "This block purchases contract of a specified type with advanced options.",
      ),
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.apollo_purchase = (
  ...
  if (multipleContracts === "TRUE" && contractQuantity > 1) {
    code =
      `// Apollo Purchase: Multiple contracts (${contractQuantity})\n` + code;
...
window.Blockly.Blocks.apollo_purchase2 = {
  ...
  definition() {
    return {
      message0: localize("Apollo Purchase 2 {{ contract_type }}", {
        contract_type: "%1",
      }),
      args0: [
        {
      ...
      colourTertiary: window.Blockly.Colours.Special1.colourTertiary,
      tooltip: localize(
        "This block purchases contract of a specified type (simplified version).",
      ),
      category: window.Blockly.Categories.Before_Purchase,
    };
  },
