...
window.Blockly.Blocks.trade_definition_accumulator = {
  ...
  validateBlocksInStatement() {
    ...
    const blocks_in_accumulator =
      this.getBlocksInStatement("ACCUMULATOR_PARAMS");
    ...
    if (blocks_in_accumulator.length > 0) {
      ...
      blocks_in_accumulator.forEach((block) => {
        ...
        const block_accumulator_take_profit = block.childValueToCode(
        ...
        if (
          ...
          block_accumulator_take_profit > 0
        ) {
        ...
        if (
          ...
          new Set(block_types_in_accumulator).size !==
            block_types_in_accumulator.length
        ) {
          runIrreversibleEvents(() => {
            block.unplug(true);
          });
  ...
  updateAmountLimits:
    window.Blockly.Blocks.trade_definition_tradeoptions.updateAmountLimits,
        ...
        if (accumulator_range.length > 0) {
          ...
          accumulator_list_dropdown?.updateOptions(accumulator_options, {
            default_value: should_use_default_value
              ? undefined
              : accumulator_list_dropdown.getValue(),
          });
        }
      });
      return;
