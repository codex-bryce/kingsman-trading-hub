...
window.Blockly.Blocks.input_list = {
  ...
  onchange(event) {
    ...
    if (
      ...
      const is_illegal_parent =
        !has_parent || surround_parent.id !== this.required_parent_id;
      ...
      if (!has_parent || is_illegal_parent) {
        runIrreversibleEvents(() => {
          ...
          const parent_block = all_blocks.find(
            (block) => block.id === this.required_parent_id,
          );

          if (parent_block) {
            const parent_connection =
              parent_block.getLastConnectionInStatement("STATEMENT");
