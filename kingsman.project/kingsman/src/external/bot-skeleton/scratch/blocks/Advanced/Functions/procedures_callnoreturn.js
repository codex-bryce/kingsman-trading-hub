...
window.Blockly.Blocks.procedures_callnoreturn = {
  ...
  onchange(event) {
    ...
    if (
      ...
      if (
        def &&
        (def.type !== this.defType ||
          JSON.stringify(def.arguments) !== JSON.stringify(this.arguments))
      ) {
      ...
      const procedure_definition = window.Blockly.Procedures.getDefinition(
        name,
        this.workspace,
      );
  ...
  updateShape() {
    this.arguments.forEach((argumentName, i) => {
      ...
      if (field) {
        ...
        const input = this.appendValueInput(`ARG${i}`).appendField(
          field,
          `ARGNAME${i}`,
        );
        input.init();
