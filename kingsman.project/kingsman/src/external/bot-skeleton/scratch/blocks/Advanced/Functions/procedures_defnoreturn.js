...
import {
  appendCollapsedProcedureBlocksFields,
  modifyContextMenu,
} from "../../../utils";
...
window.Blockly.Blocks.procedures_defnoreturn = {
  init() {
    ...
    const fieldImage = new window.Blockly.FieldImage(
      plusIconLight,
      24,
      24,
      "+",
      ...
      window.Blockly.dialog.prompt(promptMessage, "", (paramName) => {
        if (paramName) {
          ...
          if (variable) {
            ...
            paramField.setText(
              `${localize("with: ")} ${this.arguments.join(", ")}`,
            );
    ...
    xmlElement.childNodes.forEach((childNode) => {
      if (childNode.nodeName.toLowerCase() === "arg") {
        ...
        if (variable !== null) {
          ...
          console.log(
            `Failed to create a variable with name ${var_name}, ignoring.`,
          );
        }
      }
    });
  ...
  getProcedureCallers() {
    return this.workspace
      .getAllBlocks(false)
      .filter(
        (block) => block.type === this.callType && block.data === this.id,
      );
  },
  /**
  ...
  customContextMenu(options) {
    ...
    option.callback = window.Blockly.ContextMenu.callbackFactory(
      this,
      xmlBlock,
    );
    options.push(option);
    ...
    if (!this.isCollapsed()) {
      this.argument_var_models.forEach((argumentVarModel) => {
        ...
        getOption.text = localize('Create "get %1"').replace(
          "%1",
          argumentVarModel.name,
        );

        const xmlField =
          window.Blockly.Variables.generateVariableFieldDom(argumentVarModel);
        ...
        getOption.callback = window.Blockly.ContextMenu.callbackFactory(
          this,
          xmlOptionBlock,
        );
...
window.Blockly.JavaScript.javascriptGenerator.forBlock.procedures_defnoreturn =
  (block) => {
    ...
    let branch = window.Blockly.JavaScript.javascriptGenerator.statementToCode(
      block,
      "STACK",
    );
