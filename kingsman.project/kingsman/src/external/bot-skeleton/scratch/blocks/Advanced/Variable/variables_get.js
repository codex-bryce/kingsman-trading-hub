...
window.Blockly.Blocks.variables_get = {
  ...
  onchange(event) {
    if (event.type === window.Blockly.Events.VAR_RENAME) {
      ...
      const function_blocks = all_blocks.filter(
        (block) => block.category_ === "custom_functions",
      );
      const old_param = event.oldName;
      const new_param = event.newName;
      function_blocks.forEach((block) => {
        if (block.arguments?.length) {
          const param_index = block.arguments.findIndex(
            (item) => item === old_param,
          );
          if (param_index !== -1) {
            ...
            if (param_field) {
              block.setFieldValue(
                `${localize("with: ")} ${block.arguments.join(", ")}`,
                "PARAMS",
              );
            }
