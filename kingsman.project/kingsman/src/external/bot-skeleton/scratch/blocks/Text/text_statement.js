...
window.Blockly.Blocks.text_statement = {
  ...
  init() {
    ...
    const field_image = new window.Blockly.FieldImage(
      minusIconDark,
      25,
      25,
      "",
  ...
  onchange(event) {
    ...
    if (event.type === window.Blockly.Events.BLOCK_CREATE) {
      ...
      if (
        !this.required_parent_id &&
        surround_parent?.type === this.required_parent_type
      ) {
    ...
    if (event.type === window.Blockly.Events.BLOCK_DRAG && !event.isStart) {
      const stack_blocks = window.Blockly.getMainWorkspace().getBlockById(
        event.blockId,
      );

      if (
        this.required_parent_id &&
        (!surround_parent || surround_parent.id !== this.required_parent_id)
      ) {
        const original_parent = window.Blockly.getMainWorkspace().getBlockById(
          this.required_parent_id,
        );

        if (original_parent) {
          const first_block_in_stack =
            original_parent.getInputTargetBlock("STACK");

          if (first_block_in_stack) {
            const last_connection =
              original_parent.getLastConnectionInStatement("STACK");
            ...
            runIrreversibleEvents(() => {
              original_parent
                .getInput("STACK")
                .connection.connect(stack_blocks.previousConnection);
            });
          }
        }
      }
