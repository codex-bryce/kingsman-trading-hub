...
export default class BlockConversion {
  ...
  getConversions() {
    const generateGrowingListBlock = (
      block_node,
      block_type,
      variable_name,
      child_value_input_name,
    ) => {
      ...
      if (mutation_dom) {
        ...
        if (items_amount > 0) {
          ...
          block_child_nodes
            .filter(
              (child_node) => child_node.tagName.toLowerCase() === "value",
            )
            .forEach((child_node) => {
              const value_input_name = child_node.getAttribute("name");
              const value_input_index = parseInt(
                value_input_name.replace(/[^0-9]+/g, ""),
              );

              Array.from(child_node.children).forEach((value_block_node) => {
                const value_block = this.convertBlockNode(
                  value_block_node,
                  block,
                );
                ...
                const value_input = statement_block.getInput(
                  child_value_input_name,
                );
              ...
              if (
                parent_element &&
                child_element &&
                parent_element?.contains(child_element)
              ) {
    ...
    const generateIndicatorBlock = (block_node, block_type, variable_name) => {
      ...
      switch (block_type) {
        ...
        case "bba_statement": {
          ...
          if (bb_result_list) {
            bb_result_list.setValue(
              this.getFieldValue(block_node, "BBRESULT_LIST"),
            );
          }
          break;
        }
        case "macda_statement": {
          ...
          if (macd_fields_list) {
            macd_fields_list.setValue(
              this.getFieldValue(block_node, "MACDFIELDS_LIST"),
            );
          }
          break;
        }
      ...
      blocks_in_statement.forEach((block_in_statement) => {
        const value_name_mappings =
          value_input_name_mappings[block_in_statement.type];

        if (value_name_mappings) {
          value_name_mappings.forEach((value_name_mapping) => {
            const el_value = block_node.querySelector(
              `value[name="${value_name_mapping.old}"]`,
            );

            if (el_value) {
              const value_input = block_in_statement.getInput(
                value_name_mapping.new,
              );

              Array.from(el_value.children).forEach((el_value_child) => {
                const converted_block = this.convertBlockNode(
                  el_value_child,
                  block,
                );
                ...
                value_input.connection.connect(
                  converted_block.outputConnection,
                );
              });
    ...
    const tradeOptions = (block_node, is_symbol_type = false) => {
      ...
      block_child_nodes
        .filter(
          (block_child_node) =>
            block_child_node.tagName.toLowerCase() === "value",
        )
        .forEach((block_child_node) => {
          const value_input_name = block_child_node.getAttribute("name");

          if (
            value_input_name === "BARRIEROFFSET" ||
            value_input_name === "SECONDBARRIEROFFSET"
          ) {
            barrier_inputs++;
    ...
    const conversions = {
      bb: (block_node) =>
        generateIndicatorBlock(block_node, "bb_statement", "bb"),
      bba: (block_node) =>
        generateIndicatorBlock(block_node, "bba_statement", "bba"),
      ema: (block_node) =>
        generateIndicatorBlock(block_node, "ema_statement", "ema"),
      emaa: (block_node) =>
        generateIndicatorBlock(block_node, "emaa_statement", "emaa"),
      lists_create_with: (block_node) =>
        generateGrowingListBlock(
          block_node,
          "lists_create_with",
          localize("list"),
          "VALUE",
      ...
      macda: (block_node) =>
        generateIndicatorBlock(block_node, "macda_statement", "macda"),
      market: (block_node) => {
        this.has_market_block = true;
        return tradeOptions(block_node);
      },
      rsi: (block_node) =>
        generateIndicatorBlock(block_node, "rsi_statement", "rsi"),
      rsia: (block_node) =>
        generateIndicatorBlock(block_node, "rsia_statement", "rsia"),
      sma: (block_node) =>
        generateIndicatorBlock(block_node, "sma_statement", "sma"),
      smaa: (block_node) =>
        generateIndicatorBlock(block_node, "smaa_statement", "smaa"),
      ...
      trade: (block_node) => {
        ...
        Object.keys(block_fields).forEach((child_block_name) => {
          ...
          block_fields[child_block_name].forEach((field_name) => {
            child_block.setFieldValue(
              this.getFieldValue(block_node, field_name),
              field_name,
            );
          });
        ...
        Object.values(blocks_to_connect).forEach((child_block) => {
          const last_connection =
            block.getLastConnectionInStatement("TRADE_OPTIONS");
          last_connection.connect(child_block.previousConnection);
        });
  ...
  getIllegalBlocks() {
    ...
    Object.keys(opposites).forEach((opposites_name) =>
      illegal_blocks.push(opposites_name),
    );
  ...
  createWorkspace() {
    ...
    const workspace = window.Blockly.createVirtualWorkspace_(
      el_injection_div,
      options,
      false,
      false,
  ...
  naivelyFixLegacyStrategyAfterConversion() {
    ...
    const has_initialization_block =
      trade_definition_block?.getBlocksInStatement("INITIALIZATION").length > 0;
    if (trade_definition_block) {
      trade_definition_block
        .getBlocksInStatement("SUBMARKET")
        .forEach((block) => {
          if (
            ...
            const last_connection =
              trade_definition_block.getLastConnectionInStatement(
                "INITIALIZATION",
              );
            block.unplug(true);
  ...
  generateUniqueVariable(variable_name) {
    ...
    const ws_variable = window.Blockly.Variables.getOrCreateVariablePackage(
      this.workspace,
      "",
      current_name,
      "",
  ...
  convertStrategy(strategy_node, showIncompatibleStrategyDialog) {
    ...
    if (
      strategy_node.hasAttribute("is_dbot") &&
      strategy_node.getAttribute("is_dbot") === "true"
    ) {
    ...
    Object.keys(this.blocks_pending_reconnect).forEach(
      (previous_sibling_id) => {
        const previous_sibling_block =
          this.workspace.getBlockById(previous_sibling_id);
        const child_blocks = this.blocks_pending_reconnect[previous_sibling_id];
        const sibling_previous_connection =
          previous_sibling_block.previousConnection;

        if (sibling_previous_connection) {
          const sibling_previous_target_connection =
            sibling_previous_connection.targetConnection;
          ...
          if (sibling_previous_target_connection) {
            ...
            let current_previous_connection =
              previous_sibling_block.previousConnection;
            ...
            sibling_previous_target_connection.connect(
              current_previous_connection,
            );
          }
        }
      },
  ...
  convertBlockNode(el_block, parent_block = null) {
    ...
    const is_collapsed =
      (el_block.getAttribute("collapsed") &&
        el_block.getAttribute("collapsed") === "true") ||
      false;
    const is_immovable =
      el_block.getAttribute("movable") &&
      el_block.getAttribute("movable") === "false";
    const is_undeletable =
      el_block.getAttribute("deletable") &&
      el_block.getAttribute("deletable") === "false";
    const is_disabled =
      el_block.getAttribute("disabled") &&
      el_block.getAttribute("disabled") === "true";
    ...
    if (is_old_block) {
      ...
      if (parent_block && conversion_obj.statement_blocks) {
        conversion_obj.statement_blocks.forEach((statement_block) => {
          ...
          const previous_connection =
            this.getClosestLegalPreviousConnection(parent_block);
          ...
          if (previous_connection) {
            ...
            this.blocks_pending_reconnect[target_block.id].push(
              statement_block,
            );
          }
        });
      ...
      const is_legal_block = Object.keys(window.Blockly.Blocks).includes(
        block_type,
      );
  ...
  processValueInputs(block, el_input) {
    ...
    Array.from(el_input.children).forEach((el_input_child_block) => {
      const input_child_block = this.convertBlockNode(
        el_input_child_block,
        block,
      );
      ...
      if (!input_child_block) {
        ...
        console.warn(
          "Illegal child.",
          el_input_child_block.getAttribute("type"),
        );
        return false;
      }
  ...
  // eslint-disable-next-line consistent-return
  processStatementInputs(
    block,
    statement_name,
    el_node_with_children,
    block_to_search = null,
  ) {
    ...
    // eslint-disable-next-line consistent-return
    Array.from(el_node_with_children.children).forEach(
      (el_input_child_block) => {
        const input_child_block = this.convertBlockNode(
          el_input_child_block,
          block_to_use,
        ...
        if (statement_input && input_child_block.previousConnection) {
          statement_input.connection.connect(
            input_child_block.previousConnection,
          );
        }
      },
    );
  ...
  getClosestLegalPreviousConnection(supposed_parent_block) {
    ...
    if (
      config().procedureDefinitionBlocks.includes(supposed_parent_block.type)
    ) {
      const blocks_in_statement =
        supposed_parent_block.getBlocksInStatement("STACK");
      ...
      return supposed_parent_block.getInputTargetBlock("STACK")
        .previousConnection;
    }

    let current_block = this.getFirstBlockInStack(supposed_parent_block);
    let is_protected_statement =
      this.isConnectedToProtectedStatementInput(current_block);
    ...
    if (supposed_parent_block.conversion_parent) {
      return this.getClosestLegalPreviousConnection(
        supposed_parent_block.conversion_parent,
      );
    }
