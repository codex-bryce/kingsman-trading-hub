...
class DBot {
  ...
  async initWorkspace(
    ...
    var that = this;
    window.Blockly.Blocks.trade_definition_tradetype.onchange = function (
      event,
    ) {
      if (
        !this.workspace ||
      ...
      if (type === window.Blockly.Events.BLOCK_CHANGE) {
        ...
        if (is_symbol_list_change || is_trade_type_cat_list_change) {
          ...
          const market_block = top_parent_block.getChildByType(
          ...
          if (is_symbol_list_change) {
            ...
            if (
              ...
              const run_button = document.querySelector(
      ...
      try {
        ...
        if (window_width < 1640) {
          if (is_mobile) {
            ...
            const scratch_div_width =
              document.getElementById("scratch_div")?.offsetWidth;
        ...
        const loadStrategyIntoWorkspace = () => {
          ...
          // Normalize foreign XML block names before import for BOT! BOT@ and similar flows
          if (
            typeof window.Blockly.derivWorkspace.strategy_to_load === "string"
          ) {
        ...
        if (recent_files && recent_files.length) {
          ...
          // Fall back to the built-in starter XML when there is no saved workspace.
          loadStrategyIntoWorkspace();
        }
      } catch (error) {
        // TODO: Handle error.
  ...
  async saveRecentWorkspace() {
    const current_xml_dom = this?.workspace
    ...
    try {
      ...
      if (
        current_xml_dom &&
        this.isStrategyUpdated(current_xml_dom, recent_files)
      ) {
  ...
  checkForErroredBlocks() {
    ...
    const error_blocks = all_blocks
      ...
      .filter(
        (block, index, self) =>
          index ===
          self.findIndex((b) => b.error_message === block.error_message),
      );
  ...
  unHighlightAllBlocks() {
    this.workspace
      ?.getAllBlocks()
      .forEach((block) => block.setErrorHighlighted(false));
  }
  ...
  valueInputLimitationsListener(event, force_check = false) {
    ...
    const isClickEvent = () =>
      event.type === window.Blockly.Events.UI &&
      (event.element === "click" || event.element === "selected");
    ...
    this.workspace.getAllBlocks(true).forEach((block) => {
      if (
        ...
        block.setErrorHighlighted(
          should_highlight,
          block.error_message || undefined,
        );

        // Automatically expand blocks that have been highlighted.
        if (
          force_check &&
          (block.is_error_highlighted || block.hasErrorHighlightedDescendant())
        ) {
  ...
  static handleDropOver(event, handleFileChange) {
    ...
    const local_drag_zone = document.getElementById(
