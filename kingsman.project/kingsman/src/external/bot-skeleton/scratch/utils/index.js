...
export const getSelectedTradeType = (
  ...
  if (selected_trade_type === "multiplier")
    mandatory_tradeoptions_block = "trade_definition_multiplier";
  if (selected_trade_type === "accumulator")
    mandatory_tradeoptions_block = "trade_definition_accumulator";
...
export const extractTranslateValues = () => {
  const transform_value =
    window.Blockly?.derivWorkspace?.trashcan?.svgGroup.getAttribute(
      "transform",
    );
...
export const validateErrorOnBlockDelete = () => {
  ...
  const blockRect = window.Blockly.getSelected()
    ?.getSvgRoot()
  ...
  if (required_block_types?.includes(window.Blockly?.getSelected()?.type)) {
    if (
      ...
      globalObserver.emit(
        "ui.log.error",
        error_message_map?.()?.[window.Blockly?.getSelected()?.type]?.default,
      );
    }
  }
};
...
export const updateWorkspaceName = () => {
  ...
  const file_name =
    load_modal?.dashboard_strategies?.[0]?.name ?? config().default_file_name;
  if (document.title.indexOf("-") > -1) {
    const string_to_replace = document.title.substr(
      document.title.indexOf("-"),
    );
    const new_document_title = document.title.replace(
      string_to_replace,
      `- ${file_name}`,
    );
...
export const isMainBlock = (block_type) =>
  config().mainBlocks.indexOf(block_type) >= 0;
...
export const cleanUpOnLoad = (blocks_to_clean, drop_event, workspace) => {
  ...
  const blockly_top =
    ...
    blockly_metrics.viewHeight -
    blockly_metrics.viewTop;
  const cursor_x = clientX ? (clientX - blockly_left) * scale_cancellation : 0;
  const cursor_y = clientY
    ? (clientY - blockly_top - toolbar_height) * scale_cancellation
    : 0;
...
export const load = async ({
  ...
  try {
    ...
    const event_group = is_collection
      ? `load_collection${Date.now()}`
      : `dbot-load${Date.now()}`;
    window.Blockly.Events.setGroup(event_group);
    removeLimitedBlocks(
      workspace,
      Array.from(blockly_xml).map((xml_block) =>
        xml_block.getAttribute("type"),
      ),
    );
  ...
  return new Promise((resolve, reject) => {
    ...
    Array.from(xml.children).forEach((el_block) => {
      ...
      if (block_type === "loader") {
        el_block.remove();
        const loader = window.Blockly.Xml.domToBlock(
          el_block,
...
export const addDomAsBlock = (el_block, parent_block = null) => {
  ...
  const block_xml = window.Blockly.Xml.blockToDom(
    block_conversion.convertBlockNode(el_block),
  );
  ...
  const block = window.Blockly.Xml.domToBlock(
    block_xml,
    window.Blockly.derivWorkspace,
  );
    ...
    if (required_block_types.includes(block.type)) {
      return (
        (block.childBlocks_.length === 0 &&
          required_block_types.includes(block.category_)) ||
  ...
  return required_block_types.filter((blockType) => {
    if (blockType === "purchase") {
      ...
      return !workspace
        ...
        .some(
          (block) =>
            block.type === "purchase" || block.type === "apollo_purchase",
        );
    }
...
const throwNewErrorMessage = (error_blocks, key) => {
  return error_blocks.forEach((block) => {
    if (key === "misplaced" && block)
      globalObserver.emit(
        "ui.log.error",
        error_message_map?.()?.[block?.type]?.[key],
      );
    else if (key === "missing" && block)
      globalObserver.emit(
        "ui.log.error",
        error_message_map?.()?.[block]?.[key],
      );
      ...
      if (block.disabled && parent_error_message) {
        ...
        block.childBlocks_.forEach((childBlock) => {
          const child_error_message =
            error_message_map?.()?.[childBlock.type]?.[key];
...
export const isAllRequiredBlocksEnabled = (workspace) => {
  ...
  const required_blocks_check = getAllRequiredBlocks(
    workspace,
...
export const scrollWorkspace = (
  ...
  if (is_RTL) {
    ...
    const toolbox_top = document
      .getElementById("gtm-toolbox")
      ?.getBoundingClientRect()?.top;
    const block_canvas_rect_top =
      workspace.svgBlockCanvas_?.getBoundingClientRect()?.top;
 ...
 */
export const runGroupedEvents = (
  use_existing_group,
  callbackFn,
  opt_group_name,
) => {
  const group =
    (use_existing_group && window.Blockly.Events.getGroup()) ||
    opt_group_name ||
    true;
...
export const removeExtraInput = (instance) => {
  ...
  if (collapsed_input && instance.collapsed_ && !collapsed_input.icon_added) {
    ...
    const field_expand_icon = new window.Blockly.FieldImage(
      dropdown_path,
      16,
      16,
      localize("Collapsed"),
    ...
    if (procedures_array.includes(instance.type)) {
      collapsed_input
        ...
        .appendField(
          new window.Blockly.FieldLabel(function_name + args, "header__title"),
        )
...
export const appendCollapsedMainBlocksFields = (block_instance) => {
  try {
    ...
    const [block_image, block_name] =
      block_instance?.inputList?.[0]?.fieldRow.map((field) => field.value_) ||
      [];
    const collapsed_field = block_instance?.getField(
      Blockly.constants.COLLAPSED_FIELD_NAME,
    );
    const collapsed_input = block_instance?.getInput(
      Blockly.constants.COLLAPSED_INPUT_NAME,
    );

    // Initialize the icon_added property if not already done
    if (collapsed_input && collapsed_field) {
      // Add the image and label fields
      collapsed_input.appendField(
        new Blockly.FieldImage(block_image, 25, 25, "", ""),
      );
      collapsed_input.appendField(
...
export const appendCollapsedProcedureBlocksFields = (instance) => {
  ...
  if (collapsed_input && instance.collapsed_ && !collapsed_input.icon_added) {
    ...
    collapsed_input
      ...
      .appendField(
        new Blockly.FieldLabel(function_name + args, "header__title"),
      )
      .appendField(field_expand_icon);
