...
const updateCandles = (candles, ohlc) => {
  ...
  const prevCandles =
    lastCandle.epoch === ohlc.epoch ? candles.slice(0, -1) : candles.slice(1);
...
export default class TicksService {
  ...
  async stopMonitor(options) {
    ...
    if (
      type === "candles" &&
      this.ohlcListeners.hasIn([symbol, Number(granularity), key])
    ) {
  ...
  async unsubscribeIfEmptyListeners(options) {
    ...
    const ohlcListener = this.ohlcListeners.getIn([
      symbol,
      Number(granularity),
    ]);
  ...
  unsubscribeAllAndSubscribeListeners(symbol) {
    ...
    Promise.all(
      subscription.map((id) => doUntilDone(() => api_base.api.forget(id))),
    );
