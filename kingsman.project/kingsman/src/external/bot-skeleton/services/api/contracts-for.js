...
export default class ContractsFor {
  constructor(store) {
    ...
    this.contracts_for = {};
    this.store = store;
    this.server_time = store.server_time;
  ...
  async getBarriers(symbol, trade_type, duration, barrier_types) {
    ...
    const contracts_for_category = await this.getContractsByTradeType(
      symbol,
      trade_type,
    );
    ...
    if (contracts_for_category.length > 0) {
      barrier_types.forEach((barrier_type, index) => {
        ...
        let contract = contracts_for_category.find((c) => {
          ...
          const has_matching_duration =
            durations.findIndex((d) => d.unit === duration) !== -1;
          ...
          const has_matching_barrier_type =
            ...
            (has_selected_offset_type &&
              isOffset(c.barrier || c[barrier_props[index]])) ||
        ...
        if (contract) {
          const barrier_prop_name =
            contract.barriers === 1 ? "barrier" : barrier_props[index];

          if (contract[barrier_prop_name]) {
            const barrier_match = `${contract[barrier_prop_name]}`.match(
              offset_regexp,
            );
            barriers.values[index] = barrier_match
              ? barrier_match[1]
              : contract[barrier_prop_name];
          }
      ...
      if (!has_absolute_default_value) {
        ...
        barrier_types.every(
          (barrier_type) => barrier_type === barrier_types[0],
        ) &&
  ...
  getTradeTypeCategoryNameByTradeType(trade_type) {
    ...
    const trade_type_category =
      this.getTradeTypeCategoryByTradeType(trade_type);
  ...
  async getContractsByTradeType(symbol, trade_type) {
    ...
    return contracts.filter((contract) => {
      ...
      // barrier_category is absent in New API responses — skip the filter when missing
      const has_matching_barrier =
        contract.barrier_category === undefined ||
  ...
  async getContractsFor(symbol) {
    ...
    if (this.contracts_for[symbol]) {
      ...
      const is_expired =
        this.server_time.unix() - timestamp > this.cache_age_in_min * 60;
  ...
  async getDurations(symbol, trade_type, convert_day_to_hours = true) {
    ...
    const contracts = await this.getContractsFor(symbol);
    const { NOT_AVAILABLE_DURATIONS, DEFAULT_DURATION_DROPDOWN_OPTIONS } =
      config();
    ...
    const getDurationIndex = (input) =>
      DEFAULT_DURATION_DROPDOWN_OPTIONS.findIndex(
        (d) => d[1] === input.replace(/\d+/g, ""),
      );
    ...
    const convertDuration = (duration, unit_to_convert) => {
      ...
      DEFAULT_DURATION_DROPDOWN_OPTIONS.slice(
        target_index + 1,
        duration_index + 1,
      )
        .reverse()
    ...
    contracts_for_category.forEach((contract) => {
      ...
      DEFAULT_DURATION_DROPDOWN_OPTIONS.slice(
        ...
        const is_existing_duration =
          durations.findIndex((d) => d.unit === default_duration[1]) !== -1;
    ...
    if (
      contracts_for_category.every(
        (contract) => contract.expiry_type === "intraday",
      )
    ) {
    ...
    return durations.sort(
      (a, b) => getDurationIndex(a.unit) - getDurationIndex(b.unit),
    );
  }
  ...
  async getPredictionRange(symbol, trade_type) {
    ...
    if (
      DIGIT_CATEGORIES.includes(contract_category) &&
      trade_type !== "evenodd"
    ) {
  ...
  async getGroupedTradeTypes(symbol) {
    ...
    for (let i = 0; i < contracts.length; i++) {
      ...
      const trade_type_category = this.getTradeTypeCategoryByTradeType(
        contracts[i].contract_category,
      );
      const trade_type_category_name = this.getTradeTypeCategoryNameByTradeType(
        contracts[i].contract_category,
      );
  ...
  async getTradeTypeByTradeCategory(
    market,
    submarket,
    symbol,
    trade_type_category,
  ) {
    const { NOT_AVAILABLE_DURATIONS, TRADE_TYPE_CATEGORIES, opposites } =
      config();
    ...
    if (subcategories && subcategories.length) {
      for (let i = 0; i < subcategories.length; i++) {
        ...
        const has_durations =
          JSON.stringify(durations) !== JSON.stringify(NOT_AVAILABLE_DURATIONS);
        const is_disabled = this.isDisabledOption({
          market,
          submarket,
          symbol,
  ...
  getHiddenCategories = (trade_types) => {
    ...
    for (let j = 0; j < trade_types.length; j++) {
      ...
      const has_barrier =
        config().QUICK_STRATEGY.DISABLED.BARRIER_TRADE_TYPES.includes(
          trade_type.value,
        );
      const has_prediction =
        config().QUICK_STRATEGY.DISABLED.PREDICTION_TRADE_TYPES.includes(
          trade_type.value,
        );
  ...
  async getTradeTypesForQuickStrategy(symbol, trade_type = "") {
    ...
    const trade_type_categories = await this.getTradeTypeCategories(
      market,
      submarket,
      symbol,
    );
    ...
    for (let i = 0; i < filtered_trade_type_categories.length; i++) {
      ...
      trade_type_options.push(
        ...this.getTradeTypeOptions(trade_types, trade_type_category),
      );
    }
    return trade_type_options;
  }
  ...
  async getTradeTypeCategories(market, submarket, symbol) {
    const { TRADE_TYPE_CATEGORY_NAMES, NOT_AVAILABLE_DROPDOWN_OPTIONS } =
      config();
    const contracts = await this.getContractsFor(symbol);
      ...
      if (trade_type_category_name) {
        ...
        if (!is_disabled) {
          const is_existing_category =
            trade_type_categories.findIndex(
              (c) => c[1] === trade_type_category,
            ) !== -1;

          if (!is_existing_category) {
            trade_type_categories.push([
              trade_type_category_name,
              trade_type_category,
            ]);
          }
        }
  ...
  async getTradeTypes(market, submarket, symbol, trade_type_category) {
    ...
    if (subcategories) {
      for (let i = 0; i < subcategories.length; i++) {
        ...
        if (!is_disabled && has_durations) {
          ...
          trade_types.push([
            types.map((type) => type[Object.keys(type)[0]]).join("/"),
            trade_type,
          ]);
        }
      }
    ...
    return trade_types.length > 0
      ? trade_types
      : config().NOT_AVAILABLE_DROPDOWN_OPTIONS;
  }

  isDisabledOption(compare_obj) {
    return this.disabled_options.some((disabled_obj) =>
      Object.keys(disabled_obj).every(
        (prop) => compare_obj[prop] === disabled_obj[prop],
      ),
    );
  }
  ...
  getContractTypes = (trade_type) => {
    ...
    const categories = opposites[trade_type_value.toUpperCase()].map(
      (opposite) => ({
