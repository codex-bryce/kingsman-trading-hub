...
export default class TradingTimes {
  constructor(store) {
    ...
    this.trading_times = {};
    this.store = store;
    this.server_time = store.server_time.clone();
  }
    ...
    if (!Object.keys(this.trading_times).length) {
      ...
      const periodicUpdate = async () => {
        ...
        if (!next_update_date) {
          ...
          const next_update_moment = this.last_update_moment
            .clone()
            .add(1, "days");
          ...
          Object.keys(this.trading_times).forEach((symbol_name) => {
            is_open_map[symbol_name] =
              this.trading_times[symbol_name].is_opened;
          });
        ...
        const wait_period =
          next_update_date - this.server_time.local().toDate();
  ...
  async updateTradingTimes() {
    ...
    if (!response || response.error) {
      throw new Error(
        response?.error?.message || "Trading times data is unavailable.",
      );
    }
    ...
    if (!markets) {
      throw new Error("Trading times data did not include any markets.");
    }
    ...
    markets?.forEach((market) => {
      ...
      submarkets?.forEach((submarket) => {
        ...
        symbols?.forEach((symbol_obj) => {
          ...
          const open = Array.isArray(times?.open) ? times.open : [];
          const close = Array.isArray(times?.close) ? times.close : [];
          if (!symbol || !open.length || !close.length) return;
          const is_open_all_day =
            open.length === 1 &&
            open[0] === "00:00:00" &&
            close[0] === "23:59:59";
  ...
  calcIsMarketOpened(symbol_name) {
    ...
    const { times, is_open_all_day, is_closed_all_day } =
