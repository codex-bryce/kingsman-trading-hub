...
export const shouldThrowError = (error, errors_to_ignore = []) => {
  ...
  if (error.error?.code === "OpenPositionLimitExceeded")
    globalObserver.emit("bot.recoverOpenPositionLimitExceeded");
  ...
  return new Promise((resolve, reject) => {
    ...
    if (promise) {
      promise.then(resolve).catch((error) => {
         ...
         */
        if (
          shouldThrowError(error, errors_to_ignore) ||
          (api_base && !api_base.is_running)
        ) {
          reject(error);
          return;
        }
...
export const createDetails = (contract) => {
  ...
  console.log(
    "[createDetails] 💰 Profit for bot (REAL API):",
    profit,
    "Result:",
    result,
  );
...
const hasBlockOfType = (targetType, workspace) => {
  ...
  return allBlocks.some(
    (block) => block.type === targetType && !!block.parentBlock_,
  );
};
