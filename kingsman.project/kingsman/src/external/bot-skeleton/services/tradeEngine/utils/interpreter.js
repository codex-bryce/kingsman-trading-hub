...
JSInterpreter.prototype.takeStateSnapshot = function () {
  const newStateStack = cloneThorough(
    this.stateStack,
    undefined,
    undefined,
    undefined,
...
const shouldRestartOnError = (bot, errorName = "") =>
  !unrecoverable_errors.includes(errorName) &&
  botInitialized(bot) &&
  bot.tradeEngine.options.shouldRestartOnError;
...
const timeMachineEnabled = (bot) =>
  botInitialized(bot) && bot.tradeEngine.options.timeMachineEnabled;
...
const Interpreter = () => {
  ...
  function createAsync(js_interpreter, func) {
    const asyncFunc = (...args) => {
      ...
      const first_defined_arg_idx = reversed_args.findIndex(
        (arg) => arg !== undefined,
      );

      // Remove extra undefined args from end of the args
      const function_args =
        first_defined_arg_idx < 0
          ? []
          : reversed_args.slice(first_defined_arg_idx).reverse();
      // End of workaround
  ...
  function initFunc(js_interpreter, scope) {
    ...
    js_interpreter.setProperty(
      scope,
      "console",
      js_interpreter.nativeToPseudo(custom_console),
    );
    js_interpreter.setProperty(
      scope,
      "alert",
      js_interpreter.nativeToPseudo(alert),
    );
    js_interpreter.setProperty(
      scope,
      "prompt",
      js_interpreter.nativeToPseudo(prompt),
    );
    js_interpreter.setProperty(
      scope,
    ...
    js_interpreter.setProperty(
      scope,
      "sleep",
      createAsync(js_interpreter, sleep),
    );
  }
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (!bot.tradeEngine.contractId && is_timeouts_cancellable) {
          ...
          global_timeouts.forEach((timeout) =>
          ...
          isMultiplierContract(
            bot?.tradeEngine?.data?.contract?.contract_type ?? "",
          )
        ) {
    ...
    return new Promise((resolve, reject) => {
      const onError = (e) => {
        ...
        const canRestoreState =
          $scope.startState &&
          interpreter?.restoreStateSnapshot instanceof Function;
        if (canRestoreState) {
