...
export default class TradeEngine extends Balance(
  ...
  async start(tradeOptions) {
    ...
    // CRITICAL: API should already be using demo account if show_as_cr is set
    // V2GetActiveToken() and V2GetActiveClientId() handle this automatically during init
    // No need to switch accounts here - API is already on demo account
    const showAsCR =
      typeof window !== "undefined" ? localStorage.getItem("show_as_cr") : null;
    if (showAsCR) {
      console.log(
        "✅ [TradeEngine] Special CR account detected - API should already be on demo account",
      );
      console.log(
        "✅ [TradeEngine] Current API account:",
        api_base.account_info?.loginid,
      );
      console.log(
        "✅ [TradeEngine] Current API balance:",
        ...
        if (
          ...
          this.transaction_recovery_timeout = setTimeout(() => {
            ...
            const is_same_contract =
              contract.contract_id === data.transaction.contract_id;
  ...
  makeDirectPurchaseDecision() {
    const { has_payout_block, is_basis_payout } =
      checkBlocksForProposalRequest();
    this.is_proposal_subscription_required =
      has_payout_block || is_basis_payout;
