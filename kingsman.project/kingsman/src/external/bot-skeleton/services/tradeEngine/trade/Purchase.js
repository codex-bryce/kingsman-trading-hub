  ...
  class Purchase extends Engine {
    applyAlternateMarketsToCurrentTradeOptions() {
      try {
        ...
        if (
          force_symbol &&
          force_symbol !== "disable" &&
          this.tradeOptions?.symbol !== force_symbol
        ) {
        ...
        const settings =
          (window && window.DBot && window.DBot.__alt_markets) || {};
        const enabled = !!settings.enabled;
        const every = Number(settings.every || 0);
        if (!enabled || !every || !this.tradeOptions?.symbol)
          return this.tradeOptions;

        // Next run index is current completed runs + 1 (about to buy)
        const next_run_index =
          (typeof this.getTotalRuns === "function" ? this.getTotalRuns() : 0) +
          1;
        if (next_run_index % every !== 0) return this.tradeOptions;

        const helper_instance = ApiHelpers?.instance;
        const list =
          helper_instance?.active_symbols?.getSymbolsForBot?.() || [];
        const cont = list.filter((s) =>
          (s?.group || "").startsWith("Continuous Indices"),
        );
    ...
    async purchase(contract_type) {
      ...
      console.log(
        "💰 [PURCHASE] Current API balance:",
        api_base.account_info?.balance,
      );
      ...
      const isSpecialCR =
        displayedAccount && isSpecialCRAccount(displayedAccount);
      const shouldUseDemo =
        isSpecialCR || !!(showAsCR && isSpecialCRAccount(showAsCR));
      ...
      if (shouldUseDemo) {
        console.log(
          "✅ [PURCHASE] Special CR account - API should already be on demo account",
        );
        console.log(
          "✅ [PURCHASE] Current API account:",
          api_base.account_info?.loginid,
        );
        console.log(
          "✅ [PURCHASE] Current API balance:",
        ...
        if (
          api_base.account_info?.loginid &&
          !isDemoLoginId(api_base.account_info.loginid)
        ) {
          console.warn(
        ...
        if (
          ...
          (!this.accountInfo ||
            this.accountInfo.loginid !== api_base.account_info.loginid)
        ) {
          this.accountInfo = {
            ...api_base.account_info,
            loginid: api_base.account_info.loginid,
          };
          console.log(
            "✅ [PURCHASE] Normal account - set accountInfo to:",
            this.accountInfo.loginid,
          );
        }
      }
      ...
      console.log(
        "💰 [PURCHASE] Final API account:",
        api_base.account_info?.loginid,
      );
      console.log(
        "💰 [PURCHASE] Final API balance:",
        api_base.account_info?.balance,
      );
      console.log("💰 [PURCHASE] ============================================");

      const activeToken = localStorage.getItem("authToken") || "";
      if (activeToken && activeToken.startsWith("ory_")) {
        const hasTradingSocket =
          await api_base.ensureOAuthTradingSocket("purchase");
        if (!hasTradingSocket) {
          throw new Error(
            "OAuth trading session is not ready yet. Please refresh and sign in again.",
          );
        }
      }
      ...
      if (shouldUseDemo) {
        const demoToken = getPreferredDemoToken(showAsCR);
        const demoLoginId = getPreferredDemoLoginId(showAsCR);
        ...
        if (demoLoginId && !isOnDemoAccount && demoToken && api_base.api) {
          console.warn(
            "⚠️ [PURCHASE] API not on demo account! Current:",
            api_base.account_info?.loginid,
          );
          console.warn(
            "⚠️ [PURCHASE] Re-authorizing with demo token synchronously...",
          );
          ...
          try {
            const { authorize, error } =
              await api_base.api.authorize(demoToken);
            if (error) {
              console.error(
                "❌ [PURCHASE] Failed to re-authorize with demo token:",
                error,
              );
              ...
              console.log(
                "✅ [PURCHASE] Re-authorized with demo account:",
                demoLoginId,
              );
              console.log(
                "✅ [PURCHASE] Demo account balance:",
                authorize?.balance,
              );
            }
          } catch (authError) {
          ...
          console.log(
            "✅ [PURCHASE] API already on demo account:",
            api_base.account_info?.loginid,
          );
        }
      }
      ...
      const onSuccess = (response) => {
        ...
        const currentApiAccount =
          api_base.account_info?.loginid || this.accountInfo?.loginid;
        console.log(
          "[Purchase] 📨 Subscribing to contract updates for:",
          buy.contract_id,
        );
        ...
        try {
          ...
          const subscriptionPromise = doUntilDone(() => {
            console.log(
              "[Purchase] 📡 Sending contract subscription request...",
            );
            return api_base
              .ensureOAuthTradingSocket("proposal_open_contract")
              .then((hasTradingSocket) => {
                if (!hasTradingSocket) {
                  throw new Error(
                    "OAuth trading session is not ready yet. Please refresh and sign in again.",
                  );
          ...
          Promise.race([
            ...
            .catch((err) => {
              ...
              setTimeout(() => {
                try {
                  console.log(
                    "[Purchase] 🔄 Retrying contract subscription...",
                  );
                  api_base.api.send({
                    proposal_open_contract: 1,
                    contract_id: buy.contract_id,
                  ...
                  console.log(
                    "[Purchase] ✅ Contract subscription sent (final attempt)",
                  );
                } catch (finalErr) {
                  console.error(
                    "[Purchase] ❌ Final subscription attempt failed:",
                    finalErr,
                  );
                }
              }, 500);
          ...
          try {
            ...
            console.error(
              "[Purchase] ❌ Fallback subscription failed:",
              fallbackErr,
            );
          }
        }
        ...
        // CRITICAL: Use the actual API account ID (demo account if we switched)
        // This ensures contract events are tracked with the correct account
        // ALWAYS use the current API account (which should be demo after switch)
        const accountIdForInfo =
          api_base.account_info?.loginid || this.accountInfo?.loginid;
        console.log(
          "[Purchase] 📢 Emitting info() with accountID:",
          accountIdForInfo,
        );
        console.log("[Purchase] 📢 Contract ID:", buy.contract_id);
        console.log("[Purchase] 📢 Transaction ID:", buy.transaction_id);
        ...
        console.log(
          "[Purchase] 📢 Balance after purchase:",
          api_base.account_info?.balance,
        );

        // CRITICAL: All balance updates and contract tracking use demo account ID
        // This ensures balance is deducted from demo account, not special CR account
        info({
          accountID: accountIdForInfo, // This is the demo account ID (VRTC14696449)
      ...
      if (this.is_proposal_subscription_required) {
        ...
        const action = () => {
          ...
          return api_base
            .ensureOAuthTradingSocket("buy_proposal")
            .then((hasTradingSocket) => {
              if (!hasTradingSocket) {
                throw new Error(
                  "OAuth trading session is not ready yet. Please refresh and sign in again.",
      ...
      try {
        ...
        if (dbot?.interpreter?.bot?.tradeEngine) {
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                console.warn(
                  "[Martingale Fix] Could not read Stake variable:",
                  e3,
                );
              }
            }
          ...
          if (
            ...
            this.tradeOptions.amount = Number(
              stakeValue.toFixed(decimalPlaces),
            );
            console.log(
        ...
        console.warn(
          "[Martingale Fix] Error updating tradeOptions.amount from Stake variable:",
          e,
        );
      }
      ...
      try {
        globalObserver.emit("replicator.purchase", {
          ...
          tradeOptions: this.tradeOptions,
          contract_type,
          account_id: shouldUseDemo
            ? getPreferredDemoLoginId(showAsCR) || this.accountInfo?.loginid
            : this.accountInfo?.loginid,
        });
      } catch {}
      ...
      const action = () =>
        api_base
          .ensureOAuthTradingSocket("buy_parameters")
          .then((hasTradingSocket) => {
            if (!hasTradingSocket) {
              throw new Error(
                "OAuth trading session is not ready yet. Please refresh and sign in again.",
    ...
    shouldUseDemoAccountForTrade() {
      ...
      // CRITICAL: Check show_as_cr flag first - when CR9641252 is displayed,
      ...
      if (showAsCR && isSpecialCRAccount(showAsCR)) {
        console.log(
          "[Purchase] 🎯 Special CR account detected via show_as_cr:",
          showAsCR,
        );
        return true;
      }
    ...
    async switchToDemoAccountForTrade(demoToken, demoLoginId) {
      if (!api_base.api || !demoToken || !demoLoginId) {
        console.error(
          "[Special CR Account] Missing required parameters for account switch",
        );
        return false;
      }
      ...
      try {
        ...
        if (error) {
          console.error(
            "[Special CR Account] Failed to authorize with demo account:",
            error,
          );
          return false;
        }
        ...
        if (authorize) {
          ...
          console.log(
            `[Special CR Account] Successfully switched to demo account ${demoLoginId}`,
          );
          console.log(
            `[Special CR Account] Demo account balance: ${authorize.balance || "N/A"}`,
          );
          return true;
        } else {
        ...
        console.error(
          "[Special CR Account] Error switching to demo account:",
          error,
        );
        return false;
      }
