...
const getIndicatorsInterface = (tradeEngine) => {
  return {
    ...
    bb: (input, config, field) =>
      decorate(bb, input, tradeEngine, config)[field],
    bba: (input, config, field) =>
      decorate(bba, input, tradeEngine, config).map((r) => r[field]),
    macda: (input, config, field) =>
      decorate(macda, input, tradeEngine, config).map((r) => r[field]),
  };
};
