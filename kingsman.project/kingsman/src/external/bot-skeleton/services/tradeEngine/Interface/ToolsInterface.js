...
const getToolsInterface = (tradeEngine) => {
  return {
    dateTimeStringToTimestamp: (datetime_string) => {
      const invalid_msg = localize("Invalid date/time: {{ datetime_string }}", {
        datetime_string,
      });
      ...
      if (date_time.length >= 2) {
        validated_datetime =
          d.test(date_time[0]) && t.test(date_time[1])
            ? `${date_time[0]}T${date_time[1]}`
            : null;
    ...
    highlightBlock: (block_id) => {
      ...
      window.Blockly.BlockSvg.prototype.highlightExecutedBlock = function () {
        ...
        if (
          !window.Blockly.utils.dom.hasClass(
            this.svgGroup_,
