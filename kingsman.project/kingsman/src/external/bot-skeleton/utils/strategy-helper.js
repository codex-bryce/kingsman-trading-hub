...
export const extractBlocksFromXml = (xml) => {
  ...
  const blocksTypeAndFields = Array.from(blocks).map((block) => {
    ...
    const fields = Array.from(block.getElementsByTagName("field")).map(
      (field) => {
        return {
...
export const compareXml = (xml1, xml2) => {
  ...
  for (let i = 0; i < blocks1.length; i++) {
    if (
      blocks1[i]?.type !== blocks2[i]?.type ||
      !areFieldsEqual(blocks1[i]?.fields, blocks2[i]?.fields)
    ) {
      return false;
    }
  }
