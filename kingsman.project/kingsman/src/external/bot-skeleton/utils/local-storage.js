...
export const saveWorkspaceToRecent = async (
  ...
  const current_workspace_index = workspaces.findIndex(
  ...
  localForage.setItem(
    "saved_workspaces",
    LZString.compress(JSON.stringify(workspaces)),
  );
};
...
export const getSavedWorkspaces = async () => {
  try {
    return (
      JSON.parse(
        LZString.decompress(await localForage.getItem("saved_workspaces")),
      ) || []
    );
  } catch (e) {
    return [];
...
export const convertStrategyToIsDbot = (xml_dom) => {
  if (!xml_dom) return;
  if (
    xml_dom.hasAttribute("collection") &&
    xml_dom.getAttribute("collection") === "true"
  ) {
