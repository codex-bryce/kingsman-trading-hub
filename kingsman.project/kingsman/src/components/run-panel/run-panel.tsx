...
const StatisticsTile = ({
  ...
  <div className="run-panel__tile">
    ...
    <div className={classNames("run-panel__tile-content", contentClassName)}>
      {content}
    </div>
  </div>
);
  ...
  >
    ...
    <div className="run-panel__stat--tiles">
      <StatisticsTile
        ...
        alignment="top"
        content={
          <Money amount={total_stake} currency={currency} show_currency />
        }
      />
      <StatisticsTile
        ...
        alignment="top"
        content={
          <Money amount={total_payout} currency={currency} show_currency />
        }
      />
      <StatisticsTile
    ...
    <>
      <Tabs active_index={active_index} onTabItemClick={setActiveTabIndex} top>
        <div
          ...
          label={<Localize i18n_default_text="Summary" />}
        >
        ...
        <div
          ...
          label={<Localize i18n_default_text="Journal" />}
        >
          <Journal />
        </div>
      </Tabs>
...
const DesktopPanelHandle = ({
  ...
  <button
    id={
      is_drawer_open
        ? "db-run-panel__desktop-hide-button"
        : "db-run-panel__desktop-open-button"
    }
    className={classNames("run-panel__desktop-handle", {
      "run-panel__desktop-handle--closed": !is_drawer_open,
    })}
    onClick={() => toggleDrawer(!is_drawer_open)}
    type="button"
    aria-label={localize(is_drawer_open ? "Hide panel" : "Open panel")}
  >
    <span>{is_drawer_open ? ">" : "<"}</span>
  </button>
);
...
const StatisticsInfoModal = ({
  ...
  return (
    <Modal
      className={classNames("statistics__modal", {
        "statistics__modal--mobile": is_mobile,
      })}
      ...
      <Modal.Body>
        <div
          className={classNames("statistics__modal-body", {
            "statistics__modal-body--mobile": is_mobile,
          })}
        >
...
const RunPanel = observer(() => {
  ...
  return (
    <>
      {isDesktop ? (
        <div
          className={classNames("run-panel__desktop-fixed", {
            "run-panel__desktop-fixed--closed": !is_drawer_open,
            "run-panel__desktop-fixed--tour-active": active_tour,
          })}
          style={{ zIndex: popover_zindex.RUN_PANEL }}
        >
          <DesktopPanelHandle
            is_drawer_open={is_drawer_open}
          ...
          />
          {is_drawer_open && (
            <div className="run-panel__desktop-body run-panel__content">
              {content}
            </div>
          )}
          {is_drawer_open && <div className="dc-drawer__footer">{footer}</div>}
        </div>
      ) : (
        <div
          className={
            is_drawer_open ? "run-panel__container--mobile" : "run-panel"
          }
        >
          <Drawer
            anchor="right"
          ...
          {active_tab !== STRATEGIES && <MobileDrawerFooter />}
        </div>
      )}
