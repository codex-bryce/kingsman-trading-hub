...
const Layout = () => {
  ...
  const checkClientAccount = JSON.parse(
  ...
  const ifClientAccountHasCurrency =
    Object.values(checkClientAccount).some(
      (account: any) => account.currency === currency,
    ) ||
    currency === "demo" ||
    currency === "";
  const [clientHasCurrency, setClientHasCurrency] = useState(
  ...
  const validateApiAccounts = ({ data }: any) => {
    if (data.msg_type === "authorize") {
      ...
      const account_list_filter = account_list.filter(
        (acc: any) => acc.is_disabled === 0,
      );
      api_accounts.push(account_list_filter || []);
      const allCurrencies = new Set(
        Object.values(checkClientAccount).map((acc: any) => acc.currency),
      );
      ...
      if (hasMissingCurrency || hasMissingToken) {
        ...
        const account_list_ =
          account_list_filter?.find(
            (acc: { currency: string }) => acc.currency === currency,
        ...
        if (!new URLSearchParams(window.location.search).has("account")) {
          window.history.pushState(
            {},
            "",
            `${window.location.pathname}?${session_storage_currency}`,
          );
        }
  ...
  useEffect(() => {
    ...
    const checkOIDCEnabledWithMissingAccount =
      !isEndpointPage && !clientHasCurrency;
    const shouldAuthenticate =
      (isLoggedInCookie && !isClientAccountsPopulated && !isEndpointPage) ||
  ...
  return (
    <div className={clsx("layout", { responsive: isDesktop })}>
      <AppHeader
        isAuthenticating={isAuthenticating || !isInitialAuthCheckComplete}
      />
      <Body>
        <Outlet />
      </Body>
