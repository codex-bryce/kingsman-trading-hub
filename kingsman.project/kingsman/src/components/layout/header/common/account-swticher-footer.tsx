...
import { isDemoLoginId, isManagedRealLoginId } from "@/utils/account-prefixes";
import {
  getWalletUrl,
  handleTraderHubRedirect,
} from "@/utils/traders-hub-redirect";
...
const AccountSwitcherFooter = ({
  ...
  const show_manage_button = isManagedRealLoginId(loginid);
  ...
  const is_virtual =
    isDemoLoginId(loginid) || account_param === "demo" || false;
  ...
  const redirect_url_str =
    handleTraderHubRedirect(redirectParams) || standalone_routes.traders_hub;
    ...
    <div className="">
      ...
      <div
        className={classNames("account-switcher-footer__actions", {
          "account-switcher-footer__actions--hide-manage-button":
            !show_manage_button,
        })}
      >
        {show_manage_button && (
