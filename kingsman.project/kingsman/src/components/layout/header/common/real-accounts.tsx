...
const RealAccounts = ({
  ...
  const hasNonEuAccounts =
    modifiedCRAccountList && modifiedCRAccountList?.length > 0;
  const hasEuAccounts =
    modifiedMFAccountList && modifiedMFAccountList?.length > 0;
  ...
  return (
    <>
      ...
      {!hasNonEuAccounts && !hasEuAccounts && upgradeable_landing_companies && (
        <div
          key={upgradeable_landing_companies}
