...
export const AccountSwitcherWallet = observer(
  ({ is_visible, toggle, residence }: TAccountSwitcherWalletProps) => {
    const { data: wallet_list, has_wallet = false } =
      useStoreWalletAccountsList() || {};
    const { hubEnabledCountryList } = useFirebaseCountriesConfig();
    const dtrade_account_wallets = wallet_list?.filter(
      (wallet) => wallet.dtrade_loginid,
    );
    ...
    const handleTradersHubRedirect = async () => {
      ...
      const redirect_url_str =
        handleTraderHubRedirect(redirectParams) ||
