...
export const isEuLandingCompany = (landing_company) =>
  /^(maltainvest|malta|iom)$/.test(landing_company);

export const hasEuAccount = (token_list) =>
  token_list.some((token_obj) =>
    isEuLandingCompany(token_obj.loginInfo.landing_company_name),
  );
...
const isLowRisk = (financial, gaming, upgrade) => {
  ...
  return (
    (f_sc === "maltainvest" && g_sc === "svg") ||
    (upgrade?.includes("svg") && upgrade?.includes("maltainvest"))
  );
};
...
const isHighRisk = (financial_company, gaming_company, risk_classification) => {
  ...
  return (
    ...
    high_risk_landing_company ||
    restricted_countries
  );
};

const isMultiplier = (landing_company_list) => {
  const multiplier_account =
    landing_company_list?.financial_company?.legal_allowed_contract_categories;
...
export const checkSwitcherType = async (account_data) => {
  ...
  let is_low_risk = isLowRisk(
    financial_company,
    gaming_company,
    upgradeable_landing_companies,
  );
  let is_high_risk = isHighRisk(
    financial_company,
    gaming_company,
    risk_classification,
  );
  const low_risk_no_account =
    is_low_risk && Object.keys(client_accounts).length === 1;
  const high_risk_no_account =
    is_high_risk && Object.keys(client_accounts).length === 1;
  ...
  account_data.modifiedAccountList.forEach((account) => {
    if (isDemoLoginId(account.loginid))
      virtual_accounts.push({ ...client_accounts[account], account });
    if (isEuRealLoginId(account.loginid))
      eu_accounts.push({ ...client_accounts[account], account });
    if (isNonEuRealLoginId(account.loginid))
      non_eu_accounts.push({ ...client_accounts[account], account });
  });

  const real_accounts = eu_accounts.length + non_eu_accounts.length;
  const renderCountryIsLowRiskAndHasNoRealAccount =
    !isVirtual && is_country_low_risk && real_accounts === 0;
  const renderCountryIsEuAndNoRealAccount =
    !isVirtual && !is_country_low_risk && is_eu && real_accounts === 0;
