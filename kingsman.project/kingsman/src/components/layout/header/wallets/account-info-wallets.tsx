...
type TInfoIcons = {
  gradients: Exclude<
    ReturnType<typeof useStoreWalletAccountsList>["data"],
    undefined
  >[number]["gradients"];
  icons: Exclude<
    ReturnType<typeof useStoreWalletAccountsList>["data"],
...
const MobileInfoIcon = observer(
  ({ gradients, icons, icon_type }: TInfoIcons) => {
    const {
      ui: { is_dark_mode_on },
    } = useStore();
...
const DesktopInfoIcons = observer(
  ({ gradients, icons, icon_type }: TInfoIcons) => {
    const { ui } = useStore();
...
const AccountInfoWallets = observer(
  ({ is_dialog_on, toggleDialog }: TAccountInfoWallets) => {
    ...
    const linked_wallet = wallet_list?.find(
      (wallet) =>
        wallet.dtrade_loginid === linked_dtrade_trading_account_loginid,
    );
      ...
      <div className="acc-info__wrapper">
        ...
        >
          <div
            ...
            onClick={
              active_account?.is_disabled ? undefined : () => toggleDialog()
            }
          >
            {isDesktop ? (
              <DesktopInfoIcons
