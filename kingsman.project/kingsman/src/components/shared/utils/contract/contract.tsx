...
export const getContractStatus = ({
  ...
  const closed_contract_status =
    profit && profit < 0 && exit_tick_time ? "lost" : "won";
...
export const getCancellationPrice = (contract_info: TContractInfo) => {
  const { cancellation: { ask_price: cancellation_price = 0 } = {} } =
    contract_info;
...
export const isOpen = (contract_info: TContractInfo) =>
  getContractStatus(contract_info) === "open";

export const isUserSold = (contract_info: TContractInfo) =>
  contract_info.status === "sold";

export const isValidToCancel = (contract_info: TContractInfo) =>
  !!contract_info.is_valid_to_cancel;

export const isValidToSell = (contract_info: TContractInfo) =>
  !isEnded(contract_info) &&
  !isUserSold(contract_info) &&
  !!contract_info.is_valid_to_sell;

export const hasContractEntered = (contract_info: TContractInfo) =>
  !!contract_info.entry_spot;

export const hasBarrier = (contract_type = "") =>
  /VANILLA|TURBOS|HIGH_LOW|TOUCH/i.test(contract_type);

export const hasTwoBarriers = (contract_type = "") =>
  /EXPIRY|RANGE|UPORDOWN/i.test(contract_type);
...
export const isMultiplierContract = (contract_type = "") =>
  /MULT/i.test(contract_type);

export const isTouchContract = (contract_type: string) =>
  /TOUCH/i.test(contract_type);

export const isTurbosContract = (contract_type = "") =>
  /TURBOS/i.test(contract_type);

export const isVanillaContract = (contract_type = "") =>
  /VANILLA/i.test(contract_type);

export const isVanillaFxContract = (contract_type = "", symbol = "") =>
  isVanillaContract(contract_type) &&
  VANILLA_FX_SYMBOLS.includes(symbol as (typeof VANILLA_FX_SYMBOLS)[number]);

export const isSmartTraderContract = (contract_type = "") =>
  /RUN|EXPIRY|RANGE|UPORDOWN|ASIAN|RESET|TICK|LB/i.test(contract_type);

export const isAsiansContract = (contract_type = "") =>
  /ASIAN/i.test(contract_type);

export const isLookBacksContract = (contract_type = "") =>
  /LB/i.test(contract_type);

export const isTicksContract = (contract_type = "") =>
  /TICK/i.test(contract_type);

export const isResetContract = (contract_type = "") =>
  /RESET/i.test(contract_type);

export const isCryptoContract = (underlying = "") =>
  underlying.startsWith("cry");

export const getAccuBarriersDefaultTimeout = (symbol: string) => {
  return symbols_2s.includes(symbol)
    ? DELAY_TIME_1S_SYMBOL * 2
    : DELAY_TIME_1S_SYMBOL;
};
...
export const getAccuBarriersDTraderTimeout: TGetAccuBarriersDTraderTimeout = ({
  ...
  if (has_default_timeout || !tick_update_timestamp)
    return getAccuBarriersDefaultTimeout(underlying);
  const target_update_time =
    tick_update_timestamp +
    getAccuBarriersDefaultTimeout(underlying) +
    ANIMATION_CORRECTION_TIME;
  const difference = target_update_time - barriers_update_timestamp;
  return difference < 0 ? 0 : difference;
};

export const getAccuBarriersForContractDetails = (
  contract_info: TContractInfo,
) => {
  ...
  const accu_high_barrier = is_contract_open
    ? current_spot_high_barrier
    : high_barrier;
  const accu_low_barrier = is_contract_open
    ? current_spot_low_barrier
    : low_barrier;
...
export const getCurrentTick = (contract_info: TContractInfo) => {
  ...
  const current_tick =
    ...
    isAsiansContract(contract_info.contract_type)
      ? tick_stream.length
...
export const getLastContractMarkerIndex = (markers: TContractStore[] = []) => {
  const sorted_markers = [...markers].sort(
    (a, b) =>
      Number(b.contract_info.date_start) - Number(a.contract_info.date_start),
  );
  const index = sorted_markers[0].contract_info.date_start
    ? markers.indexOf(sorted_markers[0])
    : -1;
  return index >= 0 ? index : markers.length - 1;
};

export const getLastTickFromTickStream = (tick_stream: TTickItem[] = []) =>
  tick_stream[tick_stream.length - 1] || {};

export const isDigitContract = (contract_type = "") =>
  /digit/i.test(contract_type);

export const getDigitInfo = (
...
export const getTimePercentage = (
  ...
  const duration_from_purchase = moment.duration(
    moment.unix(expiry_time).diff(moment.unix(start_time)),
  );
  const duration_from_now = moment.duration(
    moment.unix(expiry_time).diff(server_time),
  );
  let percentage =
    (duration_from_now.asMilliseconds() /
      duration_from_purchase.asMilliseconds()) *
    100;
...
export const getGrowthRatePercentage = (growth_rate: number) =>
  growth_rate * 100;
...
export const getContractUpdateConfig = ({
  contract_update,
  limit_order,
}: TContractInfo) => {
  const { stop_loss, take_profit } = getLimitOrderAmount(
    limit_order || contract_update,
  );
...
export const shouldShowCancellation = (symbol = "") =>
  !/^(cry|CRASH|BOOM|stpRNG|WLD|JD)/.test(symbol);

export const getContractSubtype = (type = "") =>
  /(VANILLALONG|TURBOS)/i.test(type)
    ? capitalizeFirstLetter(
        type.replace(/(VANILLALONG|TURBOS)/i, "").toLowerCase(),
      )
    : "";
...
export const getSortedTradeTypes = (array: string[] = []) => {
  if (array.includes(TRADE_TYPES.ACCUMULATOR)) {
    return [
      ...
      ...array.filter((type) => type !== TRADE_TYPES.ACCUMULATOR),
    ];
  }
  if (array.includes(TRADE_TYPES.MULTIPLIER)) {
    return [
      TRADE_TYPES.MULTIPLIER,
      ...array.filter((type) => type !== TRADE_TYPES.MULTIPLIER),
    ];
  }
  return array;
};

export const isForwardStartingBuyTransaction = (
