...
const getParamTextByValue = (value: number | string, key: string) =>
  tradeURLParamsConfig[key].find(
    (interval) => interval.value === value.toString(),
  )?.text ?? "";

export const getTradeURLParams = ({
  active_symbols = [],
  contract_types_list = {},
}: TGetTradeURLParamsArgs = {}) => {
  const searchParams = new URLSearchParams(window.location.search);
  const result: TTradeUrlParams & { showModal?: boolean } = {};
  if (searchParams.toString()) {
    const { chart_type, interval, trade_type, symbol } = [
      ...searchParams.entries(),
    ].reduce<{
      [key: string]: string;
    }>((acc, [key, value]) => ({ ...acc, [key]: value }), {});
    const validInterval = tradeURLParamsConfig.interval.find(
      (item) => item.text === interval,
    );
    const validChartType = tradeURLParamsConfig.chartType.find(
      (item) => item.text === chart_type,
    );
    const chartTypeParam =
      Number(validInterval?.value) === 0 ? "line" : validChartType?.value;
    const isSymbolValid = active_symbols.some((item) => item.symbol === symbol);
    const contractList = Object.keys(contract_types_list).reduce<string[]>(
      (acc, key) => {
        ...
        return [
          ...acc,
          ...categories.map(
            (contract) => (contract as TTextValueStrings).value,
          ),
        ];
      },
      [],
...
export const setTradeURLParams = ({
  contractType,
  symbol,
  chartType,
  granularity,
  ...
  chartType &&
    searchParams.set(
      TRADE_URL_PARAMS.CHART_TYPE,
      getParamTextByValue(chartType, "chartType"),
    );
  ...
  if (
    ...
    window.location.pathname === standalone_routes.trade
  ) {
