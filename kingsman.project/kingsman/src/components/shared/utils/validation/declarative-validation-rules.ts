...
export const validPassword = (value: string) =>
  /^(?=.*[a-z])(?=.*\d)(?=.*[A-Z])[!-~]{8,25}$/.test(value);
export const validEmail = (value: string) =>
  /^[a-zA-Z0-9_.+-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,63}$/.test(value);
const validBarrier = (value: string) => /^[+-]?\d+\.?\d*$/.test(value);
const validGeneral = (value: string) =>
  !/[`~!@#$%^&*)(_=+[}{\]\\/";:?><|]+/.test(value);
const validRegular = (value: string, options: TOptions) =>
  options.regex?.test(value);
...
export const hasInvalidCharacters = (target_string: string) =>
  /[^\dX\s]/.test(target_string);
...
export const validFile = (file: File) =>
  file?.type && /(image|application)\/(jpe?g|pdf|png)$/.test(file?.type);
export const validMT5Password = (value: string) =>
  /^(?=.*[!@#$%^&*()+\-=[\]{};':"|,.<>/?_~])[ -~]{8,16}$/.test(value);

let pre_build_dvrs: TInitPreBuildDVRs,
  form_error_messages: TFormErrorMessagesTypes;
...
export const validNumber = (value: string, opts: TOptions) => {
  ...
  if (
    !(options.type === "float" ? /^\d*(\.\d+)?$/ : /^\d+$/).test(value) ||
    isNaN(+value)
  ) {
    is_ok = false;
    ...
    is_ok = false;
    message = form_error_messages.value(
      addComma(options.min, options.decimals),
    );
  } else if (
    "min" in options &&
    "max" in options &&
...
const initPreBuildDVRs = () => ({
  ...
  signup_token: {
    ...
    message: form_error_messages.signup_token,
  },
  tax_id: {
    func: validTaxID,
...
export const initFormErrorMessages = (
  all_form_error_messages: Partial<TFormErrorMessagesTypes>,
) => {
  if (!pre_build_dvrs) {
