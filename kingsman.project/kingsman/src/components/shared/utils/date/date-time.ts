...
export const toMoment = (value?: moment.MomentInput): moment.Moment => {
  ...
  if (/invalid/i.test(moment(value).toString())) {
    ...
    const value_as_number =
      moment.utc(value, "DD MMM YYYY").valueOf() / (1000 * 60 * 60 * 24);
    return value_as_number > days_in_month
      ? moment.utc(
          today_moment.add(value as string | number, "d"),
          "DD MMM YYYY",
        )
...
export const toLocalFormat = (time: moment.MomentInput) =>
  moment.utc(time).local().format("YYYY-MM-DD HH:mm:ss Z");
 ...
 */
export const convertToUnix = (epoch: number | string, time: string) =>
  setTime(toMoment(epoch), time).unix();
...
export const formatDate = (
  ...
  should_format_null = true,
) =>
  !should_format_null && date === null
    ? undefined
    : toMoment(date).format(date_format);

export const formatTime = (
  epoch: number | string,
  time_format = "HH:mm:ss [GMT]",
 ...
 */
export const diffInMonths = (now: moment.MomentInput, then: moment.Moment) =>
  then.diff(now, "month");
/**
...
export const isTimeValid = (time_str: string) =>
  /^([0-9]|[0-1][0-9]|2[0-3]):([0-9]|[0-5][0-9])(:([0-9]|[0-5][0-9]))?$/.test(
    time_str,
  );
 ...
 * @param {String} time_str time
 */
export const isMinuteValid = (time_str: string) =>
  isTimeValid(time_str) && /^[0-5][0-9]$/.test(time_str.split(":")[1]);

/**
 * return true if the date is typeof string and a valid moment date, else return false
 * @param {String|moment} date date
 */
export const isDateValid = (date: moment.MomentInput) =>
 ...
 */
export const addWeeks = (date: string, num_of_weeks: number) =>
  toMoment(date).clone().add(num_of_weeks, "week");
...
export const minDate = (
  date_1: moment.MomentInput,
  date_2: moment.MomentInput,
...
export const formatMilliseconds = (
  miliseconds: moment.MomentInput,
  str_format: string,
  is_local_time = false,
) => {
  if (is_local_time) {
...
export const getTimeSince = (timestamp: number) => {
  ...
  if (seconds_passed < 3600) {
    return localize("{{minutes_passed}}m ago", {
      minutes_passed: Math.floor(seconds_passed / 60),
    });
  }
  if (seconds_passed < 86400) {
    return localize("{{hours_passed}}h ago", {
      hours_passed: Math.floor(seconds_passed / 3600),
    });
  }
  return localize("{{days_passed}}d ago", {
    days_passed: Math.floor(seconds_passed / (3600 * 24)),
  });
};
