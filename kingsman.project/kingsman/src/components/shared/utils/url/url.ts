...
export const legacyUrlForLanguage = (
  ...
  url.replace(
    new RegExp(`/${default_language}/`, "i"),
    `/${(target_language || "EN").trim().toLowerCase()}/`,
  );

export const urlForLanguage = (
  lang: string,
  url: string = window.location.href,
) => {
...
export const urlForCurrentDomain = (href: string) => {
  ...
  if (Object.keys(host_map).includes(url_object.hostname)) {
    url_object.hostname =
      host_map[url_object.hostname as keyof typeof host_map];
...
export const getStaticUrl = (
  ...
  const host = is_eu_url
    ? deriv_urls.DERIV_COM_PRODUCTION_EU
    : deriv_urls.DERIV_COM_PRODUCTION;
...
export const getPath = (route_path: string, parameters = {}) =>
  Object.keys(parameters).reduce(
    (p, name) =>
      p.replace(`:${name}`, parameters[name as keyof typeof parameters]),
    route_path,
  );

export const getContractPath = (contract_id?: number) =>
  getPath(standalone_routes.contract, { contract_id });
 ...
 */
export const filterUrlQuery = (
  search_param: string,
  allowed_keys: string[],
) => {
  const search_params = new URLSearchParams(search_param);
  const filtered_queries = [...search_params].filter((kvp) =>
    allowed_keys.includes(kvp[0]),
  );
  return new URLSearchParams(filtered_queries || "").toString();
};

export const excludeParamsFromUrlQuery = (
  search_param: string,
  excluded_keys: string[],
) => {
  const search_params = new URLSearchParams(search_param);
  const filtered_queries = [...search_params].filter(
    ([key]) => !excluded_keys.includes(key),
  );
  return filtered_queries.length
    ? `?${new URLSearchParams(filtered_queries).toString()}`
    : "";
};
