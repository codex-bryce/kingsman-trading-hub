...
export const reorderCurrencies = <
  T extends { value: string; type: string; name: string },
>(
  list: Array<T>,
  type = "fiat",
) => {
...
export const formatMoney = (
  ...
  return (
    sign + (exclude_currency ? "" : formatCurrency(currency_value)) + money
  );
};
...
export const calcDecimalPlaces = (currency: string) => {
  return isCryptocurrency(currency)
    ? getPropertyValue(CryptoConfig.get(), [currency, "fractional_digits"])
    : 2;
};
...
export const setCurrencies = (website_status: {
  currencies_config: TCurrenciesConfig;
}) => {
...
export const isCryptocurrency = (currency: string) => {
  return (
    /crypto/i.test(getPropertyValue(currencies_config, [currency, "type"])) ||
...
export const getMinWithdrawal = (currency: string) => {
  return isCryptocurrency(currency)
    ? getPropertyValue(CryptoConfig.get(), [currency, "min_withdrawal"]) ||
        0.002
    : 1;
};
...
export const getCurrencyDisplayCode = (currency = "") => {
  ...
  return (
    getPropertyValue(CryptoConfig.get(), [currency, "display_code"]) || currency
  );
};
...
export const getCurrencyName = (currency = "") =>
  currency === "USDT"
    ? "Tether Omni"
    : getPropertyValue(currencies_config, [currency, "name"]) || "";
