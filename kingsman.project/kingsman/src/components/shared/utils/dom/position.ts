...
const getMaxHeightByAligningBottom = ({
  parent_rect,
  child_height = 0,
}: TGetMaxHeightByAligning) =>
  (parent_rect?.top || 0) + (parent_rect?.height || 0) + child_height;

const getMinHeightByAligningTop = ({
  parent_rect,
  child_height = 0,
}: TGetMaxHeightByAligning) => Number(parent_rect?.top) - child_height;
...
export const getPosition = ({
  ...
  const max_height = getMaxHeightByAligningBottom({
    parent_rect,
    child_height,
  });
  ...
  const top_placement_style = {
    bottom:
      body_rect.bottom - (should_consider_parent_height ? top : bottom) + 8, // add 8px extra margin for better UX
