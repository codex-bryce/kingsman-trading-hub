...
export const getContractDurationType = (
  longcode: string,
  shortcode?: string,
): string => {
  ...
  if (extracted !== null) {
    ...
    const duration_text =
      duration_type[0].toUpperCase() + duration_type.slice(1);
...
export const getMarketName = (underlying: string) =>
  underlying
    ? getMarketNamesMap()[
        underlying.toUpperCase() as keyof typeof getMarketNamesMap
      ]
    : null;
...
export const getTradeTypeName = (
  ...
  const trade_type =
    category &&
    (getContractConfig(isHighLow)[
      category.toUpperCase() as keyof typeof getContractConfig
    ] as TTradeConfig);
