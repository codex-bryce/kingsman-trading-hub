...
export const showUnavailableLocationError = flow(
  function* (showError, is_logged_in) {
    ...
    const clients_country_text = (
      residence_list.residence_list.find(
        (obj_country) => obj_country.value === clients_country_code,
      ) || {}
    ).text;
    ...
    showError({
      ...
      redirectOnClick: () =>
        redirectToLogin(is_logged_in, getInitialLanguage()),
...
export const isMarketClosed = (
  ...
  return active_symbols.filter((x) => x.symbol === symbol)[0]
    ? !active_symbols.filter((symbol_info) => symbol_info.symbol === symbol)[0]
        .exchange_is_open
    : false;
};
...
const getFavoriteOpenSymbol = async (active_symbols: ActiveSymbols) => {
  try {
    ...
    const client_favorite_markets: string[] =
      JSON.parse(chart_favorites)["chartTitle&Comparison"];

    const client_favorite_list = client_favorite_markets.map(
      (client_fav_symbol) =>
        active_symbols.find(
          (symbol_info) => symbol_info.symbol === client_fav_symbol,
        ),
    );
    if (client_favorite_list) {
      ...
      if (client_first_open_symbol) {
        const is_symbol_offered = await isSymbolOffered(
          client_first_open_symbol.symbol,
        );
...
const getDefaultOpenSymbol = async (active_symbols: ActiveSymbols) => {
  ...
  return active_symbols.find(
    (symbol_info) => symbol_info.submarket === "major_pairs",
  )?.symbol;
};

const findSymbol = async (active_symbols: ActiveSymbols, symbol: string) => {
  const first_symbol = active_symbols.find(
    (symbol_info) => symbol_info.symbol === symbol && isSymbolOpen(symbol_info),
  );
...
const findFirstSymbol = async (
  active_symbols: ActiveSymbols,
  pattern: RegExp,
) => {
  const first_symbol = active_symbols.find(
    (symbol_info) =>
      pattern.test(symbol_info.submarket) && isSymbolOpen(symbol_info),
  );
  const is_symbol_offered = await isSymbolOffered(first_symbol?.symbol);
  if (is_symbol_offered) return first_symbol;
  return undefined;
...
export const findFirstOpenMarket = async (
  ...
  const first_symbol = active_symbols.find(
    (symbol_info) => market === symbol_info.market && isSymbolOpen(symbol_info),
  );
  const is_symbol_offered = await isSymbolOffered(first_symbol?.symbol);
  if (is_symbol_offered)
    return {
      category: first_symbol?.market,
      subcategory: first_symbol?.submarket,
    };
  else if (markets.length > 0)
    return findFirstOpenMarket(active_symbols, markets);
  return undefined;
};
