...
type TIsSoldBeforeStart = Required<
  Pick<TContractInfo, "sell_time" | "date_start">
>;
...
export const isContractElapsed = (
  contract_info: TContractInfo,
  tick?: null | TickSpotData,
) => {
  ...
  if (end_time && tick && tick.epoch) {
    const seconds = moment
      .duration(moment.unix(tick.epoch).diff(moment.unix(end_time)))
      .asSeconds();
    return seconds >= 2;
  }
  return false;
...
export const isEndedBeforeCancellationExpired = (
  contract_info: TContractInfo,
) => {
  const end_time = getEndTime(contract_info) || 0;
  return !!(
...
export const isSoldBeforeStart = (contract_info: TIsSoldBeforeStart) =>
  ...
  +contract_info.sell_time < +contract_info.date_start;
...
export const isUserCancelled = (contract_info: TContractInfo) =>
...
export const getEndTime = (contract_info: TContractInfo) => {
  ...
  const is_finished =
    ...
    (is_expired || isAccumulatorContract(contract_type));

  if (
    !is_finished &&
    !isUserSold(contract_info) &&
    !isUserCancelled(contract_info)
  )
    return undefined;
  ...
  if (isUserSold(contract_info) && sell_time) {
    ...
    !is_tick_contract &&
    sell_time &&
    sell_time > Number(date_expiry)
  ) {
    return date_expiry;
  }

  return Number(date_expiry) > Number(exit_tick_time) &&
    !Number(is_path_dependent)
    ? date_expiry
    : exit_tick_time;
};
 ...
 */
export const checkServerMaintenance = (
  website_status: WebsiteStatus | undefined | null,
) => {
  const { site_status = "" } = website_status || {};
  return site_status === "down" || site_status === "updating";
};

export const isContractSupportedAndStarted = (
  symbol: string,
  contract_info?: TContractInfo,
) =>
  !!contract_info &&
  ...
  !!getSupportedContracts()[
    contract_info?.contract_type as keyof ReturnType<
