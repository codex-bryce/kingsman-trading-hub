...
export const buildDurationConfig = (
  ...
  durations: TDurations = {
    min_max: { spot: {}, forward: {} },
    units_display: {},
  },
) => {
  type TDurationMaps = keyof typeof duration_maps;
  durations.units_display[
    contract.start_type as keyof typeof durations.units_display
  ] =
    durations.units_display[
      contract.start_type as keyof typeof durations.units_display
    ] || [];
  ...
  if (/^(?:tick|daily)$/.test(contract.expiry_type)) {
    ...
    Object.keys(duration_maps).forEach((u) => {
      if (
        ...
        duration_maps[u as TDurationMaps].order >=
          duration_maps[obj_min.unit as TDurationMaps].order &&
        duration_maps[u as TDurationMaps].order <=
          duration_maps[obj_max.unit as TDurationMaps].order
      ) {
        arr_units.push(u);
      }
    });
  ...
  durations.units_display[
    ...
    .sort((a, b) =>
      duration_maps[a as TDurationMaps].order >
      duration_maps[b as TDurationMaps].order
        ? 1
        : -1,
    )
    .reduce(
      (o, c) => [
        ...o,
        { text: duration_maps[c as TDurationMaps].display, value: c },
      ],
      [] as TUnit[],
    );
  return durations;
...
export const convertDurationUnit = (
  ...
  if (
    from_unit === to_unit ||
    duration_maps[from_unit as keyof typeof duration_maps].to_second === null
  ) {
    return value;
  }
  ...
  return (
    (value *
      (duration_maps[from_unit as keyof typeof duration_maps]?.to_second ??
        1)) /
...
export const getExpiryType = (store: any) => {
  const { duration_unit, expiry_date, expiry_type, duration_units_list } =
    store;
...
export const hasIntradayDurationUnit = (duration_units_list: TUnit[]) => {
  return duration_units_list.some(
    (unit) => ["m", "h"].indexOf(unit.value) !== -1,
  );
};
/**
 ...
 * @returns {*}
 */
export const resetEndTimeOnVolatilityIndices = (
  symbol: string,
  expiry_type: string | null,
) =>
  /^R_/.test(symbol) && expiry_type === "endtime"
    ? toMoment(null).format("DD MMM YYYY")
    : null;
...
export const getDurationMinMaxValues = (
  ...
  const max_value = convertDurationLimit(
    +duration_min_max[contract_expiry_type].max,
    duration_unit,
  );
  const min_value = convertDurationLimit(
    +duration_min_max[contract_expiry_type].min,
    duration_unit,
  );
...
const formatDisplayedTime = (time_unit: number) =>
