...
export const isBarrierSupported = (contract_type: string) =>
  contract_type in CONTRACT_SHADES;

export const barriersToString = (
  is_relative: boolean,
  ...barriers_list: number[]
) =>
  barriers_list
    .filter((barrier) => barrier !== undefined && barrier !== null)
    .map(
      (barrier) =>
        `${is_relative && !/^[+-]/.test(barrier.toString()) ? "+" : ""}${barrier}`,
    );

export const removeBarrier = (
  barriers: { [key: string]: string | number }[],
  key: string,
) => {
