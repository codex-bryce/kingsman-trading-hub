...
export const getDurationUnitValue = (obj_duration: moment.Duration) => {
  ...
  if (duration_ms >= 86400000) {
    ...
    const is_end_time = isEndTime(duration);
    return is_end_time
      ? formatMilliseconds(duration_ms, "mm[m] ss[s]")
...
export const getDurationUnitText = (
  obj_duration: moment.Duration,
  should_ignore_end_time?: boolean,
) => {
  ...
  if (duration_ms >= TIME.HOUR && duration_ms < TIME.DAY) {
    if (!should_ignore_end_time && isEndTime(duration_ms / TIME.HOUR))
      return "";
    return duration_ms === TIME.HOUR
      ? unit_map.h.name_singular
      : unit_map.h.name_plural;
  }
  if (duration_ms >= TIME.MINUTE && duration_ms < TIME.HOUR) {
    if (!should_ignore_end_time && isEndTime(duration_ms / TIME.MINUTE))
      return "";
    return duration_ms === TIME.MINUTE
      ? unit_map.m.name_singular
      : unit_map.m.name_plural;
  }
...
export const formatResetDuration = (contract_info: TContractInfo) => {
  ...
  const duration_ms =
    getDurationPeriod(contract_info).asMilliseconds() / TIME.SECOND / 2;
  const reset_hours =
    duration_ms === TIME.HOUR
      ? `h [${time_duration.h.name_singular}] `
      : `h [${time_duration.h.name_plural}] `;
  const reset_minutes =
    duration_ms === TIME.MINUTE
      ? `m [${time_duration.m.name_singular}] `
      : `m [${time_duration.m.name_plural}] `;
  const reset_seconds =
    duration_ms % TIME.MINUTE === 0 ? "" : `s [${time_duration.s.name}]`;

  return moment
    .utc(moment.duration(duration_ms, "milliseconds").asMilliseconds())
    .format(
      `${duration_ms >= TIME.HOUR ? reset_hours : ""}${
        duration_ms >= TIME.MINUTE && duration_ms % TIME.HOUR !== 0
          ? reset_minutes
          : ""
      }${reset_seconds}`.trim(),
    );
...
export const getDurationPeriod = (contract_info: TContractInfo) =>
  getDiffDuration(
    +epochToMoment(
      contract_info.date_start || contract_info.purchase_time || 0,
    ),
