...
export const filterDisabledPositions = (
  ...
  return Object.entries(
    LocalStore.getObject("FeatureFlagsStore")?.data ?? {},
  ).every(
    ([key, value]) =>
      !!value ||
      key !== getContractTypeFeatureFlag(type, isHighLow({ shortcode })),
  );
};
...
const isVerifiedOrNone = (
  ...
  return (
    ...
    (status_code === STATUS_CODES.NONE ||
      status_code === STATUS_CODES.VERIFIED) &&
    !is_high_risk
  );
};
...
const getIDVErrorStatus = (
  ...
  if (
    ...
    (status.includes(IDV_ERROR_STATUS.NameMismatch.code) ||
...
export const formatOnfidoError = (
  status_code: string,
  errors: Array<TOnfidoErrorStatus> = [],
) => {
  if (status_code === STATUS_CODES.EXPIRED) {
