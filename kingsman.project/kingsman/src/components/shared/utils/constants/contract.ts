...
export const getContractTypeDisplay = (
  ...
  const contract_config = getContractConfig(isHighLow)[
    type as TGetSupportedContracts
  ] as TContractConfig;
  if (showMainTitle) return contract_config?.main_title ?? "";
  return (
    (showButtonName && contract_config?.button_name) ||
    contract_config?.name ||
    ""
  );
};

export const getContractTypeFeatureFlag = (
  type: string,
  is_high_low = false,
) => {
  const contract_config = getContractConfig(is_high_low)[
    type as TGetSupportedContracts
  ] as TContractConfig;
...
export const isCallPut = (
  trade_type: "rise_fall" | "rise_fall_equal" | "high_low",
): boolean =>
