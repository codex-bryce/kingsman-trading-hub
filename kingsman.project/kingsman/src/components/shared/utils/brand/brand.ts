...
const isDomainAllowed = (domain_name: string) => {
  ...
  // Allowed deriv domains: localhost, pages.dev, devtunnels.ms, binary.sx, binary.com,
  // deriv.com, deriv.be, deriv.me, deriv.dev and their subdomains.
  return /^(((.*)\.)?(localhost:8444|pages\.dev|devtunnels\.ms|vercel\.app|binary\.(sx|com)|deriv\.(com|me|be|dev)))$/.test(
    domain_name,
  );
};
...
export const getAppstorePlatforms = () => {
  ...
  > = config_data.platforms_appstore;
  return Object.keys(platform_data).map((key) => platform_data[key]);
};

export const getPlatformSettingsAppstore = (
  platform_key: keyof TPlatformsAppstore,
): TPlatformAppstore => {
