import {
  getPreBuildDVRs,
  TRuleOptions,
} from "../validation/declarative-validation-rules";
...
class Validator<T extends object, S extends object> {
  ...
  addFailure(
    attribute: string,
    rule: { name: string; options: TRuleOptions<S> },
  ...
  check() {
    Object.keys(this.input).forEach((attribute) => {
      ...
      (this.rules as { [key: string]: Array<TRule<S>> })[attribute].forEach(
        (rule: TRule<S>) => {
          ...
          if (
            this.input[attribute as keyof S] === "" &&
            ruleObject.name !== "req"
          ) {
            return;
          }
