...
const TradeAnimation = observer(
  ({ className, should_show_overlay }: TTradeAnimation) => {
    ...
    const is_unavailable_for_payment_agent = cashier_validation?.includes(
      "WithdrawServiceUnavailableForPA",
    );
    ...
    const is_purchase_sent =
      contract_stage === (contract_stages.PURCHASE_SENT as unknown);
    const is_purchase_received =
      contract_stage === (contract_stages.PURCHASE_RECEIVED as unknown);

    let progress_status =
      contract_stage - (is_purchase_sent || is_purchase_received ? 2 : 3);
    ...
    const is_disabled = is_stop_button_visible
      ? false
      : shouldDisable || should_disable_run || is_running;

    // Show the tooltip when:
    // 1. The user is NOT in the bot builder tab, AND
    // 2. There are no bots
    const should_show_tooltip =
      !is_stop_button_visible && !is_bot_builder_tab && has_no_bots;
    ...
    const safeActiveTab = (
      typeof active_tab === "number" ? active_tab : 0
    ) as number;
    ...
    const determineTooltipAlignment = (): string => {
      ...
      if (isMobile) {
        ...
        try {
          const runPanelElement = document.querySelector(
            ".run__button_wrapper",
          );
          if (runPanelElement) {
        ...
        {should_show_tooltip ? (
          <div className="run__button_wrapper">
            <Tooltip
              ...
              message={localize(
                "The Run button is disabled because no Bot has been created yet.",
              )}
              icon="info"
        ...
        >
          ...
          <div className="animation__progress">
            <div className="animation__progress-line">
              <div
                className={`animation__progress-bar animation__progress-${contract_stage}`}
              />
            </div>
            {status_classes.map((status_class, i) => (
              <CircularWrapper
                key={`status_class-${status_class}-${i}`}
