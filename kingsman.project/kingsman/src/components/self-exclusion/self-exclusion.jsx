...
const SelfExclusionForm = (props) => {
  ...
  const onSubmitLimits = async (values) => {
    if (values.form_max_losses !== api_max_losses) {
      const set_losses = await updateSelfExclusion({
        max_losses: values.form_max_losses,
      });
      if (set_losses?.error) {
  ...
  const validateFields = (values) => {
    ...
    const max_losses_limit_message = localize(
    ...
    decimal_limit.forEach((item) => {
      ...
      const amount_decimal_places = amount_decimal_array
        ? amount_decimal_array.length || 0
        : 0;
                ...
                <div
                  className={classNames("db-self-exclusion__footer", {
                    "db-self-exclusion__footer--relative":
                      is_mobile && is_onscreen_keyboard_active,
                  })}
                >
