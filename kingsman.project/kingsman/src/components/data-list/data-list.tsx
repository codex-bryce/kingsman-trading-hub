...
const DataList = React.memo(
  ({
    ...
    React.useEffect(() => {
      if (is_dynamic_height) {
        cache.current = new CellMeasurerCache({
          ...
          keyMapper: (row_index) => {
            if (
              data_source_ref?.current &&
              row_index < data_source_ref?.current.length
            )
              return (
                keyMapper?.(data_source_ref.current[row_index]) || row_index
              );
            return row_index;
          },
        });
    ...
    const rowRenderer = ({ style, index, key, parent }: ListRowProps) => {
      ...
      const { action_type, shortcode, purchase_time, transaction_time, id } =
        row;
      if (
        isForwardStartingBuyTransaction(
          action_type,
        ...
        const is_sold = !!data_source?.find(
          (transaction) =>
            transaction.action_type === "sell" && transaction.id === id,
        );
        row = { ...row, is_sold };
      }
...
) as React.MemoExoticComponent<(props: TDataList) => JSX.Element> & {
