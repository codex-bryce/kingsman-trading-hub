...
const CardColumn = ({
  ...
  >
    ...
    <div className={`${PARENT_CLASS}__label`}>
...
const IconContainer = ({
  ...
  <div
    className={classNames(
      `${PARENT_CLASS}__table-cell`,
      `${PARENT_CLASS}__table-cell__icon-wrapper`,
    )}
  >
    {message && (
      <Popover
        alignment={isDbotRTL() ? "left" : "right"}
        message={message}
        zIndex="7"
      >
        {icon}
    ...
    >
          ...
          label={
            <div className={`${PARENT_CLASS}__icon-wrapper`}>
              ...
              <IconContainer
                ...
                icon={
                  <TradeTypeIcon type={transaction?.contract_type} size="md" />
                }
              />
            </div>
          }
      ...
      <div className={`${PARENT_CLASS}__card__row`}>
        <CardColumn
          title="Ref. ID"
          label={
            transaction?.display_transaction_ids?.buy ??
            transaction?.transaction_ids?.buy
          }
        />
      </div>
      <div className={`${PARENT_CLASS}__card__row`}>
      ...
      <div className={`${PARENT_CLASS}__card__row`}>
        <CardColumn
          title="Buy Price"
          label={Math.abs(transaction?.buy_price ?? 0).toFixed(2)}
        />
        <CardColumn
          title="Exit Spot"
          ...
          label={
            <div
              className={classNames({
                ...
                [`${PARENT_CLASS}__card__profit--loss`]:
