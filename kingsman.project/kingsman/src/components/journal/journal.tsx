...
import {
  TCheckedFilters,
  TFilterMessageValues,
  TJournalDataListArgs,
} from "./journal.types";
...
const Journal = observer(() => {
  ...
  const filtered_messages_length =
    Array.isArray(filtered_messages) && filtered_messages.length;
  const unfiltered_messages_length =
    Array.isArray(unfiltered_messages) && unfiltered_messages.length;
      ...
      <div className="journal__item-list">
        {filtered_messages_length ? (
          <DataList
            ...
            rowRenderer={(args: TJournalDataListArgs) => (
              <JournalItem {...args} />
            )}
