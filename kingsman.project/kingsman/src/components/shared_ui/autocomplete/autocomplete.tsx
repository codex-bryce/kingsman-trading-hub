...
import {
  ...
  matchStringByChar,
} from "@/components/shared/utils/string";
...
const getFilteredItems = (
  ...
  const sortedFiltered = filtered.sort((a, b) => {
    const indexA = (isString(a) ? a : a.text || "")
      .toLowerCase()
      .indexOf(searchTerm);
    const indexB = (isString(b) ? b : b.text || "")
      .toLowerCase()
      .indexOf(searchTerm);
...
const Autocomplete = React.memo((props: TAutocompleteProps) => {
    ...
    if (has_updating_list) {
      ...
      if (historyValue) {
        const index = new_filtered_items.findIndex(
          (object) =>
            (typeof object === "object" ? object.text : object) ===
            historyValue,
        );
  ...
  React.useEffect(() => {
    if (is_list_visible) {
      const index = filtered_items.findIndex(
        (item) =>
          (typeof item === "object" ? item.text : item) === historyValue,
      );
      setActiveIndex(index);
    }
  ...
  React.useEffect(() => {
    if (value) {
      const index = list_items.findIndex(
        (item) => (typeof item === "object" ? item.text : item) === value,
      );
      setActiveIndex(index);
    }
  }, [value, list_items]);
