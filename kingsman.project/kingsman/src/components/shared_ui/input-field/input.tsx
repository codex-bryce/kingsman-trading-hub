...
const Input = ({
  ...
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (navigator.userAgent.indexOf("Safari") !== -1 && type !== "checkbox") {
      ...
      changeValue(e, (evt) => {
        (evt as React.ChangeEvent<HTMLInputElement>).target.selectionEnd =
          cursor; // reset the cursor position in callback
      });
    } else {
      changeValue(e);
    }
    ...
    <div className={classNameDynamicSuffix}>
      {inline_prefix ? (
        <div className={classNameInlinePrefix}>
          ...
          >
            {inline_prefix === "UST"
              ? getCurrencyDisplayCode(inline_prefix)
              : inline_prefix}
          </Text>
        </div>
      ) : null}
      <input
        ...
        id={id}
        maxLength={
          fractional_digits && max_length
            ? max_length + fractional_digits + 1
            : max_length
        }
        name={name}
