...
const InputField = ({
  ...
  const has_error =
    error_messages && !!error_messages.length && !is_error_tooltip_hidden;
  const max_is_disabled =
    max_value && (+value >= +max_value || Number(local_value) >= +max_value);
  const min_is_disabled =
    min_value && (+value <= +min_value || Number(local_value) <= +min_value);
  ...
  const changeValue = (
    ...
    if (type === "number" || type === "tel") {
      ...
      const is_number = new RegExp(
        `${signed_regex}(\\d*)?${is_float ? "(\\.\\d+)?" : ""}$`,
      ).test(e.target.value);

      const is_not_completed_number =
        is_float &&
        new RegExp(`${signed_regex}(\\.|\\d+\\.)?$`).test(e.target.value);
      // This regex check whether there is any zero at the end of fractional part or not.
      const has_zero_at_end = new RegExp(
        `${signed_regex}(\\d+)?\\.(\\d+)?[0]+$`,
      ).test(e.target.value);
  ...
  const getDecimals = (val: string | number) => {
    const array_value =
      typeof val === "string" ? val.split(".") : val.toString().split(".");
  ...
  const incrementValue = (
    ...
    if (long_press_step) {
      const increase_percentage =
        Math.min(long_press_step, Math.max(long_press_step, 10)) / 10;
      const increase = (+value * increase_percentage) / 100;
      const new_value = parseFloat(current_value || "0") + Math.abs(increase);

      increment_value = parseFloat(
        getClampedValue(new_value).toString(),
      ).toFixed(decimal_places);
    } else if (is_crypto || (!currency && is_float)) {
      const new_value =
        parseFloat(current_value || "0") +
        parseFloat(
          (1 * 10 ** (0 - (decimal_point_change || decimal_places))).toString(),
        );
      increment_value = parseFloat(new_value.toString()).toFixed(
        decimal_point_change || decimal_places,
      );
    } else {
      increment_value = parseFloat(
        ((+current_value || 0) + 1).toString(),
      ).toFixed(decimal_places);
    }
  ...
  const calculateDecrementedValue = (long_press_step?: number) => {
    ...
    if (long_press_step) {
      ...
      decrement_value = parseFloat(new_value.toString()).toFixed(
        decimal_point_change || decimal_places,
      );
    } else {
      decrement_value = parseFloat(
  ...
  const is_increment_input =
    is_incrementable && (type === "number" || type === "tel");
  ...
  const input_tooltip = (
    <Tooltip
      className={classNames("trade-container__tooltip", {
        "dc-tooltip--with-label": label,
      })}
  ...
  return (
    <React.Fragment>
      ...
      <div
        className={classNames("dc-input-field", className)}
