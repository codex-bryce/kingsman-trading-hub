...
const SelectNativeOptions = ({
  ...
  use_text,
}: TSelectNativeOptions) => {
  const options = should_hide_disabled_options
    ? list_items.filter((opt) => !opt.disabled)
    : list_items;
  const has_group = Array.isArray(list_items) && !!list_items[0]?.group;

  if (has_group) {
    const dropdown_items = options.reduce(
      (dropdown_map: { [key: string]: Array<TListItem> }, item) => {
        if (item.group) {
        ...
        {group_names.map((option) => (
          <optgroup key={option} label={option}>
            {dropdown_items[option].map((value: TListItem) => (
              <option
                key={value.value}
                value={use_text ? value.text : value.value}
              >
