...
const ModalElement = ({
  ...
  const modal_root_ref = React.useRef(
    el_portal_node || document.getElementById(portalId),
  );
  ...
  const isPortalElementVisible = () =>
    modal_root_ref.current?.querySelectorAll(
      portal_elements_selector.join(", "),
    ).length;

  const validateClickOutside = (e: IClickEvent): boolean => {
    const is_absolute_modal_visible = document
      .getElementById("popup_root")
      ?.hasChildNodes();
    ...
    return (
      ...
      (has_close_icon &&
        ...
        !(
          elements_to_ignore &&
          path?.find((el) => elements_to_ignore.includes(el as HTMLElement))
        ))
    );
  };
