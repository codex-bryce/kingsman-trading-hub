...
const ContractCardBody = ({
  ...
  const { CONTRACT_VALUE, POTENTIAL_PAYOUT, TOTAL_PROFIT_LOSS, STAKE } =
    getCardLabels();
  ...
  const toggle_card_dialog_props = {
    ...
    setCurrentFocus,
    totalProfit:
      is_multiplier && !isNaN(Number(profit))
        ...
        <div className="dc-contract-card-items-wrapper">
          ...
          <ContractCardItem header={CONTRACT_VALUE}>
            ...
            >
              <Money
                currency={currency}
                amount={Number(sell_price || indicative)}
              />
            </div>
            {!is_sold && (
              <ArrowIndicator
        ...
        <div
          className={classNames("dc-contract-card__separatorclass", {
            "dc-contract-card__body-wrapper":
              !is_multiplier && !is_turbos && !is_lookbacks,
          })}
        >
          {card_body}
        </div>
