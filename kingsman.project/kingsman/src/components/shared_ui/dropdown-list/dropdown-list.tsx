...
const ListItems = React.forwardRef<HTMLDivElement, TListItems>((props, ref) => {
  ...
  const is_grouped_list = list_items?.some(
    (list_item) => typeof list_item === "object" && !!list_item.group,
  );
  ...
  return (
    <>
      {list_items?.length ? (
        ...
        <div className="dc-dropdown-list__item dc-dropdown-list__item--not-found">
          {not_found_text}
        </div>
      )}
    </>
...
const DropdownList = (props: TDropDownList) => {
  const { dropdown_ref, list_item_ref, list_wrapper_ref } =
    props.dropdown_refs || {};
  const {
    active_index,
    is_visible,
  ...
  if (
    list_items?.length &&
    typeof list_items[0] !== "string" &&
    typeof list_items[0] !== "object"
  ) {
    throw Error("Dropdown received wrong data structure");
  }

  const is_object =
    !Array.isArray(list_items) && typeof list_items === "object";
  const is_string_array =
    list_items?.length && typeof list_items[0] === "string";
  ...
  const el_dropdown_list = (
    ...
    >
      <div style={style} className="dc-dropdown-list" ref={list_wrapper_ref}>
        <ThemedScrollbars
          height={list_height || "220px"}
          refSetter={dropdown_ref}
