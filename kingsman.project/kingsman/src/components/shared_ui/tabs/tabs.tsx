...
const Tabs = ({
  ...
  const pushHash = (hash: string) => {
    history.replace(
      `${history.location.pathname}${window.location.search}#${hash}`,
    );
  };

  const setActiveLineStyle = React.useCallback(() => {
    const tabs_wrapper_bounds =
      tabs_wrapper_ref?.current?.getBoundingClientRect();
  ...
  useConstructor(() => {
    ...
    if (should_update_hash) {
      ...
      const hash_index = children.findIndex(
        (child) => child && child.props && child.props.hash === hash,
      );
  ...
  if (is_scrollable) {
    ...
    tab_width = fit_content
      ? "150px"
      : `${(100 / valid_children.length).toFixed(2)}%`;
  }
    ...
    >
      <div
        className={classNames({
          [`dc-tabs__list--header--${className}`]: className,
        })}
      >
        <ul
            ...
            {React.Children.map(children, (child, index) => {
              ...
              return (
                <Tab
                  ...
                  active_tab_ref={
                    index === active_tab_index ? active_tab_ref : null
                  }
            ...
            {has_active_line && !is_scrollable && (
              <span
                className={classNames("dc-tabs__active-line", {
                  ...
                  "dc-tabs__active-line--header-fit-content":
                    header_fit_content,
