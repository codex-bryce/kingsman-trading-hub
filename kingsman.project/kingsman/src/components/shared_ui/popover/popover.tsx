...
const Popover = ({
  ...
  const [popover_ref, setPopoverRef] = React.useState<
  ...
  const is_open_on_focus =
    is_hovered &&
    message &&
    (!should_toggle_on_target_tap || is_bubble_visible);
      ...
      {(popover_ref || !relative_render) && (
        <TinyPopover
          isOpen={
            is_open ??
            ((is_open_on_focus ||
              (is_bubble_hover_enabled && is_bubble_hovered)) as boolean)
          }
          positions={[alignment]}
          padding={margin + 8}
          ...
          {...(relative_render
            ? {
                ...
                contentLocation: ({ childRect, popoverRect, nudgedLeft }) => {
                  ...
                  const total_width =
                    childRect.right + (popoverRect.width - childRect.width / 2);
                  let top_offset = 0;
                  let left_offset = 0;
            ...
            <div className={classNames(classNameTarget, "dc-popover__target")}>
              {!disable_target_icon && (
                ...
                >
                  ...
                  {icon === "question" && (
                    <LegacyWarningIcon className={icon_class_name} />
                  )}
                  {icon === "dot" && (
                    <LabelPairedCircleDotCaptionFillIcon
                      className={icon_class_name}
                  ...
                  {icon === "counter" && (
                    <span className={icon_class_name}>{counter}</span>
                  )}
                </i>
              )}
