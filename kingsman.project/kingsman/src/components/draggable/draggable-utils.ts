...
export const calculateZindex = ({ setZIndex }: TCalculateZindex) => {
  ...
  const maxZIndex = Array.from(draggableModals).reduce(
    (maxZ, modal) =>
      Math.max(maxZ, parseInt(window.getComputedStyle(modal).zIndex) || 0),
    0,
  );
...
export const calculateWidth = ({
  ...
  const rightLimit =
    leftBoundary +
    boundaryWidth -
    (SAFETY_MARGIN + EXTRA_BOTTOM_RIGHT_SAFETY_MARGIN);
  const calculatedPreviousWidth =
    rightLimit - leftLimit - (rightLimit - initialSelfRight);

  if (resize_direction.includes(DRAGGABLE_CONSTANTS.LEFT)) {
    if (newWidth >= minWidth && clientX > leftLimit) return newWidth;
    if (clientX < leftLimit + SAFETY_MARGIN * 2)
      return calculatedPreviousWidth - SAFETY_MARGIN * 2;
    return prevWidth;
  }
...
export const calculateHeight = ({
  ...
  const bottomLimit =
    ...
    boundaryHeight -
    (SAFETY_MARGIN + EXTRA_BOTTOM_RIGHT_SAFETY_MARGIN);
  const calculatedPreviousHeight =
    bottomLimit - topLimit - (bottomLimit - initialSelfBottom);

  if (resize_direction.includes(DRAGGABLE_CONSTANTS.TOP)) {
    if (newHeight >= minHeight && clientY > topLimit) return newHeight;
    if (clientY < topLimit + SAFETY_MARGIN * 2)
      return calculatedPreviousHeight - SAFETY_MARGIN * 2;
    return prevHeight;
  }
