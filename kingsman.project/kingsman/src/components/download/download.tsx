...
const Download = observer(({ tab }: TDownloadProps) => {
  ...
  const downloadTransaction = () => {
    ...
    transaction_list.forEach(({ data }: { data: TTransaction }) => {
      ...
      items.push([
        data.display_name,
        data.display_transaction_ids?.buy ?? data.transaction_ids?.buy ?? "",
        data.display_transaction_ids?.sell ?? data.transaction_ids?.sell ?? "",
        data.barrier,
        data.date_start,
        data.entry_tick,
  ...
  const downloadJournal = () => {
    ...
    filtered_messages.map((item) => {
      ...
      if (item.message_type !== "success") {
        ...
        array_message = getSuccessJournalMessage(
  ...
  if (tab === "transactions") {
    ...
    if (!transaction_list.length)
      popover_message = localize("No transaction or activity yet.");
  } else if (tab === "journal") {
