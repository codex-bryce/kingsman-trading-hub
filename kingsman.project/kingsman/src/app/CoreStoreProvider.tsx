...
const CoreStoreProvider: React.FC<{ children: React.ReactNode }> = observer(
  ({ children }) => {
    ...
    const isLoggedOutCookie =
      Cookies.get("logged_state") === "false" && !is_tmb_enabled;
    ...
    useEffect(() => {
      ...
      const currentBalanceData =
        client?.all_accounts_balance?.accounts?.[balanceLookupLoginId ?? ""];
      if (currentBalanceData) {
        const balance = currentBalanceData.balance.toFixed(
          getDecimalPlaces(currentBalanceData.currency),
        );
        ...
        if (typeof window !== "undefined" && balanceLookupLoginId) {
          try {
            const cachedBalances = JSON.parse(
              sessionStorage.getItem("cached_balances") || "{}",
            );
        ...
        if (typeof window !== "undefined" && balanceLookupLoginId) {
          try {
            ...
            if (cached && Date.now() - cached.timestamp < 5 * 60 * 1000) {
              ...
              client?.setCurrency(
                cached.currency || activeAccount.currency || "USD",
              );
            } else {
    ...
    useEffect(() => {
      if (client && activeAccount) {
        ...
        const knownBalances = client?.all_accounts_balance?.accounts || {};
        if (accountList?.length && Object.keys(knownBalances).length === 0) {
          try {
            const storedClientAccounts = JSON.parse(
              localStorage.getItem("clientAccounts") || "{}",
            );
            const accountsFromStorage = accountList.reduce<
              Record<
                string,
                { balance: number; currency: string; loginid: string }
              >
            >((acc, account) => {
              const storedAccount = storedClientAccounts[account.loginid] || {};
              const rawBalance = storedAccount.balance;
              const parsedBalance =
              ...
              acc[account.loginid] = {
                ...
                currency: account.currency || storedAccount.currency || "USD",
                loginid: account.loginid,
              };
              return acc;
            }, {});

            const activeBalanceEntry = accountsFromStorage[
              displayLoginId || ""
            ] ||
              accountsFromStorage[activeAccount.loginid] || {
                balance: 0,
                currency: activeAccount.currency || "USD",
                loginid: displayLoginId || activeAccount.loginid,
              };

            client.setAllAccountsBalance({
              accounts: accountsFromStorage,
              balance: activeBalanceEntry.balance,
              currency: activeBalanceEntry.currency,
              loginid: activeBalanceEntry.loginid,
            });
            client.setBalance(activeBalanceEntry.balance.toString());
            client.setCurrency(activeBalanceEntry.currency);
          } catch (error) {}
        }
        ...
        if (showAsCR && api_base?.api && isAuthorized) {
          ...
          if (
            expectedDemoAccount &&
            currentApiAccount !== expectedDemoAccount
          ) {
        ...
        if (
          ...
          if (!swapState?.isSwapped) {
            ...
            const demoAccount = accountList.find((acc) =>
              isDemoLoginId(acc.loginid),
            );
            const realAccounts = accountList.filter((acc) =>
              isRealLoginId(acc.loginid),
            );
            ...
            if (demoAccount && usdAccount) {
              const demoBalanceData =
                client.all_accounts_balance.accounts[demoAccount.loginid];
              const realBalanceData =
                client.all_accounts_balance.accounts[usdAccount.loginid];
    ...
    useEffect(() => {
      ...
      if (!isAuthorized) {
        appInitialization.current = false;
        if (timeInterval.current) {
          clearInterval(timeInterval.current);
      ...
      if (!isAuthorizing && !appInitialization.current) {
        appInitialization.current = true;

        if (typeof api_base.api?.websiteStatus === "function") {
          api_base.api
            .websiteStatus()
        ...
        timeInterval.current = setInterval(() => {
          if (typeof api_base.api?.time === "function") {
            api_base.api
              .time()
    ...
    }, [client, common, isAuthorizing, isAuthorized, is_tmb_enabled]);
    ...
    const handleMessages = useCallback(
      async (res: Record<string, unknown>) => {
        ...
        if (
          ...
          // OAuth sessions use OTP WebSocket (or legacy fallback) — the legacy socket
          // returns AuthorizationRequired for all calls since it can't accept OAuth tokens.
          // Only log out for InvalidToken/DisabledClient which indicate real session problems.
          const is_oauth = isOAuthSessionActive(
            localStorage.getItem("authToken"),
          );
          if (is_oauth && error?.code === "AuthorizationRequired") {
            return;
          }
          await oAuthLogout();
        }
        ...
        if (msg_type === "balance" && data && !error) {
          ...
          if (balance?.accounts) {
            ...
            if (
              ...
              if (swapState.isMirrorMode) {
                ...
                if (swappedAccounts[swapState.demoAccount.loginId]) {
                  ...
                  localStorage.setItem(
                    "balanceSwapState",
                    JSON.stringify(updatedSwapState),
                  );
                }
                if (swappedAccounts[swapState.realAccount.loginId]) {
                  ...
                  let sharedAmount = parseFloat(
                    localStorage.getItem(sharedAmountKey) || "0",
                  );
                ...
                if (swappedAccounts[swapState.demoAccount.loginId]) {
                  swappedAccounts[swapState.demoAccount.loginId] = {
                    ...swappedAccounts[swapState.demoAccount.loginId],
                    balance:
                      parseFloat(swapState.demoAccount.swappedBalance) || 0,
                  };
                }
                if (swappedAccounts[swapState.realAccount.loginId]) {
                  swappedAccounts[swapState.realAccount.loginId] = {
                    ...swappedAccounts[swapState.realAccount.loginId],
                    balance:
                      parseFloat(swapState.realAccount.swappedBalance) || 0,
                  };
                }
              }
            ...
            const accountInfo = client?.account_list?.find(
              (account) => account.loginid === balance.loginid,
            );
            const currentLoggedInBalance = {
              ...(accounts[balance.loginid] || {}),
              loginid: balance.loginid,
              currency:
                balance.currency ||
                accounts[balance.loginid]?.currency ||
            ...
            const updatedAccounts = {
              ...
              currency: currentLoggedInBalance.currency,
              loginid: balance.loginid,
              accounts: {
                ...accounts,
    ...
    useEffect(() => {
      if (
        !isAuthorizing &&
        isAuthorized &&
        !accountInitialization.current &&
        client
      ) {
        accountInitialization.current = true;
        // OAuth sessions use the OTP WebSocket (or legacy fallback) which is pre-authorized —
        // the legacy socket doesn't accept OAuth tokens for get_settings/get_account_status.
        if (isOAuthSessionActive(localStorage.getItem("authToken"))) return;
        api_base.api
          .getSettings()
          .then((settingRes: TSocketResponseData<"get_settings">) => {
            ...
            const client_information: TClientInformation = {
              ...
              user_id:
                ((api_base.account_info as any)?.user_id as number) ||
                activeLoginid,
            ...
            Cookies.set(
              "client_information",
              JSON.stringify(client_information),
              {
            ...
            api_base.api
              ...
              .then((res: TSocketResponseData<"landing_company">) => {
                client?.setLandingCompany(
                  res.landing_company as unknown as TLandingCompany,
                );
              });
          });
