...
const SpeedBot = observer(() => {
  ...
  const isSharedApiRef = useRef<boolean>(false);
  ...
  const [symbols, setSymbols] = useState<
    Array<{ symbol: string; display_name: string }>
  >([]);
  ...
  // Martingale/recovery
  const [martingaleMultiplier, setMartingaleMultiplier] =
    useState<string>("1.0");
  ...
  const getHintClass = (d: number) => {
    ...
    if (tradeType === "DIGITOVER" || tradeType === "DIGITUNDER") {
      const activePred = lastOutcomeWasLossRef.current
        ? ouPredPostLoss
        : ouPredPreLoss;
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        const { api, is_shared } = await getStandaloneOAuthAwareApi("speedbot");
        apiRef.current = api;
        isSharedApiRef.current = is_shared;

        // Fetch active symbols (volatility indices)
        const { active_symbols, error: asErr } = await api.send({
          active_symbols: "brief",
        });
        if (asErr) throw asErr;
        const syn = (active_symbols || [])
          .filter(
            (s: any) => /synthetic/i.test(s.market) || /^R_/.test(s.symbol),
          )
          .map((s: any) => ({
            symbol: s.symbol,
    ...
    try {
      ...
      const onMsg = (evt: MessageEvent) => {
        try {
          ...
          if (
            data?.forget?.id &&
            data?.forget?.id === tickStreamIdRef.current
          ) {
            // stopped
          }
        } catch {}
  ...
  const purchaseOnce = async () => {
    ...
    if (tradeType === "DIGITOVER" || tradeType === "DIGITUNDER") {
      trade_option.prediction = Number(
        lastOutcomeWasLossRef.current ? ouPredPostLoss : ouPredPreLoss,
      );
    ...
    if (error) throw error;
    setStatus(
      `Purchased: ${buy?.longcode || "Contract"} (ID: ${buy?.contract_id})`,
    );
    return buy;
  };
    ...
    try {
      ...
      while (!stopFlagRef.current) {
        ...
        const effectiveStake =
          step > 0
            ? Number((baseStake * Math.pow(martingaleNum, step)).toFixed(2))
            : baseStake;
          ...
          const onMsg = (evt: MessageEvent) => {
            try {
              ...
              if (data?.msg_type === "proposal_open_contract") {
                ...
                if (!pocSubId && data?.subscription?.id)
                  pocSubId = data.subscription.id;
        ...
        <div className="speedbot__card">
          ...
          <div className="speedbot__footer-bar">
            <div className="speedbot__footer-item">
              ...
              {Number(store?.summary_card?.profit || 0).toFixed(2)}
            </div>
            <div className="speedbot__footer-item">
              {localize("Last Digit:")} {lastDigit ?? "-"}
            </div>
            <div className="speedbot__footer-item">
              {localize("Consecutive Wins:")} {consecWins}{" "}
              {localize("Consecutive Losses:")} {consecLosses}
            </div>
          </div>

          <div className="speedbot__cta">
            <button
              className="speedbot__cta-once"
              onClick={() => onRun()}
              disabled={!symbol}
            >
              {localize("Trade once")}
          ...
          {status && (
            <div className="speedbot__status">
              <Text
                size="xs"
                color={/error|fail/i.test(status) ? "loss-danger" : "prominent"}
              >
                {status}
              </Text>
            </div>
