...
const QuickStrategyForm = observer(() => {
  ...
  const [isEnabledToggleSwitch, setIsEnabledToggleSwitch] = React.useState(
    values?.boolean_max_stake ?? false,
  );
            ...
            switch (field.type) {
              ...
              case "number": {
                ...
                if (should_have?.length) {
                  if (
                    !should_enable &&
                    (!isDesktop || hide_without_should_have)
                  ) {
                    return null;
                  }
                  return (
              ...
              case "label": {
                ...
                const { should_have = [], hide_without_should_have = false } =
                  field;
                const should_enable = shouldEnable(should_have);
