...
const GrowthRateSelect: React.FC<TContractTypes> = observer(({ name }) => {
  ...
  const { setFieldValue, values, setFieldError, errors } =
    useFormikContext<TFormData>();
  ...
  React.useEffect(() => {
    if (values.boolean_tick_count) {
      ...
      setFieldError(
        "take_profit",
        prev_error.current?.take_profit ?? undefined,
      );
  ...
  const validateMinMaxForAccumulators = async (values: TFormData) => {
    ...
    prev_proposal_payload.current = {
      ...request_proposal,
      boolean_tick_count: values.boolean_tick_count,
    };
    try {
      const response = await requestProposalForQS(
        request_proposal,
        api_base.api,
      );
      const min_ticks = 1;
      ...
      ref_max_payout.current =
        response?.proposal?.validation_params?.max_payout;
      ...
      let error_message =
        error_response?.message ?? error_response?.error?.message;
      ...
      if (values.boolean_tick_count) {
        ...
        if (error_response?.error?.details?.field === "stake") {
          ...
          const min_stake_match = error_response?.error?.message.match(
            /minimum stake of (\d+\.\d+)/i,
          );
          const max_payout_match = error_response?.error?.message.match(
            /maximum payout of (\d+\.\d+)/i,
          );

          if (min_stake_match && max_payout_match) {
            const min_stake = min_stake_match[1];
            const max_payout = max_payout_match[1];
            const current_payout =
              Number(values.take_profit) + Number(values.stake);
            ...
            const max_stake =
              quick_strategy?.additional_data?.max_stake || "1000";
