...
const DurationUnit: React.FC<TDurationUnit> = ({ attached }: TDurationUnit) => {
  ...
  const { setValue, setCurrentDurationMinMax, current_duration_min_max } =
    quick_strategy;
  ...
  React.useEffect(() => {
    if (tradetype && symbol) {
      const getDurationUnits = async () => {
        ...
        const durations = await contracts_for?.getDurations?.(
          symbol,
          tradetype,
        );
        ...
        const has_selected = duration_units?.some(
          (duration) => duration.value === selected,
        );
        if (
          !has_selected ||
          prevSymbol !== symbol ||
          prevTradeType !== tradetype
        ) {
          ...
          const duration = duration_units?.find(
            (duration: TDurationUnitItem) => duration.value === selected,
          );
