...
const TradeTypeOption: React.FC<TTradeTypeOption> = ({
  trade_type: { value, icon, text },
}: TTradeTypeOption) => {
  return (
...
const TradeTypeSelect: React.FC = () => {
  ...
  React.useEffect(() => {
    if (values?.symbol) {
      ...
      const { contracts_for } =
        (ApiHelpers?.instance as unknown as TApiHelpersInstance) ?? {};
      const getTradeTypes = async () => {
        const trade_types =
          await contracts_for?.getTradeTypesForQuickStrategy?.(
            values?.symbol,
            is_symbol_accumulator,
          );
        const has_selected = trade_types?.some(
          (trade_type) => trade_type.value === selected,
        );
  ...
  const trade_type_dropdown_options = React.useMemo(
    () =>
      trade_types.map((trade_type) => ({
        component: (
          <TradeTypeOption key={trade_type.text} trade_type={trade_type} />
        ),
        ...trade_type,
      })),
    [trade_types],
    ...
    <div className="qs__form__field qs__form__field__input qs__form__field__group-icons">
      <Field name="tradetype" key="tradetype" id="tradetype">
        {({ field }: FieldProps) => {
          ...
          const is_accumulator = is_strategy_accumulator
            ? "Buy"
            : selected_trade_type?.text;
          return (
            <Autocomplete
              {...field}
              readOnly
              ...
              leading_icon={
                <Text>
                  <TradeTypeIcon
                    type={selected_trade_type?.icon?.[0] || "CALL"}
                    size="sm"
                  />
                  <TradeTypeIcon
                    type={selected_trade_type?.icon?.[1] || "PUT"}
                    size="sm"
                  />
                </Text>
              }
