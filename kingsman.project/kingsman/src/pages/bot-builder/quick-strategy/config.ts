  ...
  validation: [
    ...
    "ceil",
    {
      ...
      getMessage: (min: string | number) =>
        localize("Minimum take profit allowed is {{ min }}", { min }),
    },
  ],
});
...
const TICK_COUNT = (): TConfigItem => ({
  ...
  validation: [
    ...
    "ceil",
    {
      ...
      value: 1,
      getMessage: (min: string | number) =>
        localize("Minimum tick count allowed is {{ min }}", { min }),
    },
  ],
});
...
const NUMBER_DEFAULT_VALIDATION = (): TValidationItem => ({
  ...
  value: 1,
  getMessage: (min: string | number) =>
    localize("Must be a number higher than {{ min }}", {
      min: Number(min) - 1,
    }),
});
...
const STAKE = (): TConfigItem => ({
  ...
  validation: [
    ...
    "ceil",
    {
      ...
      value: 0.35,
      getMessage: (min: string | number) =>
        localize("Minimum stake allowed is {{ min }}", { min }),
      getDynamicValue: (store: any) =>
        store.quick_strategy?.additional_data?.min_stake || 0.35,
    },
    {
      type: "max",
      value: 1000,
      getMessage: (max: string | number) =>
        localize("Maximum stake allowed is {{ max }}", { max }),
      getDynamicValue: (store: any) =>
        store.quick_strategy?.additional_data?.max_stake || 1000,
    },
  ],
  has_currency_unit: true,
});
...
const LABEL_LAST_DIGIT_PREDICTION = (): TConfigItem => ({
  ...
  should_have: [
    { key: "tradetype", value: "", multiple: ["matchesdiffers", "overunder"] },
  ],
  hide_without_should_have: true,
});
