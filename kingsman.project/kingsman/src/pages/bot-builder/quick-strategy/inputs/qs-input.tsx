...
export const MaxStakeErrorProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
...
const QSInput: React.FC<TQSInput> = observer(
  ({
    ...
    const [error_message, setErrorMessage] = React.useState<string | null>(
      null,
    );
    ...
    useEffect(() => {
      ...
      if (name === "max_stake" && values.stake && values.max_stake) {
        ...
        if (initial_stake > max_stake) {
          ...
          const max_stake_input = document.querySelector(
            'input[name="max_stake"]',
          );
          if (max_stake_input) {
      ...
      if (name === "stake" && values.stake && values.max_stake) {
        ...
        if (initial_stake > max_stake) {
          ...
          if (max_stake_input) {
            ...
            const max_stake_component = document.querySelector(
              '[data-testid="max_stake-popover"]',
            );
            if (max_stake_component) {
              ...
              const messageElement = max_stake_component.querySelector(
                ".qs__warning-bubble div",
              );
              if (messageElement) {
              ...
              const popoverElement = max_stake_component.querySelector(
                ".qs__warning-bubble",
              );
              if (popoverElement) {
            ...
            const allMaxStakeInputs = document.querySelectorAll(
              'input[name="max_stake"]',
            );
                ...
                >
                  <Input
                    ...
                    className={classNames(
                      ...
                      {
                        highlight:
                          loss_threshold_warning_data?.highlight_field?.includes(
                            name,
                          ),
                      },
                    )}
                    ...
                    onBlur={(e) => {
                      ...
                      if (name === "stake" || name === "max_stake") {
                        const min_stake =
                          ...
                          0.35;
                        const max_stake =
                          (quick_strategy?.additional_data as any)?.max_stake ||
                          1000;
                        ...
                        if (value === "" || value === "0" || value === "0.") {
                          setErrorMessage(
                            `Minimum stake allowed is ${min_stake}`,
                          );
                        } else {
                          ...
                          if (numValue < min_stake) {
                            ...
                            setErrorMessage(
                              `Maximum stake allowed is ${max_stake}`,
                            );
                          } else {
                            setErrorMessage(null);
                          }
                        ...
                        if (name === "stake") {
                          ...
                          if (
                            initial_stake_value &&
                            initialStakeFixed > numValueFixed
                          ) {
                            // If initial stake is greater than max stake, show error
                            setErrorMessage(
                              `Initial stake cannot be greater than max stake`,
                    ...
                    onKeyUp={(e) => {
                      ...
                      if (name === "stake" || name === "max_stake") {
                        ...
                        if (name === "stake") {
                          // When initial stake changes, validate against max_stake
                          const max_stake_value = values.max_stake;
                          const numValueFixed = parseFloat(
