...
const getErrorMessage = (
  ...
  const errors: { [key: string]: { MIN: string; MAX: string } } = {
    ...
    DEFAULT: {
      MIN: localize("The value must be equal or greater than {{ value }}", {
        value,
      }),
      MAX: localize("The value must be equal or less than {{ value }}", {
        value,
      }),
    },
  };
...
const FormikWrapper: React.FC<TFormikWrapper> = observer(({ children }) => {
  ...
  const { selected_strategy, form_data, initializeLossThresholdWarningData } =
    quick_strategy;
  ...
  const getInitialValue = () => {
    ...
    initial_value = {
      ...
      type: data?.type ?? "",
      durationtype:
        data?.durationtype ?? qs_config().QUICK_STRATEGY.DEFAULT.durationtype,
      ...
      group.forEach((field) => {
        if (field?.validation?.length && field?.name) {
          if (field.validation.includes("number")) {
            let schema: Record<string, any> = Yup.number().typeError(
              localize("Must be a number"),
            );
            let min = 0;
            let max = 10;
                ...
                if (typeof validation === "string") {
                  switch (validation) {
                    case "required":
                      schema = schema.required(
                        localize("Field cannot be empty"),
                      );
                      break;
                    case "min":
                  ...
                  if (validation?.type) {
                    ...
                    schema = schema[validation.type](
                      value,
                      localize(validation.getMessage(value)),
                    );
                  }
                }
              });
...
const QuickStrategy = observer(() => {
  ...
  const { is_open, setFormVisibility, form_data, selected_strategy } =
    quick_strategy;
  ...
  const sendRudderStackQsFormCloseData = () => {
    const active_tab =
      active_tab_ref.current
        ?.querySelector(".active")
        ?.textContent?.toLowerCase() === "learn more"
        ? "learn more"
        : "trade parameters";
