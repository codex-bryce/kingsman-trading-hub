...
const FormWrapper = observer(
  ({
    ...
    const { submitForm, isValid, setFieldValue, validateForm, values } =
      useFormikContext<TFormValues>();
    const { quick_strategy } = useStore();
    const { selected_strategy, onSubmit, is_stop_bot_dialog_open } =
      quick_strategy;
    const { handleSubmit } = useQsSubmitHandler();

    const selected_startegy_label =
      STRATEGIES()[selected_strategy as keyof typeof STRATEGIES].label;
