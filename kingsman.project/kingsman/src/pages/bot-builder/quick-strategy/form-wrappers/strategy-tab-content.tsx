...
const StrategyTabContent: React.FC<TStrategyDescription> = observer(
  ({ formfields, active_tab, tutorial_selected_strategy }) => {
    ...
    const expanded_subtitles_storage_default: TExpandedSubtitlesStorageDefault =
      {};
    const [expanded_subtitles_storage, setExpandedSubtitlesStorage] = useState(
    ...
    const strategy =
      STRATEGIES()[
        tutorial_selected_strategy ||
          (selected_strategy as keyof typeof STRATEGIES)
      ];

    const makeGroupedObjectsByTitle = () => {
      return strategy?.description?.reduce(
        (acc: TDescription, obj: TDescriptionItem, idx) => {
          const is_subtitle =
            obj.type === "subtitle_italic" || obj.type === "subtitle";
          if (is_subtitle) {
            acc.push([]);

            const generateStorageKey = (
              obj: TDataGroupedObjectsByTitle,
              selected_strategy: string,
            ): string => {
              return `${obj.content[0]}__${selected_strategy}`
                .split(" ")
                .join("_")
                .toLowerCase();
            };
            ...
            expanded_subtitles_storage_default[
              generateStorageKey(
                obj as TDataGroupedObjectsByTitle,
                selected_strategy,
              )
            ] = obj?.expanded ?? false;
          }
    ...
    const grouped_objects_by_title: TDescription = Array.isArray(
