...
const useQsSubmitHandler = () => {
  ...
  const { submitForm, setFieldValue, values, isValid, validateForm } =
  ...
  const handleSubmit = async () => {
    ...
    const stored_dont_show_warning_value = localStorage?.getItem(
      "qs-dont-show-loss-threshold-warning",
    );
    const dont_show_warning = JSON.parse(
      stored_dont_show_warning_value ?? "false",
    );
    if (
      !loss_threshold_warning_data.already_shown &&
      (loss_amount > 0.5 * Number(balance ?? 0) ||
        loss_amount > 2 * profit_threshold) &&
      is_logged_in &&
      !dont_show_warning
    ) {
      setLossThresholdWarningData({
