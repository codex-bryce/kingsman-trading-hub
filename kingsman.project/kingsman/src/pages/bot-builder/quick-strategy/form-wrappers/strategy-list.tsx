...
const StrategyList = ({
  ...
  STRATEGY_TRADE_ASSOCIATIONS.filter(
    ...
    if (is_searching && search_value !== "") {
      if (
        ...
        tmp_item.parent =
          matched_parents.length > 0 ? matched_parents : item.parent;
      }
    } else {
      tmp_item.parent = [...item.parent];
    }
  ...
  const options = result.filter((item) =>
    item.parent.includes(TRADE_TYPES[TRADE_TYPE_INDEX.OPTIONS]),
  );
  const multiplier = result.filter((item) =>
    item.parent.includes(TRADE_TYPES[TRADE_TYPE_INDEX.MULTIPLIERS]),
  );
  const accumulator = result.filter((item) =>
    item.parent.includes(TRADE_TYPES[TRADE_TYPE_INDEX.ACCUMULATORS]),
  );
  ...
  const should_render = (type: string) =>
    selected_chip_value === localize("All") || selected_chip_value === type;

  const filtered_strategies = strategy_types.filter(({ type }) =>
    should_render(type),
  );
  const has_visible_strategies = filtered_strategies.some(
