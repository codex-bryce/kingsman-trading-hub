...
const FAQContent = ({ faq_list, handleTabChange }: TFAQContent) => {
  ...
  const handleAccordionClick = () => {
    ...
    timer_id.current = setTimeout(() => {
      ...
      const previous_sibling_element =
        open_accordion_element?.previousElementSibling as HTMLElement;
      if (faq_wrapper_element.current && open_accordion_element) {
        const offset = previous_sibling_element
          ? previous_sibling_element.offsetTop - 80
          : 0;
        const desktop_scroll_element = document.querySelector(
          ".dc-tabs__content--tutorials",
        ) as HTMLElement;
        const mobile_scroll_element = document.querySelector(
          ".tutorials-mobile__faq",
        ) as HTMLElement;
        const scroll_element = isDesktop
