...
const BotBuilderTourMobile = observer(() => {
  ...
  const test_id =
    tour_step === 3 ? "finish-bot-builder-tour" : "next-bot-builder-tour";

  React.useEffect(() => {
    setTourActiveStep(tour_step);
    //component does not rerender so calling this to highlight
    !show_mobile_tour_dialog &&
      highlightLoadModalButton(active_tour, tour_step);
  ...
  const tour_button_text =
    tour_step === 3 ? localize("Finish") : localize("Next");
            ...
            <div className="dbot-slider__progress-bar">
              {
                <ProgressBarTracker
                  step={tour_step}
                  steps_list={BOT_BUILDER_MOBILE.map((v) =>
                    v.tour_step_key.toString(),
                  )}
