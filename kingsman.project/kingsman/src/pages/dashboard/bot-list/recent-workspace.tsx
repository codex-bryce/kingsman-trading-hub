...
export const CONTEXT_MENU = [
  {
    type: STRATEGY.OPEN,
    icon: (
      <LabelPairedPageCircleArrowRightSmRegularIcon fill="var(--text-general)" />
    ),
...
const RecentWorkspace = observer(({ workspace, index }: TRecentWorkspace) => {
  ...
  React.useEffect(() => {
    ...
    const select_first_strategy =
      dashboard_strategies?.length &&
      index === 0 &&
      !is_div_triggered_once.current;
  ...
  return (
    ...
    >
      ...
      <div className="bot-list__item__load-type">
        {getRecentFileIcon(
          workspace.save_type,
          "bot-list__item__load-type__icon--active",
        )}
        ...
        <>
          ...
          <div
            className={classnames("bot-list__item__responsive", {
              ...
              "bot-list__item__responsive--min":
                dashboard_strategies.length <= 5,
            })}
          >
            {CONTEXT_MENU.map((item) => (
              <div
              ...
              >
                <div className="bot-list__item__responsive__menu__icon">
                  {item.icon}
                </div>
                <Text
                  color="prominent"
