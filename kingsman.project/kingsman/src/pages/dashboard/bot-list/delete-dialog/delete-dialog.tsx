...
const DeleteDialog = observer(() => {
  ...
  const resetStrategiesAfterDelete = async (
    deleted_strategy_id: string,
    updated_workspaces: Array<TStrategy>,
  ) => {
  ...
  const removeBotStrategy = async (strategy_id: string) => {
    ...
    const updated_workspaces = workspaces.filter(
      (strategy_from_workspace: TStrategy) =>
        strategy_from_workspace.id !== strategy_id,
    );
    setDashboardStrategies(updated_workspaces);
    // TODO: Need to move this to skeleton
    localForage.setItem(
      "saved_workspaces",
      LZString.compress(JSON.stringify(updated_workspaces)),
    );
