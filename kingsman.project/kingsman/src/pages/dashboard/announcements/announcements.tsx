...
const Announcements = observer(
  ({ is_mobile, is_tablet, handleTabChange }: TAnnouncements) => {
    const {
      load_modal: { toggleLoadModal },
      dashboard: { showVideoDialog },
      quick_strategy: { setFormVisibility },
    } = useStore();
    const [is_announce_dialog_open, setIsAnnounceDialogOpen] =
      React.useState(false);
    const [is_open_announce_list, setIsOpenAnnounceList] =
      React.useState(false);
    const [selected_announcement, setSelectedAnnouncement] =
      React.useState<TAnnouncement | null>(null);
    const [read_announcements_map, setReadAnnouncementsMap] = React.useState(
    ...
    const storeDataInLocalStorage = (
      updated_local_storage_data: Record<string, boolean>,
    ) => {
      localStorage?.setItem(
        "bot-announcements",
        JSON.stringify(updated_local_storage_data),
      );
    };
    ...
    const modalButtonAction =
      (announce_id: string, announcement: TAnnouncement) => () => {
        setSelectedAnnouncement(announcement);
        setIsAnnounceDialogOpen(true);
        setIsOpenAnnounceList((prev) => !prev);
        rudderStackSendAnnouncementClickEvent({
          announcement_name: announcement.announcement.main_title,
        });
    ...
    const updateNotifications = () => {
      ...
      if (allUserAccounts) {
        ...
        const currentAccount = allUserAccounts?.find(
          (account) => account.loginid == loggedInAccountId,
        );
          ...
          tmp_notifications.push({
            ...
            message: (
              <MessageAnnounce
                ...
                date={item.date}
                announce={is_not_read}
              />
            ),
            buttonAction: performButtonAction(
              item,
              modalButtonAction,
              handleRedirect,
            ),
    ...
    React.useEffect(() => {
      const number_ammount_active_announce = Object.values(
    ...
    const openAccumulatorsVideo = () => {
      const accumulators_video = guide_content().find(
        (guide_content) => guide_content.id === 4,
      );
    ...
    const handleOnCancel = () => {
      rudderStackSendAnnouncementActionEvent({
        ...
        announcement_action:
          selected_announcement?.announcement.cancel_button_text,
      });
    ...
    const handleOnConfirm = () => {
      rudderStackSendAnnouncementActionEvent({
        announcement_name: selected_announcement?.announcement.main_title,
        announcement_action:
          selected_announcement?.announcement.confirm_button_text,
      });
      ...
      <div className="announcements">
        ...
        >
          ...
          {amount_active_announce !== 0 && (
            <div className="announcements__amount">
              <p data-testid="announcements__amount">
                {amount_active_announce}
              </p>
            </div>
          )}
        </button>
        ...
        {selected_announcement?.announcement && (
          <AnnouncementDialog
            ...
            handleOnCancel={
              !selected_announcement?.should_not_be_cancel
                ? handleOnCancel
                : null
            }
