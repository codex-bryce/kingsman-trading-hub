...
const StopBotModalContent = ({
  ...
  const confirm_button_text =
    is_running && is_multiplier
      ? localize("Keep my contract")
      : localize("Stop my bot");
  const cancel_button_text =
    is_running && is_multiplier
      ? localize("Close my contract")
      : localize("Back");
  const title_text =
    is_running && is_multiplier
      ? localize("Keep your current contract?")
      : localize("Stop your current bot?");
  const toggle_dialog_or_stop =
    is_running && is_multiplier ? closeMultiplierContract : toggleStopBotDialog;
  ...
  return (
    <React.Fragment>
      <Dialog
        ...
        title={title_text}
        is_visible={
          is_running ? is_stop_bot_dialog_open : is_contract_dialog_open
        }
