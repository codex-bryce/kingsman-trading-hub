...
const Diffbot = observer(() => {
  ...
  useEffect(() => {
    const init = async () => {
      const { api } = await getStandaloneOAuthAwareApi("diffbot");
      apiRef.current = api;
  ...
  const subscribeToTicks = async () => {
    ...
    const analysisNum =
      typeof analysisCount === "string"
        ? parseInt(analysisCount) || 100
        : analysisCount;
    ...
    try {
      ...
      const onMsg = (evt) => {
        try {
          ...
          if (
            data?.forget?.id &&
            data?.forget?.id === tickSubscription.current
          ) {
  ...
  const placeTrades = async () => {
    ...
    const numContracts =
      typeof numberOfContracts === "string"
        ? parseInt(numberOfContracts) || 1
        : numberOfContracts;
    if (numContracts < 1) {
    ...
    const stakeAmount =
      typeof stake === "string" ? parseFloat(stake) || 0.5 : stake;
    if (stakeAmount <= 0) {
    ...
    try {
      ...
      const tradePromises = Array.from(
        { length: numContracts },
        async (_, index) => {
          try {
            const buy_req = {
      ...
      <div className="diffbot-content">
        <div className="diffbot-controls">
          ...
          <div className="control-group">
            ...
            <input
              ...
              onChange={(e) =>
                setNumberOfContracts(parseInt(e.target.value) || 1)
              }
              disabled={isPlacingTrades}
            />
          </div>
          ...
          <div className="control-group">
            ...
            <input
              ...
              value={analysisCount}
              onChange={(e) =>
                setAnalysisCount(parseInt(e.target.value) || 100)
              }
              disabled={isPlacingTrades}
            />
          </div>
          ...
          <div className="control-group">
            ...
            <div className="current-digit-display">
              <span
                className={`digit-value ${currentDigit === predictedDigit ? "predicted-digit" : ""}`}
              >
            ...
            <div className="prediction-display">
              ...
              {predictedDigit !== null && digitStats.length > 0 && (
                <span className="prediction-info">
                  (Appeared{" "}
                  {digitStats.find((s) => s.digit === predictedDigit)?.count ||
                    0}{" "}
                  times)
                </span>
              )}
          ...
          <div className="control-group">
            <button
              ...
              onClick={() =>
                predictedDigit !== null && setSelectedDigit(predictedDigit)
              }
        ...
        <div className="diffbot-info">
          ...
          <ul>
            ...
            <li>
              Each contract uses <strong>DIGITDIFF</strong> - you{" "}
              <strong>WIN</strong> if the result digit is <strong>NOT</strong>{" "}
              the selected digit
            </li>
            <li>
              Example: If you select digit 3, you win if result is
              0,1,2,4,5,6,7,8,9 (any digit except 3)
            </li>
          </ul>
          ...
          <ul>
            ...
            <li>
              Current digit is highlighted in red if it matches the prediction
            </li>
            <li>
