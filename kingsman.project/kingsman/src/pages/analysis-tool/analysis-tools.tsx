...
type AnalysisToolSubTab = "dcircles" | "signals" | "analysis-tool";
...
const AnalysisTools: React.FC = () => {
  ...
  const readFromWorkspace = () => {
    try {
      ...
      if (!workspace) return;
      const trade_def = workspace
        .getAllBlocks(false)
        .find((b: any) => b.type === "trade_definition");
      if (!trade_def) return;
      ...
      const market_block = trade_def.getChildByType?.(
        "trade_definition_market",
      );
      const trade_type_block = trade_def.getChildByType?.(
        "trade_definition_tradetype",
      );
      const contract_block = trade_def.getChildByType?.(
        "trade_definition_contracttype",
      );
      const trade_options_block = trade_def.getChildByType?.(
        "trade_definition_tradeoptions",
      );
      const candle_block = trade_def.getChildByType?.(
        "trade_definition_candleinterval",
      );

      const market = cleanValue(
        market_block?.getFieldValue("MARKET_LIST"),
        tradeConfig.market,
      );
      const submarket = cleanValue(
        market_block?.getFieldValue("SUBMARKET_LIST"),
        tradeConfig.submarket,
      );
      const symbol = cleanValue(
      ...
      const tradeType = cleanValue(
        trade_type_block?.getFieldValue("TRADETYPE_LIST"),
        tradeConfig.tradeType,
      );
      const contract = cleanValue(
        contract_block?.getFieldValue("TYPE_LIST"),
        tradeConfig.contract,
      );
      const durationUnit =
        durationUnitMap[
          trade_options_block?.getFieldValue("DURATIONTYPE_LIST") || ""
        ] || tradeConfig.durationUnit;
      const duration = getNumberFromInput(
        trade_options_block,
        "DURATION",
        tradeConfig.duration,
      );
      const stake = getNumberFromInput(
        trade_options_block,
        "AMOUNT",
        tradeConfig.stake,
      );
      ...
      let prediction = getNumberFromInput(
        trade_options_block,
        "PREDICTION",
      ...
      const stakeVar = findVariableNumber(
        workspace,
        ["stake"],
        tradeConfig.stake,
      );
  ...
  const setNumberInput = (
    ...
    inputName: string,
    value: string,
    blockType = "math_number_positive",
  ) => {
  ...
  const writeToWorkspace = () => {
    try {
      ...
      const selectedMarket = ensureValueInOptions(
        tradeConfig.market,
        marketOptions,
        marketOptions[0]?.[1],
      );
      ...
      const submarketOptions =
        active?.getSubmarketDropdownOptions?.(selectedMarket) || [];
      ...
      const symbolOptions =
        active?.getSymbolDropdownOptions?.(selectedSubmarket) || [];
      const selectedSymbol = ensureValueInOptions(
        tradeConfig.symbol,
        symbolOptions,
        symbolOptions[0]?.[1],
      );
      ...
      const selectedTtc = ensureValueInOptions(
        tradeConfig.tradeTypeCategory,
        ttcOptions,
        ttcOptions[0]?.[1],
      );
      ...
      const selectedTt = ensureValueInOptions(
        tradeConfig.tradeType,
        ttOptions,
        ttOptions[0]?.[1],
      );
      ...
      const ctOptions =
        options.contractTypes.length && options.tradeTypes.length
          ...
          : fallbackData.contractTypesByTradeType[normalizeKey(selectedTt)] ||
            [];
      const selectedCt = ensureValueInOptions(
        tradeConfig.contract,
        ctOptions,
        ctOptions[0]?.[1],
      );
      if (selectedCt && selectedCt !== "Select")
        contract_block?.setFieldValue(selectedCt, "TYPE_LIST");

      trade_options_block?.setFieldValue(
        durationUnitMap[tradeConfig.durationUnit] || "t",
        "DURATIONTYPE_LIST",
      );
  ...
  const applyFallbacks = () => {
    setOptions((prev) => {
      ...
      const normalizedTradeTypeCat = normalizeKey(
        tradeConfig.tradeTypeCategory,
      );
      ...
      const submarkets =
        prev.submarkets.length || !normalizedMarket
          ? prev.submarkets
          : fallbackData.submarketsByMarket[normalizedMarket] ||
            prev.submarkets;

      const symbols =
        prev.symbols.length || !normalizedSubmarket
          ? prev.symbols
          : fallbackData.symbolsBySubmarket[normalizedSubmarket] ||
            prev.symbols;
      ...
      const tradeTypes =
        prev.tradeTypes.length || !normalizedTradeTypeCat
          ? prev.tradeTypes
          : fallbackData.tradeTypesByCategory[normalizedTradeTypeCat] ||
            prev.tradeTypes;

      const contractTypes =
        prev.contractTypes.length || !normalizedTradeType
          ? prev.contractTypes
          : fallbackData.contractTypesByTradeType[normalizedTradeType] ||
            prev.contractTypes;
    ...
    setTradeConfig((prev) => {
      ...
      const subOpts =
        fallbackData.submarketsByMarket[normalizeKey(next.market)] || [];
      if (
        (!normalizedSubmarket || normalizedSubmarket === "not available") &&
        subOpts.length
      ) {
      ...
      const symOpts =
        fallbackData.symbolsBySubmarket[normalizeKey(next.submarket)] || [];
      if (
        (!normalizedSymbol || normalizedSymbol === "not available") &&
        symOpts.length
      ) {
      ...
      const typeOpts =
        fallbackData.tradeTypesByCategory[
          normalizeKey(next.tradeTypeCategory)
        ] || [];
      if (
        (!normalizedType || normalizedType === "not available") &&
        typeOpts.length
      ) {
      ...
      const cOpts =
        fallbackData.contractTypesByTradeType[normalizeKey(next.tradeType)] ||
        [];
      if (
        (!normalizedContract || normalizedContract === "not available") &&
        cOpts.length
      ) {
    ...
    try {
      ...
      if (level === "tradeType") {
        ...
        if (
          ...
          tt =
            fallbackData.tradeTypesByCategory[
              normalizeKey(tradeConfig.tradeTypeCategory)
            ] || [];
        }
      ...
      if (level === "contractType") {
        ...
        if (contracts?.getContractTypes && tradeConfig.tradeType) {
          const result =
            contracts.getContractTypes(tradeConfig.tradeType) || [];
          ct = result.map((c: any) => [c.text, c.value]);
        } else {
          ct =
            fallbackData.contractTypesByTradeType[
              normalizeKey(tradeConfig.tradeType)
            ] || [];
        }
  ...
  useEffect(() => {
    ...
    if (!tradeConfig.tradeTypeCategory && options.tradeTypeCategories.length) {
      setTradeConfig((p) => ({
        ...p,
        tradeTypeCategory: options.tradeTypeCategories[0][1],
      }));
    }
  ...
  return (
    <div className="analysis-tools">
      ...
      <div className="analysis-tools__actions">
        <button
          type="button"
          className="analysis-tools__trade-button"
          onClick={() => setShowTradeConfig(true)}
        >
          Trading Configuration
        </button>
      </div>
      {active_tool && (
        <div className="analysis-tools__content">{renderContent()}</div>
      )}
      {show_trade_config && (
        <div
          className="analysis-tools__trade-modal-backdrop"
          ...
          >
            ...
            <div className="analysis-tools__trade-grid">
              <label>
                ...
                <select
                  value={
                    volatilityOptions.some(
                      (opt) => opt.value === tradeConfig.symbol,
                    )
                      ? tradeConfig.symbol
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTradeConfig((p) => ({ ...p, market: e.target.value }))
                  }
                >
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTradeConfig((p) => ({
                      ...p,
                      tradeTypeCategory: e.target.value,
                    }))
                  }
                >
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTradeConfig((p) => ({ ...p, tradeType: e.target.value }))
                  }
                >
              ...
              <label>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setTradeConfig((p) => ({ ...p, contract: e.target.value }))
                  }
                >
              ...
              <label>
                ...
                <div className="analysis-tools__trade-inline">
                  <input
                    ...
                    onChange={(e) =>
                      setTradeConfig((p) => ({ ...p, stake: e.target.value }))
                    }
                  />
                  <select
