...
const Hyperbot = observer(() => {
  ...
  useEffect(() => {
    const init = async () => {
      const { api } = await getStandaloneOAuthAwareApi("hyperbot");
      apiRef.current = api;
  ...
  useEffect(() => {
    const thresh =
      typeof threshold === "string" ? parseInt(threshold) || 5 : threshold;
    setPredictions((prev) =>
      prev
        .map((p) => {
  ...
  const updatePredictionStake = (digit, stake) => {
    setPredictions((prev) =>
      prev.map((p) => (p.digit === digit ? { ...p, stake } : p)),
    );
  };
  ...
  const togglePrediction = (digit) => {
    ...
    if (existing?.enabled) {
      ...
      const contractType =
        digit < thresh ? "DIGITOVER" : digit > thresh ? "DIGITUNDER" : null;
  ...
  const startAutoTrading = async () => {
    ...
    enableEntryPointRef.current = enableEntryPoint;
    const entryDigitValue =
      typeof entryPointDigit === "string"
        ? parseInt(entryPointDigit) || 5
        : entryPointDigit;
    ...
    if (enableEntryPoint) {
      console.log(
        `🚀 Starting auto trading with entry point enabled. Waiting for digit: ${entryDigitValue}`,
      );
      console.log(
      ...
      console.log(
        `🚀 Starting auto trading without entry point (immediate trading)`,
      );
    }
          ...
          if (Math.random() < 0.01) {
            ...
            const current =
              typeof currentDigit === "string"
                ...
                : currentDigit;
            console.log(
              `⏳ Waiting for entry point digit ${entryDigit}. Current digit: ${current}`,
            );
          }
          return;
        } else {
  ...
  const totalStake = predictions
    ...
    .reduce((sum, p) => {
      ...
      const stakeValue =
        typeof stake === "number" ? stake : parseFloat(String(stake));
        ...
        <div className="ou-stats-container">
          ...
          <div className="ou-stat-card ou-stat-equal">
            ...
            <div className="ou-stat-value">
              ...
              <span className="ou-stat-pct">
                ({overUnderStats.equalPct.toFixed(1)}%)
              </span>
            </div>
            <div className="ou-barline">
              <div
          ...
          <div className="ou-stat-card ou-stat-over">
            ...
            <div className="ou-barline">
              <div
                className="ou-fill ou-fill-over"
                style={{ width: `${overUnderStats.overPct}%` }}
              ></div>
            </div>
          </div>
        </div>
              ...
              <input
                ...
                onBlur={(e) => {
                  ...
                  if (value === "" || isNaN(num) || num < 0 || num > 9) {
                    ...
                    entryPointDigitRef.current = num;
                    console.log(
                      `📝 Entry point digit set to: ${num} (type: ${typeof num})`,
                    );
                  }
                }}
                min={0}
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      step={0.01}
                      disabled={
                        !isEnabled || useDefaultStake || isPlacingTrades
                      }
                      placeholder="0.35"
                    />
                  </td>
