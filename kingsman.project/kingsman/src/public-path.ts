...
const setSurvicateUserAttributes = (
  country: string,
  type: string,
  creationDate: string,
) => {
  if (window.Survicate) {
    ...
    if (creationDate)
      window.Survicate.track("accountCreationDate", creationDate);
  }
};
...
const initSurvicate = () => {
  ...
  const client_accounts =
    JSON.parse(localStorage.getItem("accountsList") as string) || undefined;
  const setAttributesIfAvailable = () => {
    if (active_loginid && client_accounts) {
      const { residence, account_type, created_at } =
        client_accounts[active_loginid] || {};
